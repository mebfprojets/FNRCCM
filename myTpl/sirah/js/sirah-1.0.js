...
SIRAH.basePath          = SIRAH.basePath || (SIRAH.basePath="/projectbase/templates/sirah/");
    ...
    if(  jQuery.isArray(messages) && !SIRAH.util.isEmpty(messages) ) {    	
         ...
         {	
           ...
           var errorRemoveIcon = jQuery("<i class='glyphicon glyphicon-remove'></i>").addClass("close")
                                                                                     .bind("click", function(e) { e.preventDefault();msgSelector.fadeOut("slow");});
...
jQuery(document).ready(function(){
      jQuery('#phone1').numeric();
	  jQuery('#phone2').numeric();
	  jQuery('#tel_mob').numeric();
	  jQuery('#tel_dom').numeric();
	  jQuery('#tel_bureau').numeric();
