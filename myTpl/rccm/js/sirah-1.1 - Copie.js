...
SIRAH.basePath         = SIRAH.basePath || (SIRAH.basePath="/erccm/myTpl/erccm/");
...
SIRAH.error.raise = function( selector , messages , type )
{
    ...
    if( jQuery.isArray(messages) && !SIRAH.util.isEmpty(messages)){    	
        ...
        {	
			...
			var errorRemoveIcon = jQuery("<i class='glyph-icon alert-close-btn icon-remove'></i>").css({cursor:"pointer"}).addClass("close").bind("click",function(e){e.preventDefault();msgSelector.fadeOut("slow");});
			msgSelector.append("<strong>"+SIRAH.util.strtoupper(SIRAH.util.text(msgType))+" : </strong> "+SIRAH.util.text(sMsg)).append(errorRemoveIcon);                     
    ...
    if ( !SIRAH.util.isUndefined(selector.attr("id")) ) {
		  if( parseInt(selector.height()) > 300 ) {
		      ...
		      messagesSelector.attr('style','position:absolute;display:none;top:'+messageSelectorTop+'px;left:'+messageSelectorLeft+'px;width:'+messageSelectorWidth+'px; z-index:6000')
			                  .appendTo("body");
	          messagesSelector.fadeToggle({duration:20000,easing:"swing",done:function(){messagesSelector.fadeToggle({duration:30000});}});
		  } else {
...
var basicLoadPage = function(){
jQuery('a[href="#"]').click(function(event) {event.preventDefault();});
jQuery('.sirah-ui-table:first').find('input[type=\'checkbox\']').bind('checkAll', function(event){enableButtons(['editButton','exportButtonDropdown','exportButton','deleteButton','releveButton']);});
jQuery('.sirah-ui-table:first').find('input[type=\'checkbox\']').bind('unCheckAll',function(event){disableButtons(['editButton','exportButtonDropdown','exportButton','deleteButton','releveButton']);});
jQuery('.sirah-ui-table:first').find('input[type=\'checkbox\']').bind('clickRow',function(event){enableButtons(['editButton','exportButtonDropdown','exportButton','deleteButton','releveButton']);});
jQuery('.sirah-ui-table:first').find('input[type=\'checkbox\']').bind('unClickRow',function(event){
	checkedItems = jQuery('.sirah-ui-table:first').find('input[type=\'checkbox\'][name!=\'checkall\']:checked');
	if( checkedItems.length <= 0 ) {
		disableButtons(['editButton','exportButton','exportButtonDropdown','deleteButton','releveButton']);
	}
});
 var scrollTop = jQuery(document).scrollTop();
  if( scrollTop > 0){
	  ...
	  jQuery('.toolsBar').width(jQuery('.pagerow').width());
  }
jQuery('table > tbody > tr > td > input, table > thead > tr > th > input').iCheck({checkboxClass:'icheckbox_minimal-blue',radioClass: 'radio_minimal-blue'});
jQuery('#checkAll').closest("div").attr("title", "Tout sélectionner").attr("data-toggle","tooltip");
jQuery('#checkAll,#reinscriptionCheckAll').on('ifChecked', function(event){
...
jQuery('#checkAll,#reinscriptionCheckAll').on('ifUnchecked', function(event){
  ...
  jQuery(document).scroll(function(e){
    ...
    if( scrollTop > 0){		
        ...
        if( jQuery("#checkboxBtn").attr("id") !== undefined) {
			...
			var checkBoxBtn  = jQuery("<div></div>").addClass("btn-group").attr("id", "checkboxBtn");
		}		
		checkBoxRow.appendTo(checkBoxBtn);
		checkBoxBtn.prependTo(jQuery(".sirah-ui-buttonbar").eq(0));
		jQuery('.sirah-ui-table').find("thead:first").find("tr").eq(0).find("th").eq(0).html("&nbsp;");
        ...
        if( jQuery('#checkboxBtn').attr("id") !== undefined)  {
			...
			var firstCol    = jQuery('.sirah-ui-table').find("thead:first").find("tr").eq(0).find("th").eq(0);
			firstCol.html("");
			checkBoxRow.appendTo(firstCol);
			jQuery('#checkboxBtn').empty();		
		}
        jQuery("#checkAll").iCheck({checkboxClass:'icheckbox_minimal-blue',radioClass: 'radio_minimal-blue'});
        jQuery('#checkAll').on('ifChecked', function(event){
	            jQuery('.sirah-ui-table').find('input').iCheck('check');
	            jQuery('.sirah-ui-table').find('tbody > tr').addClass('checkedRow').removeClass("unCheckedRow");
	            jQuery(this).trigger('checkAll');
...
jQuery(".dialogViewInfos").click(function(event){
	       ...
	       dialogViewInfos.dialog2({title:viewTitle,content:viewLink,autoOpen:false,modalClass:'modal-wide',removeOnClose:true,closeOnEscape:false,closeOnOverlayClick:true,showCloseHandle:true,initialLoadText:'Chargement...'});        		
