...
SIRAH.basePath         = SIRAH.basePath || (SIRAH.basePath="/rccm/myTpl/default/");
...
SIRAH.error.raise = function( selector , messages , type )
{
    ...
    if(jQuery.isArray(messages) && !SIRAH.util.isEmpty(messages)){    	
         ...
         {	
          ...
          var errorRemoveIcon = jQuery("<i class='glyphicon glyphicon-remove'></i>").addClass("close").bind("click",function(e){e.preventDefault();msgSelector.fadeOut("slow");});
          msgSelector.append(errorRemoveIcon).append("<strong>"+SIRAH.util.strtoupper(SIRAH.util.text(msgType))+" : </strong> "+SIRAH.util.text(sMsg));                     
...
SIRAH.dialog.alert  = function(message , title) {	
    ...
    var alertRow = jQuery("<div id=\"alertRow\" class=\"row row-fluid\"></div>");
    var alertMsg = jQuery("<div class='alert alert-danger'><div class='bg-red alert-icon'><i class='glyph-icon icon-warning'></i></div> <div class='alert-content'> "+message+" </div></div>");
...
SIRAH.loading.on         = function( container ){	
    ...
    var loaderMarginLeft = ( SIRAH.util.isUndefined( containerOffset )) ? parseInt(parseInt(containerOffset.left)+10) : 10;
    var loaderMarginTop  = ( SIRAH.util.isUndefined( containerOffset )) ? parseInt(parseInt(containerOffset.top)-20)  : 10 ;
...
SIRAH.loading.off = function(remove) {
	...
	jQuery("#loadingBox").hide(); jQuery(".loadingBox").hide();
} ;
