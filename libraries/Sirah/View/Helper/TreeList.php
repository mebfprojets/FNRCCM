 ...
 */
  ...
  class Sirah_View_Helper_TreeList extends Sirah_View_Helper_TreeFormSelect
  {
    ...
    protected $_rootParent    = 0;
  	 ...
  	 public function treeList( $id, $value = null, $attributes = array(), $items = array() , $parent = 0 ,  )
  	 {
  	 ...
  	 public static function build( $options, $selectedValue , $level = 0 , $parent = 0 )
  	 {
  	 	$htmlOutput  = "";
  	 	if( count(   $options )) {
  	 		foreach( $options as $itemId => $optionItem ) {
  	 			     ...
  	 			     $dash       = ( $parent != 0 ) ? str_repeat("--", $level ) : "";
  	 			     ...
  	 			     if( !empty( $children)) {
  	 			     	 $level++;
  	 			     	 $htmlOutput .= Sirah_View_Helper_TreeFormSelect::build( $children, $selected, $level, $itemId );
  	 			     } 	 			       	 			      	 			
  	 		}
  	 	}
  	 	return $htmlOutput;
