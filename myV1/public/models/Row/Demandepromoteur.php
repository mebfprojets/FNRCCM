...
class Model_Demandepromoteur extends Sirah_Model_Default
{
	...
	public function getRow($data = array())
	{
	    ...
	    if( isset($data["numidentite"])) {
			$selectPromoteur->where(  "D.numidentite=?",$filters["numidentite"])
			                ->orWhere("I.numero=?", $filters["numidentite"]);
		}
		if( isset($data["numero"])) {
			$selectPromoteur->where("I.numero=?",$filters["numero"]);
		}
		if( isset($data["date_etablissement"])) {
			$selectPromoteur->where("I.date_etablissement=?",$filters["date_etablissement"]);
		}
		if( isset($data["lieu_etablissement"])) {
			$selectPromoteur->where("I.lieu_etablissement=?",$filters["lieu_etablissement"]);
		}
		if( isset($data["organisme_etablissement"])) {
			$selectPromoteur->where("I.organisme_etablissement=?",$filters["organisme_etablissement"]);
		}
	...
	public function getList($filters = array() , $pageNum = 0 , $pageSize = 0, $orders=array("D.lastname ASC","D.firstname ASC"))
	{
		...
		$tablePrefix      = $modelTable->info("namePrefix");		
		$tableName        = $modelTable->info("name");
		$selectPromoteurs = $dbAdapter->select()->from(    array("D"=> $tableName))
		                                        ->join(    array("I"=> $tablePrefix."reservation_demandeurs_identite")      ,"I.identityid=D.identityid", array("I.typeid","numeroPiece"=>"I.numero","I.numero","I.organisme_etablissement","I.lieu_etablissement","I.date_etablissement"))
												->joinLeft(array("T"=> $tablePrefix."reservation_demandeurs_identite_types"),"T.typeid=I.typeid"        , array("typePiece"  =>"T.libelle"));
		...
		if( isset($filters["lastname"]) && !empty($filters["lastname"]) ){
			$selectPromoteurs->where("D.lastname LIKE ?","%".strip_tags($filters["lastname"])."%");
		}
		if(isset($filters["firstname"]) && !empty($filters["firstname"]) ){
			$selectPromoteurs->where("D.firstname LIKE ?","%".$filters["firstname"]."%");
		}
		...
		if( isset($filters["promoteurid"]) && intval($filters["promoteurid"])  ) {
			$selectPromoteurs->where("D.promoteurid = ?", intval( $filters["promoteurid"] ) );
		}
		if( isset( $filters["promoteurids"] ) && is_array( $filters["promoteurids"] )) {
			if( count( $filters["promoteurids"])) {
				$selectPromoteurs->where("D.promoteurid IN (?)", array_map("intval",$filters["promoteurids"]));
			}			
		}
	...
	{		
		...
		$selectPromoteurs  = $dbAdapter->select()->from(array("D"=> $modelTable->info("name")))
