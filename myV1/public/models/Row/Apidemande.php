...
class Model_Apidemande extends Sirah_Model_Default
{
	...
	public function getList($filters=array(),$pageNum=1, $pageSize=10, $orders=array("R.date DESC","R.demandeid DESC"))
	{
		$filters["paid"]= 1;
		$modelTable     = $this->getTable();
		$dbAdapter      = $modelTable->getAdapter();
		...
		$selectDemandes = $dbAdapter->select()->from(array("R" => $tablePrefix."reservation_demandes"))
											  ...
											  ->join(array("IP"=> $tablePrefix."reservation_demandeurs_identite") ,"IP.identityid=P.identityid", array("numeroPiecePromoteur"=>"IP.numero","organismeEtablissementPromoteur"=>"IP.organisme_etablissement","lieuEtablissementPromoteur"=>"IP.lieu_etablissement","dateEtablissementPromoteur"=>"IP.date_etablissement"))
											  ->join(array("E" => $tablePrefix."reservation_demandes_entreprises"),"E.demandeid=R.demandeid AND E.entrepriseid=R.entrepriseid AND E.nomcommercial=R.denomination",array("E.nomcommercial","E.sigle","E.denomination","E.formid","E.address","E.activite","E.telephone","E.domaineid"))
		...
		if( isset($filters["sync_demandeid"]) && ($filters["sync_demandeid"]!="-1") &&  ($filters["sync_demandeid"]!=null) &&  ($filters["sync_demandeid"]!=false)) {
			$selectDemandes->where("R.sync_demandeid = ?", intval( $filters["demandeid"] ));
		}
		if( isset($filters["synchronized"]) && ($filters["synchronized"]!="-1") &&  ($filters["synchronized"]==0) &&  ($filters["synchronized"]==1)) {
			if( $filters["synchronized"]==1 ) {
				$selectDemandes->where("R.sync_demandeid >0");
			}
			if( $filters["synchronized"]==0 ) {
				$selectDemandes->where("(R.sync_demandeid='') OR (R.sync_demandeid=NULL)");
			}
		}
		...
		if(!empty($orders) && is_array($orders) ) {
			...
			$selectDemandes->order(array("R.date DESC","R.demandeid DESC"));
		}
	...
	public function getListTest($filters=array(),$pageNum=0, $pageSize=0, $orders=array("R.expired ASC","R.date DESC","D.lastname ASC","D.firstname ASC"))
	{
		$modelTable     = $this->getTable();
		$dbAdapter      = $modelTable->getAdapter();
		$tablePrefix    = $modelTable->info("namePrefix");		
