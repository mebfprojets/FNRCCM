...
class RequetesController extends Sirah_Controller_Default
{
	...
	public function verifyAction()
	{
		...
		if( $this->_request->isPost()  ) {
			...
			if( isset($verificationStateStore->verificationCounter) && ($verificationStateStore->verificationCounter > 150)) {
				if( $this->_request->isXmlHttpRequest()) {
					$this->_helper->viewRenderer->setNoRender(true);
					$this->_helper->layout->disableLayout(true);
				...
				$this->setRedirect("Vous avez déjà fait 150 vérifications. Veuillez patienter quelques jours" , "error");
				$this->redirect("public/index/index");
			}
					...
					if( count(   $reservedEntreprises) ) {
						...
						if( count(   $blacklisted)) {
							...
							foreach( $blacklisted as $item ) {
								     $keywordsCleaned     = $modelRegistre->cleanName($keywords);
