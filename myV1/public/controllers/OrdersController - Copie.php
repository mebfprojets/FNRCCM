...
class OrdersController extends Sirah_Controller_Default
{
	...
	public function createAction()
	{
		...
		if( $this->_request->isPost()  ) {
			...
			if( count($errorMessages) ) {
				...
				$orderCart->commandeValeur     = $montantTotalTTC;
	...
	public function updateAction()
	{
		...
		if( count($errorMessages) && $invoiceid) {
			...
			$orderCart->commandeid              = $commandeid;
			$orderCart->invoiceid               = $invoiceid;
			$orderCart->commandeValeur          = $commandeValeur;
	...
	public function checkoutAction()
	{
		...
		$montantTotal                           = $commandeRow->valeur_ttc;
		...
		if(!empty($transid) && !count($commandePaiements)) {
			...
			if( $dbAdapter->insert( $prefixName."erccm_vente_modepaiements", $modePaiementData)) {
				...
				if( $dbAdapter->insert( $prefixName."erccm_vente_modepaiements_web",$webPaiementData)) {
					...
					$paiementData["montant"]             = $montantTotal = $commandeRow->valeur_ttc;
					$paiementData["totalAPayer"]         = $montantTotal = $commandeRow->valeur_ttc;
			...
			$modePaiementData["banque"]      = "LIGDICASH";
			$modePaiementData["address"]     = ($invoiceAddress)? sprintf("%s,email:%s,tel:%s",$invoiceAddress->address,$invoiceAddress->email,$invoiceAddress->phone) : sprintf("%s,email:%s,tel:%s",$member->address,$member->email,$member->tel1);
			$modePaiementData["montant"]     = $montantTotal = $commandeRow->valeur_ttc;
			$modePaiementData["processed"]   = 0;
			$modePaiementData["date"]        = date("Y-m-d");
		...
		try {
			...
			$notifyURL        = $this->_helper->HttpUri(array("scheme"=>"https","module"=>"public","controller"=>"payments","action"=>"validate","params"=> array("id"=>$paiementId,"orderid"=>$commandeid,"trans_id"=>$transid)));
			...
			$payload["invoice"]["items"]["name"]         = "Frais de Transaction";
			$payload["invoice"]["items"]["description"]  = sprintf("Frais de transaction de %d%% reversés à LIGDICASH", PAYMENT_GATEWAY_API_SHIPPING_COST);
			$payload["invoice"]["items"]["quantity"]     = 1;
			$payload["invoice"]["items"]["unit_price"]   = $fraisTransaction;
			$payload["invoice"]["items"]["total_price"]  = $fraisTransaction;
			
			$requestClient->setMethod(Zend_Http_Client::POST);
			$requestClient->setRawData( $payload, 'application/json');
			...
			if( isset($paymentGatewayResultDataJson["response_code"]) && isset($paymentGatewayResultDataJson["response_text"]) && ($paymentGatewayResultDataJson["response_code"]=="00")) {
			    $paiement_url             = $paymentGatewayResultDataJson["response_text"];
	...
	public function finalizeAction()
	{
		...
		if(!isset($orderCart->commande) || !isset($orderCart->commandeid) ||
		   !isset($orderCart->commande["ref"]) || !intval($orderCart->commandeid)) {
			$this->setRedirect("Veuillez valider d'abord la commande","error");
			$this->redirect("public/orders/create");
		}
		if(!isset($orderCart->paiement) || (!isset($orderCart->paiement["transid"])) ) {
			$this->setRedirect("Veuillez procéder d'abord au paiement","error");
			$this->redirect("public/orders/checkout");
		}
		$model                         = $this->getModel("commande");
		$modelPaiement                 = $this->getModel("commandepaiement");
		...
		$apipayNumOrder                = (isset($requestParams["num_command"]            ))? strip_tags(substr($requestParams["num_command"],0,25))             : 0;
		$apiNumTrans                   = (isset($requestParams["num_transaction_from_gu"]))? strip_tags(substr($requestParams["num_transaction_from_gu"],0,25)) : 0;
		$touchPayAmount                = (isset($requestParams["amount"]                 ))? floatval(  substr($requestParams["amount"],0,30))                  : 0;
		$transactionPaiement           = (!empty($apipayNumOrder))? $modelPaiement->transaction($apipayNumOrder)  : false;
		$checkCommandeId               = ($transactionPaiement     )? ($commandeid==$transactionPaiement->commandeid) : false;
		if(($paiementTransId != $apipayNumOrder) || !$transactionPaiement || ($checkCommandeId==false)) {
			if( $this->_request->isXmlHttpRequest()) {
				echo ZendX_JQuery::encodeJson(array("error"=> "Les références de cette transaction ne sont pas valides. Veuillez procéder au paiement."));
				exit;
			}
		...
		$commandeRow                   = ($commandeid)? $model->findRow($commandeid,"commandeid",null,false) : null;
		...
		$paiementDataUpdated           = array("totalPaid"=>$totalPaid,"totalAPayer"=>$totalAPayer,"frais_transaction"=>$fraisTransaction,
		                                       "statutid"=>2,"validated"=>1,"num_transaction"=>$apiNumTrans,"num_commande"=>$apipayNumOrder,"updatedate"=>time(),"updateduserid"=>26);
		try {
