...
class UserdocumentsController extends Sirah_Controller_Default
{
	...
	public function infosAction()
	{
        ...
        if(($me->userid != $document->userid) && ($me->userid != $document->creatorid)) {
			if( $this->_request->isXmlHttpRequest() ) {
				$this->_helper->viewRenderer->setNoRender(true);
				$this->_helper->layout->disableLayout( true );
				echo ZendX_JQuery::encodeJson( array("error" => "Le document que vous souhaitez télécharger est inaccessible") );
				exit;
			}
			$this->setRedirect("Le document que vous souhaitez télécharger est inaccessible" , "error");
			$this->redirect("public/dashboard/list");
		}
		...
		if( !file_exists( $filename ) ) {
			...
			$this->redirect("public/userdocuments/list");
		}
