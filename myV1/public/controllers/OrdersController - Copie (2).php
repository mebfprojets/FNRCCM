...
class OrdersController extends Sirah_Controller_Default
{
	...
	public function checkoutAction()
	{
		...
		try {
			$fraisTransaction = (intval(PAYMENT_GATEWAY_API_SHIPPING_COST))?((PAYMENT_GATEWAY_API_SHIPPING_COST*$montantTotal)/100) : (($montantTotal*2)/100);
			$notifyURL        = $this->_helper->HttpUri(array("scheme"=>"https","module"=>"public","controller"=>"orders","action"=>"finalize","params"=> array("id"=>$paiementId,"orderid"=>$commandeid,"trans_id"=>$transid)));
		...
		try {
			...
			$dbAdapter->update( $prefixName."erccm_vente_commandes"          , array("valeur_ttc"=>$montantTotal,"valeur_ht"=>$montant_ht,"frais"=>$fraisTransaction), array("commandeid=?"=>$commandeid));
		} catch(Exception $e ) {
			$this->getHelper("Message")->addMessage( "Erreur technique sur la mise à jour des informations du paiement","error");
		}	
		//var_dump($paiementData); die();
		//On génère l'url de notification
		$notifyURL              = $this->_helper->HttpUri(array("scheme"=>"https","module"=>"public","controller"=>"orders","action"=>"finalize","params"=> array("id"=>$paiementId,"order_id"=>$commandeid),"orderid"=>$commandeid));
	...
	public function finalizeAction()
	{
		...
		$mToken           = (isset($orderCart->payment_token) && !empty($orderCart->payment_token))?$orderCart->payment_token : ((isset($requestParams["ligdicashtoken"]) && ($requestParams["ligdicashtoken"] <> ""))?$requestParams["ligdicashtoken"] : "");
		$order_id         = $commandeid = $orderCart->commandeid;
		$mOrder_id        = 0;
		if( empty($mToken)) {
			if( $this->_request->isXmlHttpRequest()) {
				$this->_helper->viewRenderer->setNoRender(true);
				$this->_helper->layout->disableLayout(true);
				echo ZendX_JQuery::encodeJson(array("error"=>"Veuillez enregistrer et valider d'abord la commande"));
				exit;
			}
			$this->setRedirect("Veuillez enregistrer et valider d'abord la commande","error");
			$this->redirect("public/orders/create");
		...
		$requestClient->setParameterGet("order_id", $order_id);
		$requestClient->setMethod(Zend_Http_Client::GET);
		$checkPaymentRes      = $requestClient->request();
		$checkPaymentData     = ($checkPaymentRes)? json_decode($checkPaymentRes, true) : array();
		...
		$errorMessage         = "Votre paiement a été refusé par {$operatorName}. Assurez-vous de disposer des ressources financières dans votre compte de paiement. Veuillez contacter l'opérateur ou l'administrateur.";
		
		if(!isset($checkPaymentData["response_code"]) || empty($checkPaymentData["response_code"]) || ($checkPaymentData["response_code"]=="00")) {
		    if( $this->_request->isXmlHttpRequest()) {
				$this->_helper->viewRenderer->setNoRender(true);
				$this->_helper->layout->disableLayout(true);
		...
		if( isset($checkPaymentData["status"]) && ($checkPaymentData["status"]=="nocompleted")) {
			$orderCart->commande       = array();
			$orderCart->invoice        = array();
			$orderCart->invoiceAddress = array();
			$orderCart->commandeItems  = array();
			...
			if( $this->_request->isXmlHttpRequest()) {
				$this->_helper->viewRenderer->setNoRender(true);
				$this->_helper->layout->disableLayout(true);
				echo ZendX_JQuery::encodeJson(array("info"=>"Votre paiement a été refusé par {$operatorName}. Votre commande sera validée après un traitement favorable de l'opérateur"));
				exit;
			}
			$this->setRedirect("Votre paiement a été refusé par {$operatorName}. Votre commande sera validée après un traitement favorable de l'opérateur","info");
			$this->redirect("public/members/dashboard");
		}
		if( isset($checkPaymentData["status"]) && ($checkPaymentData["status"]=="pending")) {
			$orderCart->commande       = array();
			$orderCart->invoice        = array();
			$orderCart->invoiceAddress = array();
			$orderCart->commandeItems  = array();
			...
			if( $this->_request->isXmlHttpRequest()) {
				$this->_helper->viewRenderer->setNoRender(true);
				$this->_helper->layout->disableLayout(true);
				echo ZendX_JQuery::encodeJson(array("info"=>"Votre paiement est en attente de validation par {$operatorName}. Votre commande sera validée après un traitement favorable de l'opérateur"));
				exit;
			}
			$this->setRedirect("Votre paiement est en attente de validation par {$operatorName}. Votre commande sera validée après un traitement favorable de l'opérateur","info");
			$this->redirect("public/members/dashboard");
		}
		...
		if( isset($custom_data["valueof_customdata"]) && !empty($custom_data["valueof_customdata"])) {
			$mOrder_id = $custom_data["valueof_customdata"];
			
			if( $mOrder_id   != $order_id) {
				$errorMessage = "Votre paiement semble invalide et n'est pas associé à la commande du panier";
				if( $this->_request->isXmlHttpRequest()) {
					$this->_helper->viewRenderer->setNoRender(true);
					$this->_helper->layout->disableLayout(true);
					echo ZendX_JQuery::encodeJson(array("error"=>$errorMessage));
				...
				$commandeid   = intval($mOrder_id);
				$commandeRow  = $commande = $modelCommande->findRow(intval($mOrder_id),"commandeid",null,false);
			}
		} else {
		...
		if( null == $commande ) {
			$errorMessage     = "Votre commande ne semble pas  validé. Veuillez contacter l'administrateur de la plateforme";
			if( $this->_request->isXmlHttpRequest()) {
				$this->_helper->viewRenderer->setNoRender(true);
				$this->_helper->layout->disableLayout(true);
				echo ZendX_JQuery::encodeJson(array("error"=>$errorMessage));
		...
		if( $commandeRow->creatorid != $me->userid ) {
			$errorMessage     = "Votre commande ne semble pas  validé. Veuillez contacter l'administrateur de la plateforme";
			if( $this->_request->isXmlHttpRequest()) {
				$this->_helper->viewRenderer->setNoRender(true);
				$this->_helper->layout->disableLayout(true);
		...
		$invoiceid            = $orderCart->invoiceid;
		$paiementTransId      = $trans_id = (isset($orderCart->paiement["transid"]) )?$orderCart->paiement["transid"] : "";
		...
		if(!count($commandePaiements) || !$commandeRow || empty($paiementTransId)) {
			if( $this->_request->isXmlHttpRequest()) {
		...
		$fraisTransaction     = $orderCart->frais_transaction;
		$totalPaid            = $orderCart->montant;
		$totalAPayer          = $orderCart->montant;
		
		$paiementDataUpdated  = array( "totalPaid"=>$totalPaid,"totalAPayer"=>$totalAPayer,"frais_transaction"=>$fraisTransaction,
		                               "statutid" =>2,"validated"=>1,"num_transaction"=>$mToken,"num_commande"=>$mToken,"updatedate"=>time(),"updateduserid"=>26);
		try {
		...
		$orderCart->commandeid         = 0;
		$orderCart->invoiceid          = 0;
		$orderCart->counter            = $orderCart->montant = $orderCart->commandeValeur = $orderCart->frais_transaction = 0;
		$orderCart->initialised        = true;
		$orderCart->setExpirationSeconds(APPLICATION_ORDERCART_EXPIRATION);
