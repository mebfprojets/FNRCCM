...
{	
    ...
    public function ajaxsearchAction()
	{
		...
		$stateStore->setExpirationSeconds(2700);
		$stateStore->setExpirationSeconds(1800,"searchCounter");
		
		if( isset($stateStore->searchCounter) && ($stateStore->searchCounter>10)) {
			echo ZendX_JQuery::encodeJson(array("error"=>"Vous ne pouvez pas effectuer cette opération plus de 10 fois"));
		    exit;
		}
		...
		$rows         = $model->getList(array("searchQ"=>$searchQ,"type"=>$typeid,"types"=>array(1,2,3,4)), 1, $totalResults);
	...
	{		
		...
		&& !isset($stateStore->filters["_registres"]["numifu"])  && !isset($stateStore->filters["_registres"]["numcnss"])) {
			$stateStore->filters["_registres"] = array("searchQ"=>$searchQ,"q"=>null,"name"=>null,"libelle"=>null,"numifu"=>null,"numcnss"=>null,"numero"=>null,"domaineid"=>0,"localiteid"=>0,"annee"=>0,"nom"=>"","prenom"=>"","searchQ"=>"","date_year"=>null,"date_month"=>null,"date_day"=>null,"passport"=>null,"telephone"=>null,"name"=>null,"typeid"=>0,"type"=>0,"keywords"=>null,"maxitems"=>0);
		}
		...
		if( isset($stateStore->searchCounter) && ($stateStore->searchCounter>15)) {
			$this->setRedirect("Vous ne pouvez pas effectuer cette opération plus de 10 fois","error");
	...
	public function infosAction()
	{
        ...
        $dirigeants                = $registre->dirigeants($registreid);
		...
		$this->view->address       = $this->view->adresse = $registre->findParentRow("Table_Registreadresses");
