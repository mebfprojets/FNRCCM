...
class Model_Project extends Sirah_Model_Default
{
	...
	public function stats($projectid=1,$toArray=false)
	{
		$fetchType     = 5;
		if( $toArray==true ) {
			$fetchType = Zend_Db::FETCH_ASSOC;
		}
		$table         = $this->getTable();
		$dbAdapter     = $table->getAdapter();
		$tablePrefix   = $table->info("namePrefix");
		$selectProject = $dbAdapter->select()->from(array("S"=>$tablePrefix."rccm_projet_application_stats"))
		                                     ->join(array("P"=>$tablePrefix."rccm_projet_application"),"P.projectid=S.projectid",array("P.libelle","P.presentation_equipe","P.presentation_structure","P.presentation","P.introduction","P.context","P.objectifs","P.objectif_global","P.objectif_strategique"))
											 ->where("P.projectid=?",intval($projectid))
											 ->where("S.projectid=?",intval($projectid));
		return $dbAdapter->fetchRow( $selectProject, array() ,$fetchType);									 
	}
