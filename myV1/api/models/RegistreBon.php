...
class Model_Registre extends Sirah_Model_Default
{
	...
	{		 
		$emptyWords              = array("Burkina","burkina","faso","Le","LE","La","LA","SA","Sarl","SARL","du","Ets","Etablissement","Société","Entreprise","ENTREPRISE","Et","ET","Au","AU","DE","de","A","à","DU","BURKINA","FASO","of","OF",
		                                 "Frères","etablissement","société","Société","Societe","SOCIETE","ETABLISSEMENT","SARL","SA","Freres","FRERE","FRERES","Ouagadougou","Ouaga","OUAGADOUGOU","S.A.R.L","SARL","S.A","Union","Service","SERVICE","SERVICES","MULTISERVICES","Multi-service","MULTI-SERVICES","multiservices","multiservice","GENERAL");
	    return $emptyWords;
	}
	...
	public function insertsigue($data, $creatorid=26)
	{
		...
		$NumeroRCCM          = (isset($registreRow["NumeroRCCM"]         ))?$stringFilter->filter(preg_replace("/[^A-Za-z0-9]/","",$registreRow["NumeroRCCM"] )) : "";		
		...
		$Description         = (isset($registreRow["SecteurArticle"]    ))?$stringFilter->filter($registreRow["SecteurArticle"] )            : "";		
		$IdEntreprise        = (isset($registreRow["IdEntreprise"]       ))?$stringFilter->filter($registreRow["IdEntreprise"] )               : "";
		$IdArticle          = (isset($registreRow["IdArticle"]         ))?$stringFilter->filter($registreRow["IdArticle"] )                 : "";
		...
		$codeEntreprise      =  new Zend_Db_Expr("CAST(E.IdEntreprise AS VARCHAR(60))");
		 ...
		 if( empty($NomCommercial) ) {
			 $NomCommercial  = sprintf("%s %s", $DenominationSociale, $Sigle) ;
		 }
		 ...
		 if(($NomCommercial=="NULL") || !$strNotEmptyValidator->isValid($NomCommercial) ) {
			 ...
			 $NomCommercial  = sprintf(" %s (%s)", $NomCommercial, $Sigle );
		 }						 
		 ...
		 $numTypeRegistre    = strtoupper(trim(substr($NumeroRCCM, 9, 1)));								 
		 ...
		 if( $numTypeRegistre=="A") {
			 $rccmTypeId     = 1;
		 } elseif( $numTypeRegistre=="B") {
			 $rccmTypeId     = 2;
		 } elseif( $numTypeRegistre!=="A" && $numTypeRegistre!=="B") {
			 return false;
		 }		
		 ...
		 if( $foundCodeArticle               = $modelDomaine->findRow($registreRow["codeArticle"],"code", null, false)) {
			 $rccmDomaineId                   = $foundCodeArticle->domaineid;  
		 } elseif($foundLibArticle           = $modelDomaine->findRow($registreRow["libArticle"] ,"libelle", null, false)) { 
			 $rccmDomaineId                   = $foundLibArticle->domaineid;  
			 
		 } else {
			 $insert_domaine_data             = array("creatorid"=>$creatorid,"creationdate"=>time(),"updatedate"=>0,"updateduserid"=>0,"parentid"=>0);
			 $insert_domaine_data["code"]     = $stringFilter->filter($registreRow["codeArticle"]);
			 $insert_domaine_data["libelle"]  = $stringFilter->filter($registreRow["libArticle"]);
			 try {
		 ...
		 $insert_rccm_data["adresse"]         = $insert_rccm_data["telephone"] = $insert_rccm_data["params"]    = "";	
		...
		try {
			if( $dbAdapter->insert($tablePrefix."rccm_registre", $insert_rccm_data)) {
                ...
                if( $entrepriseid && ($registreRow["CodeTypePiece"]== "CNI" || $registreRow["CodeTypePiece"] == "CNIM" || $registreRow["CodeTypePiece"] == "PI" || $registreRow["CodeTypePiece"] == "PASS")) {
					...
					$promoteur_data["nom"]            = $stringFilter->filter( $registreRow["NomRaisonSociale"] );
					$promoteur_data["prenom"]         = sprintf("%s %s", $registreRow["Prenom"] , $registreRow["Surnom"]);
					...
					$promoteur_data["passport"]       = "";
					$promoteur_data["cnib"]           = "";
					$promoteur_data["sexe"]           = $stringFilter->filter( $registreRow["Sexe"] );
					...
					if( $promoteur_data["sexe"] =="Masculin" || $promoteur_data["sexe"] =="Homme") {
	...
	public function siguesearch( $filters = array(), $page=0, $pageSize=10, $useAPI = false)
	{
		...
		if( $useAPI == false ) {
			...
			$selectSQL = "SELECT R.[NumeroRCCM],R.[DateRCCM],E.NomCommercial,E.IdEntreprise,E.DenominationSociale,AC.NomArticle,AD.Mobile1,AD.Mobile2,AD.Tel1Domicile,AD.Tel2Domicile,U.DateNaissance,U.LieuNaissance,U.Sexe,E.IdEtat,I.NumeroIFU,CN.NumeroCNSS,CN.DateCNSS,REG.LibelleRegimeImposition AS Regime,
			                     R.[NumeroRCCM] AS numero,E.NomCommercial AS libelle,E.NombreActions,E.DontNumeraire,E.Capital,E.IdEntreprise,E.EffectifEmployePermanent,E.EffectifEmployeEtranger,I.DateImmatriculationIFU,I.DateCreation,I.BICA,I.BNC,I.TVA,EU.IdEntreprise,EU.IdUsager,
								 (SELECT R1.NumeroRCCM FROM [SIGU_Commun_PROD].[cefore].Entreprise E1,[SIGU_Commun_PROD].[cefore].[RCCM] R1 WHERE E1.IdRCCM=R1.IdRCCM AND E1.IdEntreprise=E.IdEntrepriseOrigine) AS 'Ancien RCCM',E.IdFormeJuridique,F.CodeFormeJuridique,
								 U.NomRaisonSociale,U.NomJeuneFille,U.NomJeuneFille,U.Prenom,U.Surnom,U.Fonction,U.DateNaissance,U.LieuNaissance,U.SituationMatrimoniale,U.Sexe,U.IdAdresse,U.DateNaissance,CONCAT(U.DateNaissance,' ','00:00:00') AS datenaissance
						    FROM [SIGU_Commun_PROD].[cefore].[RCCM] R							
							LEFT JOIN [SIGU_Commun_PROD].[cefore].Entreprise E  ON R.IdRCCM=E.IdRCCM
							LEFT JOIN [SIGU_Commun_PROD].[cefore].EntrepriseUsager EU ON EU.IdEntreprise=E.IdEntreprise
							LEFT JOIN [SIGU_Commun_PROD].[cefore].IFU        I  ON I.IdIFU=E.IdIFU
							LEFT JOIN [SIGU_Commun_PROD].[cefore].CNSS      CN  ON CN.IdCNSS=E.IdCNSS
							LEFT JOIN [SIGU_Commun_PROD].[cefore].RegimeImposition REG ON REG.IdRegimeImposition=E.IdRegimeImposition
							LEFT JOIN [SIGU_Commun_PROD].[cefore].FormeJuridique     F ON F.IdFormeJuridique=E.IdFormeJuridique
							LEFT JOIN [SIGU_Commun_PROD].[commun].Usager     U  ON U.IdUsager=EU.IdUsager
							
							LEFT JOIN [SIGU_Commun_PROD].[commun].Adresse   AD  ON AD.IdAdresse=E.IdAdresse
							LEFT JOIN [SIGU_Commun_PROD].[cefore].Article  AC  ON AC.IdArticle=E.IdArticlePrincipale \n";
			$where       = array();
			if( isset($filters["numero"]) && !empty($filters["numero"]) ) {
				$where[] = "(R.NumeroRCCM=':numero')";
			}
			if( isset($filters["numifu"]) && !empty($filters["numifu"]) ) {
				$where[] = "(I.NumeroIFU=':numifu')";
			}
			if( isset($filters["numcnss"]) && !empty($filters["numcnss"]) ) {
				$where[] = "(CN.NumeroCNSS=':numcnss')";
			}
			if( isset($filters["libelle"]) && !empty($filters["libelle"]) ) {
				$where[] = "(E.NomCommercial LIKE '%:libelle%' OR E.DenominationSociale LIKE '%:libelle%' OR E.Sigle LIKE '%:libelle%')";
			}	
			if( isset($filters["searchQ"]) && !empty($filters["searchQ"]) ) {
				$where[] = "(E.NomCommercial LIKE '%:searchQ%' OR E.DenominationSociale LIKE '%:searchQ%' OR U.NomRaisonSociale LIKE '%:searchQ%' OR U.Prenom LIKE '%:searchQ%' OR E.Sigle LIKE '%:searchQ%' OR R.NumeroRCCM LIKE '%:searchQ%')";
			}
            if( isset($filters["name"]) && !empty($filters["name"]) ) {
				$where[] = "(U.NomRaisonSociale LIKE '%:name%' OR U.Prenom LIKE '%:name%')";
			}
            if( isset($filters["nom"]) && !empty($filters["nom"]) ) {
				$where[] = "(U.NomRaisonSociale LIKE '%:nom%')";
			}
            if( isset($filters["prenom"]) && !empty($filters["prenom"]) ) {
				$where[] = "(U.Prenom LIKE '%:prenom%')";
			}
            if( isset($filters["telephone"]) && !empty($filters["telephone"]) ) {
				$where[] = "(A.Mobile1 LIKE '%:telephone%')";
			}			
            if( isset($filters["description"]) && !empty($filters["description"]) ) {
				$where[] = "(E.ObjetSocial LIKE '%:description%' OR AC.NomArticle LIKE '%:description%')";
			}		
            if( count( $where ) ) {
            ...
            $selectSQL    .= " ORDER BY E.NomCommercial ASC,E.DenominationSociale ASC, E.Sigle ASC, U.NomRaisonSociale ASC \n "	;	
	...
	public function getNumParts($numRccm)
	{
		...
		if( strlen($numRccm)>16) {
			$pattern = "/BF\-(?<localite>[a-zA-Z])\-(<codeJuridiction>[0-2]{2})\-(?<annee>[0-9]{4})\-(?<typeRCCM>(?:A|B|C|D|E|G|K|M|R|S|T)[0-9]{1,2})\-?(?<numero>[0-9]{5})$/i";		    
		} else {
			$pattern = "/BF\-?(?<localite>[a-zA-Z]{3})\-?(?<annee>[0-9]{4})\-?(?<typeRCCM>(?:A|B|C|D|E|G|K|M|R|S|T)[0-9]{1})\-?(?<numero>[0-9]{4,5})$/i";
		}		
	...
	public function checkNum($numRccm)
	{
		...
		if( strlen($numRccm)>16) {
			$pattern = "/BF\-[a-zA-Z]{2,3}\-[0-2]{2}\-[0-9]{4}\-(?:A|B|C|D|E|G|K|M|R|S|T)[0-9]{2}\-[0-9]{5}$/i";
		} else {
			$pattern = "/BF\-?[a-zA-Z]{2,3}\-?[0-9]{4}\-?(?:A|B|C|D|E|G|K|M|R|S|T)[0-9]{1}\-?[0-9]{4,5}$/i";
		}
	...
	{		
		...
		$dbAdapter         = $table->getAdapter();
		$tablePrefix       = $table->info("namePrefix");		
		$selectDirigeants  = $dbAdapter->select()->from(array("RE"=>$tablePrefix."rccm_registre_representants"), array("RE.nom","RE.prenom","RE.adresse","RE.lieunaissance","RE.datenaissance","RE.marital_status","RE.email","RE.telephone","RE.passport","RE.representantid","RE.sexe","RE.country",
		                                                                                                               "date_naissance_year" => "YEAR(RE.datenaissance)","date_naissance_month"=>"MONTH(RE.datenaissance)","date_naissance_day" => "DAYOFMONTH(RE.datenaissance)"))
	...
	public function getNbreBySexe( $localiteid , $sexe )
	{
		...
		$selectStatistiques = $dbAdapter->select()->from(array("R" => $tablePrefix."rccm_registre" ), array("nombre" => "COUNT(DISTINCT(R.registreid))", "R.type", "R.localiteid"))
		                                          ...
		                                          ->join(array("RP"=> $tablePrefix."rccm_registre_dirigeants"), "RP.registreid = R.registreid",array("RP.representantid"))
												  ->join(array("RE"=> $tablePrefix."rccm_registre_representants"),"RE.representantid=RP.representantid",array("RP.sexe"))
	...
	public function getList( $filters = array() , $pageNum = 0 , $pageSize = 0, $orders = array("R.annee DESC","R.date DESC","R.numero DESC","R.libelle ASC") )
	{
		...
		$selectRegistre = $dbAdapter->select()->from(    array("R" => $tablePrefix."rccm_registre" ), array("R.registreid","R.localiteid","R.domaineid","R.numero","R.numifu","R.numcnss","R.libelle","R.description","R.date","R.category","R.type","date_registre"=>"FROM_UNIXTIME(R.date,'%d/%m/%Y')","annee"=>"R.annee"))
		                                      ->join(    array("RP"=> $tablePrefix."rccm_registre_dirigeants")   ,"RP.registreid=R.registreid"         , array("RP.representantid"))
		                                      ->join(    array("RE"=> $tablePrefix."rccm_registre_representants"),"RE.representantid=RP.representantid", array("RE.nom","RE.prenom","RE.adresse","RE.lieunaissance","RE.datenaissance","RE.passport","RE.sexe","RE.country","RE.marital_status","RE.telephone","RE.email","RE.profession"))											  
											  ->join(    array("L" => $tablePrefix."rccm_localites")             ,"L.localiteid=R.localiteid"          , array("localite"=>"L.libelle"))
		                                      ->joinLeft(array("D" => $tablePrefix."rccm_domaines")              ,"D.domaineid=R.domaineid"            , array("domaine" =>"D.libelle"));
			...
			if( $this->checkNum($filters["keywords"])) {
				...
				if( count(   $keywordsArr)) {
					foreach( $keywordsArr as $keywordItem ) {
							 ...
							 $searchAgainst    = "";
							 if( isset($searchSpaceArray[0]) && !empty($searchSpaceArray[0])) {
								 foreach( $searchSpaceArray as $searchKey=> $searchWord ) {
										  $searchWord = trim($searchWord);
										  if( empty($searchWord) || (strlen($searchWord) < 3)) {
											  continue;
										  }
	...
	public function getListPaginator( $filters = array() )
	{
		...
		//print_r($selectRegistre->__toString()); die();
		$selectRegistre->group(array("R.registreid","RP.registreid","RP.representantid")); 
		$paginationAdapter = new Zend_Paginator_Adapter_DbSelect( $selectRegistre );
		$rowCount          = intval(count($dbAdapter->fetchAll(   $selectRegistre )));
		$paginationAdapter->setRowCount($rowCount);
		$paginator         = new Zend_Paginator($paginationAdapter);
