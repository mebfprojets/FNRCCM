...
class Model_Demandeur extends Sirah_Model_Default
{
	...
	public function getRow($data = array())
	{
	    ...
	    if( isset($data["numidentite"])) {
			$selectDemandeur->where(  "D.numidentite=?",$filters["numidentite"])
			                ->orWhere("I.numero=?", $filters["numidentite"]);
		}
		if( isset($data["numero"])) {
			$selectDemandeur->where("I.numero=?",$filters["numero"]);
		}
		if( isset($data["date_etablissement"])) {
			$selectDemandeur->where("I.date_etablissement=?",$filters["date_etablissement"]);
		}
		if( isset($data["lieu_etablissement"])) {
			$selectDemandeur->where("I.lieu_etablissement=?",$filters["lieu_etablissement"]);
		}
		if( isset($data["organisme_etablissement"])) {
			$selectDemandeur->where("I.organisme_etablissement=?",$filters["organisme_etablissement"]);
		}
	...
	public function getList($filters = array() , $pageNum = 0 , $pageSize = 0, $orders=array("D.lastname ASC","D.firstname ASC"))
	{
		...
		$tablePrefix      = $modelTable->info("namePrefix");		
		$tableName        = $modelTable->info("name");
		$selectDemandeurs = $dbAdapter->select()->from(    array("D"=> $tableName))
	...
	{		
		...
		$selectDemandeurs  = $dbAdapter->select()->from(array("D"=> $modelTable->info("name")))
