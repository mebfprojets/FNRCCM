...
class Model_Commandefacture extends Sirah_Model_Default
{
	...
	public function getList( $filters = array(),$pageNum=0,$pageSize=0, $orders=array("F.creationdate DESC","F.date DESC","C.date DESC"))
	{
		$table            = $this->getTable();
		$dbAdapter        = $table->getAdapter();
		$tablePrefix      = $table->info("namePrefix");	
		...
		$selectInvoices   = $dbAdapter->select()->from(    array("F" =>$tableName))
												...
												->join(    array("MP"=>$tablePrefix."erccm_vente_modepaiements"),"MP.modepaiementid=P.modepaiementid",array("modePaiement"=>"MP.libelle","MP.banque"))
												...
												->join(    array("M" =>$tablePrefix."rccm_members"),"M.memberid=F.memberid OR M.accountid=F.creatorid",array("M.passport","M.birthday","M.birthaddress","M.nationalite","M.civilite","M.name"))
												->join(    array("U" =>$tablePrefix."system_users_account"),  "U.userid=M.accountid OR U.userid=F.creatorid"        , array("U.userid","U.lastname","U.firstname","U.email","U.username","U.avatar","U.phone1","U.phone2","U.sexe"))
												->join(    array("DM"=>$tablePrefix."reservation_demandeurs"),"DM.accountid=M.accountid OR DM.accountid=F.creatorid"    , array("DM.demandeurid","DM.identityid"))
		...
		if( isset($filters["documentid"]) && intval($filters["documentid"])) {
			$selectInvoices->where("F.documentid=?" , intval($filters["documentid"]));
		}
		if( isset($filters["documentcatid"]) && intval($filters["documentcatid"])) {
			$selectInvoices->where("F.documentcatid=?" , intval($filters["documentcatid"]));
		}
		if( isset($filters["has_document"]) && intval($filters["has_document"])==1) {
			$selectInvoices->where("F.documentid>=1");
		}
		if( isset($filters["has_document"]) && intval($filters["has_document"])==0) {
			$selectInvoices->where("F.documentid=0");
		}
		...
		if( isset($filters["periode_start"]) && intval($filters["periode_start"])) {
			$selectInvoices->where("C.date>=?", intval($filters["periode_start"]));
		}
	    if( isset($filters["periode_end"] )  && intval($filters["periode_end"]  )) {
			$selectInvoices->where("C.date<=?", intval($filters["periode_end"]  ));
		}
