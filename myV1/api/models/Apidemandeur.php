...
class Model_Apidemandeur extends Sirah_Model_Default
{
	...
	public function getRow($data = array())
	{
		...
		$selectDemandeur  = $dbAdapter->select()->from(array("D"=> $tableName))
	    ...
	    if( isset($data["numidentite"])) {
			$selectDemandeur->where(  "D.numidentite=?",$data["numidentite"])
			                ->orWhere("I.numero=?", $data["numidentite"]);
		}
		if( isset($data["numero"])) {
			$selectDemandeur->where("I.numero=?",$data["numero"]);
		}
		if( isset($data["date_etablissement"])) {
			$selectDemandeur->where("I.date_etablissement=?",$data["date_etablissement"]);
		}
		if( isset($data["lieu_etablissement"])) {
			$selectDemandeur->where("I.lieu_etablissement=?",$data["lieu_etablissement"]);
		}
		if( isset($data["organisme_etablissement"])) {
			$selectDemandeur->where("I.organisme_etablissement=?",$data["organisme_etablissement"]);
		}
		if( isset($data["name"]) && !empty($data["name"]) && (null!==$data["name"])){
			...
			if( empty( $nom ) ) {
				$selectDemandeur->where(new Zend_Db_Expr("D.lastname LIKE \"%".strip_tags($nom)."%\""));
			}
			if( empty( $prenom ) ) {
				$selectDemandeur->where(new Zend_Db_Expr("D.firstname LIKE \"%".strip_tags($prenom)."%\""));
			}
		}
		if( isset($data["lastname"]) && !empty($data["lastname"]) && (null!==$data["lastname"])){
			$selectDemandeur->where("D.lastname LIKE ?","%".strip_tags($data["lastname"])."%")->orWhere("D.firstname LIKE ?","%".strip_tags($data["lastname"])."%");
		}
		if(isset($data["firstname"]) && !empty($data["firstname"]) && (null!==$data["firstname"])){
			$selectDemandeur->where("D.firstname LIKE ?","%".$data["firstname"]."%")->orWhere("D.lastname LIKE ?","%".$data["firstname"]."%");
		}
	...
	public function createCode()
	{
		...
		$selectDemandeur        = $dbAdapter->select()->from(array("D" => $table->info("name")),array("total"=>"COUNT(D.demandeurid)"));
		$nbreTotal              = $dbAdapter->fetchOne($selectDemandeur)+1;
		$newCodeDemandeur       = sprintf("Dem-%06d", $nbreTotal );
		while($existDemandeur   = $this->findRow($newCodeDemandeur, "code", null, false)) {
			  $nbreTotal++;
			  $newCodeDemandeur = sprintf("Dem-%06d", $nbreTotal );
		}
		
		return $newCodeDemandeur;
	}
	...
	public function identite($identityid=0) 
	{
		...
		$tablePrefix      = $modelTable->info("namePrefix");		
		$tableName        = $modelTable->info("name");
		$selectIdentite   = $dbAdapter->select()->from(    array("I"=> $tablePrefix."reservation_demandeurs_identite")      ,array("numeroPiece"=>"I.numero","I.numero","I.organisme_etablissement","I.lieu_etablissement","I.date_etablissement","I.typeid"))
	...
	public function getList($filters=array(), $pageNum=0, $pageSize=0, $orders=array("D.creationdate DESC","D.demandeurid DESC","D.lastname ASC","D.firstname ASC"))
	{
		$modelTable       = $this->getTable();
		$dbAdapter        = $modelTable->getAdapter();
		$tablePrefix      = $modelTable->info("namePrefix");		
		$tableName        = $modelTable->info("name");
		$selectDemandeurs = $dbAdapter->select()->from(    array("D"=> $tablePrefix."reservation_demandeurs"))
		...
		if( isset($filters["searchQ"]) && !empty($filters["searchQ"])){
			...
			$selectDemandeurs->where("{$likeName}");
		}
		if( isset($filters["name"]) && !empty($filters["name"])){
			...
			if( empty( $nom ) ) {
				$selectDemandeurs->where(new Zend_Db_Expr("D.lastname LIKE \"%".strip_tags($nom)."%\""));
			}
			if( empty( $prenom ) ) {
				$selectDemandeurs->where(new Zend_Db_Expr("D.firstname LIKE \"%".strip_tags($prenom)."%\""));
			}
		}
		if( isset($filters["lastname"]) && !empty($filters["lastname"]) ){
			$selectDemandeurs->where("D.lastname LIKE ?","%".strip_tags($filters["lastname"])."%")->orWhere("D.firstname LIKE ?","%".strip_tags($filters["lastname"])."%");
		}
		if( isset($filters["firstname"]) && !empty($filters["firstname"])){
			$selectDemandeurs->where("D.firstname LIKE ?","%".$filters["firstname"]."%")->orWhere("D.lastname LIKE ?","%".$filters["firstname"]."%");
		}
		if( isset($filters["identityid"]) && intval($filters["identityid"])){
			$selectDemandeurs->where("I.identityid=?", intval($filters["identityid"]));
		}
		if( isset($filters["numidentite"]) && !empty($filters["numidentite"])){
			$selectDemandeurs->where("D.numidentite=?",$filters["numidentite"]);
		}
		if( isset($filters["numero"]) && !empty($filters["numero"])){
			$selectDemandeurs->where("I.numero=?",$filters["numero"]);
		}
		if( isset($filters["numidentitetype"]) && !empty($filters["numidentitetype"])){
			$selectDemandeurs->where("T.libelle=?",$filters["numidentitetype"]);
		}
		if( isset($filters["identitetypeid"]) &&  intval($filters["identitetypeid"])){
			$selectDemandeurs->where("I.typeid=?",intval($filters["identitetypeid"]));
		}
		if(isset($filters["email"])     && !empty($filters["email"])){
			$selectDemandeurs->where("D.email=?",$filters["email"]);
		}
		if(isset($filters["country"])   && !empty($filters["country"])){
			$selectDemandeurs->where("D.country=?",$filters["country"]);
		}
		if(isset($filters["sexe"])      && !empty($filters["sexe"])){
			$selectDemandeurs->where("D.sexe=?",$filters["sexe"]);
		}
	    if(isset($filters["telephone"]) && !empty($filters["telephone"])){
			$selectDemandeurs->where($dbAdapter->quote("D.telephone=".$filters["telephone"]));
		}
		if( isset($filters["domicile"]) && !empty($filters["domicile"])){
			$selectDemandeurs->where($dbAdapter->quote("D.tel_dom=".$filters["domicile"]));
		}	
		if( isset($filters["periode_start"]) && intval($filters["periode_start"])) {
			$selectDemandeurs->where("D.creationdate>= ?", intval($filters["periode_start"]));
		}
	    if( isset( $filters["periode_end"])  && intval($filters["periode_end"])) {
			$selectDemandeurs->where("D.creationdate<= ?", intval($filters["periode_end"]));
		}
		if( intval($pageNum) && intval($pageSize)) {
			$selectDemandeurs->limitPage($pageNum , $pageSize);
		}
		if( is_array($orders) && count($orders)) {
			$selectDemandeurs->order($orders);
		} elseif(is_string($orders)) {
			$selectDemandeurs->order(array(sprintf("%s DESC", strip_tags($orders) )));
		}
		//print_r($selectDemandeurs->__toString()); die();
		return $dbAdapter->fetchAll($selectDemandeurs , array() , Zend_Db::FETCH_ASSOC);
	}
	...
	{		
		...
		$paginationAdapter = new Zend_Paginator_Adapter_DbSelect($selectDemandeurs);
		$rowCount          = intval(count($dbAdapter->fetchAll($selectDemandeurs)));
