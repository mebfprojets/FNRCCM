...
class Model_Document extends Sirah_Model_Default
{
	...
	public function getList($filters = array() , $pageNum = 0 , $pageSize = 0, $orders=array("D.category DESC","D.documentid DESC", "D.creationdate DESC"))
	{
		$table           = $this->getTable();
		$dbAdapter       = $table->getAdapter();
		$tablePrefix     = $table->info("namePrefix");				
		$selectDocument  = $dbAdapter->select()->from(    array("D" => $table->info("name")))
		                                       ->join(    array("C" => $tablePrefix."system_users_documents_categories"),"C.id=D.category", array("categorie" => "C.libelle"))
		...
		if( isset( $filters["has_registre"] ) && intval( $filters["has_registre"] ) ) {
			$selectDocument->join(array("R" => $tablePrefix."rccm_registre_documents"),"R.documentid=D.documentid",null);
		}
	    ...
	    if( isset( $filters["categoryLib"] ) && intval( $filters["categoryLib"] ) ) {
			$selectDocument->where("C.libelle LIKE ?" , "%" . strip_tags( $filters["categoryLib"] ). "%" );
		}
