...
class Admin_DemandesController extends Sirah_Controller_Default
{
	 ...
	 public function getAction()
	{
		...
		if((empty($contentData["signataire"]) || !intval($profileid)) && intval($creatorid)) {
			$creatorProfile               = $modelProfile->getRow($creatorid,true); 
			$contentData["profile"]       = $profile       = 
		}
		$documentTpl                      = "fiche";
		...
		if( $this->_request->isPost() ) { 
			...
			$demandeFilename           = $demandesPathRoot.DS. sprintf("%s_Numero%s.pdf", $documentName , preg_replace("/[^A-Za-z0-9]/","_",$demandeNumero));
				...
				if(!is_dir(  $demandesPathRoot ) ) {
				 ...
				 if(!file_exists( $demandeFilename )) {
					...
					if( $dbAdapter->insert( $prefixName."system_users_documents", $documentData)) {
					    ...
					    $dbAdapter->update( $prefixName."reservation_demandes_requests" , array("notified"=>0,"processed"=>1), array("demandeid=?"=>$demandeid));
					    $dbAdapter->delete( $prefixName."reservation_demandes_notifications", array("demandeid=?"=>$demandeid));
					}
				 }
				 if( empty( $errorMessages )) {
