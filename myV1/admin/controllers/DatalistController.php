...
class DatalistController extends Sirah_Controller_Default
{
	...
	public function soutenanceslistAction()
	{
		...
		if( $me->isEtudiant() || $me->isStudent() ) {
		    ...
		    $filters        = array("cohortid"     => $defaultCohortid,
				                    "salleid"      => $salleid ,
				                    "lastname"     => $lastname    ,
				                    "firstname"    => $firstname   ,
				                    "annee"        => $calendarYear,
	...
	public function honoraireslistAction()
	{
		...
		$filters            = array("libelle"            => $defaultLibelle ,
				                    "teachername"        => null ,
				                    "courseid"           => 0,
				                    "cohortid"           => 0,
				                    "statut"             => -1,
	...
	public function inscriptionslistAction()
	{
		...
		$inscriptions          = $model->getList($filters , $pageNum , $pageSize);
		$paginator             = $model->getListPaginator($filters);
		if(null !== $paginator) {
			$paginator->setCurrentPageNumber($pageNum);
			$paginator->setItemCountPerPage($pageSize);
