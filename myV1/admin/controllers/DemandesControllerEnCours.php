...
class Admin_DemandesController extends Sirah_Controller_Default
{
	...
	public function requestAction()
	{
		...
		$demandeRow->signature_author    = ($profile)? sprintf("%s %s",$profile->lastname, $profile->firstname) : "";
	...
	public function getAction()
	{
		...
		$contentData["addSignature"]      = $addSignature  = (isset($demande->signature_profileid))? $demande->signature_profileid : 0;
		...
		$contentData["profile"]           = $profile   = (intval($profileid))?$modelProfile->findRow(intval($profileid),"profileid",null,false) : null;
		$contentData["creatorid"]         = $creatorid = ($profile )? $profile->userid : $demande->creatorid;
		...
		switch( strtolower($documentType)) {
			...
			default:
			   ...
			   $documentName              = "DemandeVerification";
			   $documentCategory          = 10;
			   $documentTitle             = sprintf("FICHE DE RECHERCHE DE DISPONIBILITE ET DE RESERVATION Num %s", $demande->numero);
			   break;
			case "disponibilite":
		...
		if( $this->_request->isPost() ) { 
			...
			$demandeFilename           = $demandesPathRoot.DS. sprintf("%s_Numero_%s.pdf", $documentName , preg_replace("/[^A-Za-z0-9]/","_",$demandeNumero));
			
			$codeBarreParams           = $fichePDF->serializeTCPDFtagParameters(array($demandeNumero, 'QRCODE', '', '',14,14, '', 'N'));
