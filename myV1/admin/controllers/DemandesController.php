...
class Admin_DemandesController extends Sirah_Controller_Default
{
	...
	public function tasksAction()
	{
		...
		if( isset($appConfigSession->usertasks->nonProcessedRequests) ) {
			$demandeTasks                = $appConfigSession->usertasks->nonProcessedRequests;
			$appConfigSession->usertasks->hasRead              = true;
			...
			$appConfigSession->usertasks->nonProcessedRequests = $demandeTasks = $modelRequest->nonProcessedRequests(0,$me->userid);
		}
		 
		$this->view->demandes            = $modelRequest->nonProcessedRequests(0,$me->userid);
		$this->view->title               = "Des demandes en instance de traitement";
		$this->render("demandetasks");
	}
	...
	{		
		...
		if( $this->_request->isPost() ) {
			...
			$update_data["objet"]                = (isset($postData["objet"]       ))? $stringFilter->filter($postData["objet"])	    : $demande->objet;
			$update_data["denomination"]         = (isset($postData["denomination"]))? $stringFilter->filter($postData["denomination"])	: $demande->denomination;
			...
			if( empty($errorMessages)) {
				...
				if( isset($clean_update_data["demandeurid"])) {
					unset($clean_update_data["demandeurid"]);
				}
				if( isset($clean_update_data["promoteurid"])) {
					unset($clean_update_data["promoteurid"]);
				}
				if( isset($clean_update_data["entrepriseid"])) {
					unset($clean_update_data["entrepriseid"]);
				}
				if( $dbAdapter->update($tableName, $clean_update_data, array("demandeid=?"=>$demandeid)) ) { 					
					...
					if( $dbAdapter->insert( $prefixName."reservation_demandes_verifications", $verification_data )) {
						...
						$this->redirect("admin/demandes/verify/id/".$demandeid."/demandeid/".$demandeid);
					}
				} else {
					if( $this->_request->isXmlHttpRequest() ) {						
						$this->_helper->viewRenderer->setNoRender(true);
			...
			if( count($errorMessages)) {
				$defaultData  = array_merge( $defaultData, $demandeData, $entrepriseData , $postData );
				if( $this->_request->isXmlHttpRequest()) {
					$this->_helper->viewRenderer->setNoRender(true);
	...
	public function requestAction()
	{
		...
		$userid             = $me->userid;
		$operator           = $request->operator($userid,$requestid);
		$profile            = $modelProfile->getRow($userid);
		...
		$demandeRow->signature_profileid = ($profile)? $profile->profileid  : 0;
		$demandeRow->signature_author    = ($profile)? sprintf("%s %s", $profile->firstname,$profile->lastname) : "";
		$demandeRow->creatorid           = $userid;
		$demandeRow->save();
	...
	{		
		...
		if(!isset( $stateStore->filters["_demandes"]["maxitems"])) {
			$stateStore->filters["_demandes"] = array("page"=>1,"maxitems"=>NB_ELEMENTS_PAGE,"libelle"=>null,"numero"=>null,"localiteid"=>0,"searchQ"=>null,
													  "typeid"=>0,"statutid"=>0,"expired"=>0,"disponible"=>4,"processed"=>4,"validated"=>4,"webrequests"=>4,"date"=>null,"lastname"=>null,"firstname"=>null,"name"=>null,
													  "demandeurname"=>null,"promoteurname"=>null,"demandeurid"=>0,"promoteurid"=>0,"operatorid"=>0,"nomcommercial"=>null,"creatorid"=>0,
													  "periode_start_day"=>1,"periode_start_month"=>1,"periode_start_year"=>2000,"periode_end_day" =>31,"periode_end_month"=>12,"periode_end_year"=>date("Y")
                                                );			
		}
		...
		if( isset($filters["webrequests"]) && intval($filters["webrequests"])==1) {
		    $pageSize            = $maxitems = 600;
			$pageNum  = 1;
		}			
		//print_r($params); die();
	...
	public function verifyAction()
	{
		...
		switch(strtolower($verificationSource)) {
			case "erccm":
			default:
			    $registres                  = $modelRegistre->getList(array("keywords"=>$query), 1,10);
				if( count(   $registres ) ) {
					$i                      = 0;
			...
			case "sigu":
				...
				if( count($keywords)) {
					foreach( $keywords as $keyword ) {
						     ...
						     try {
								...
								$siguSearchClient->setParameterGet("searchq"   , $keyword);
			    ...
			    break;
			case "reservation":
			    $entreprises                = $modelEntreprise->getList(array("libelle"=>$query,"reserved"=>1,"expired"=>0), 1, $totalResults);
				if( count(   $entreprises ) ) {
					...
					foreach( $entreprises as $entreprise ) {
							 ...
							 if(($entreprise["nomcommercial"] == $query || $entreprise["sigle"] == $query) && ($entreprise["expired"]==0)) {
								 $rejected  = 1;
							 }
							 $i++;
					}
	...
	public function createAction()
	{
		...
		if( $this->_request->isPost() ) {
			...
			$entrepriseData["datecreation"]     =  $entrepriseData["datefermeture"] = "0000-000-00 00:00:00";
	...
	public function reserveAction()
	{
		...
		$dbAdapter->update( $prefixName."reservation_demandes_requests"   , array("processed"=>1,"validated"=>1,"notified"=>0,"updatedate"=>time(),"updateduserid"=>$me->userid), array("demandeid=?"=>$demandeid) );
	...
	public function deleteAction()
	{
		...
		if( count(   $ids)) {
			foreach( $ids as $id) {
					 ...
					 if( $dbAdapter->delete($tablePrefix."reservation_demandes"                   , array("demandeid=?"     =>$demandeid))) {
                         $dbAdapter->delete($tablePrefix."reservation_demandes_verifications"     , array("verificationid=?"=>$demandeid));
						 $dbAdapter->delete($tablePrefix."reservation_demandes_reservations"      , array("reservationid=?" =>$demandeid));
						 $dbAdapter->delete($tablePrefix."reservation_demandes_requests"          , array("demandeid=?"     =>$demandeid));
						 $dbAdapter->delete($tablePrefix."reservation_demandes_requests_operators", array("demandeid=?"     =>$demandeid));
						 $dbAdapter->delete($tablePrefix."api_reservation_demandes"               , array("sync_demandeid=?"=>$demandeid));
					 } else {
						 $demandeRow          = $model->findRow($demandeid,"demandeid", null, false);
                         if( $demandeRow ) {
	 ...
	 public function getAction()
	{
        ...
        $this->view->identiteTypes        = $identiteTypes  = $modelIdentiteType->getSelectListe("Selectionnez un type de pièce d'identité", array("typeid", "libelle") , array() , null , null , false );		
		$myAccountId                      = $me->userid;
		$demandeurid                      = $demande->demandeurid;
		$entrepriseid                     = $demande->entrepriseid;
		$promoteurid                      = $demande->promoteurid;	
        $localiteid                       = $demande->localiteid;	
        $identityid                       = $demandeurRow->identityid;
        $domaineid                        =	($entrepriseRow)? $entrepriseRow->domaineid : 0;	
		$demandeurRow                     = ($demandeurid  )? $modelDemandeur->findRow( $demandeurid ,"demandeurid" ,null, false) : null;
		$entrepriseRow                    = ($entrepriseid )? $modelEntreprise->findRow($entrepriseid,"entrepriseid",null, false) : null;
		$promoteurRow                     = ($promoteurid  )? $modelPromoteur->findRow( $promoteurid ,"promoteurid" ,null, false) : null;
		...
		$contentData["addSignature"]      = $addSignature  = (isset($demande->signature_profileid))? $demande->signature_profileid : (($profile)?$profile->profileid : 0);
		$contentData["signataire"]        = (isset($demande->signature_author   ))? $demande->signature_author    : 0;
		$contentData["profileid"]         = $profileid     = (isset( $demande->signature_profileid))?$demande->signature_profileid : 0;
		$contentData["profile"]           = $profile       = (intval($profileid))?$modelProfile->findRow(intval($profileid),"profileid",null,false) : $modelProfile->findRow(intval($myAccountId),"userid",null,false);
		$contentData["creatorid"]         = $creatorid     = (isset($demande->creatorid))?$demande->creatorid : (($profile )? $profile->userid : $me->userid);
		
		$profileAccount                   = (intval($creatorid))?Sirah_Fabric::getUser(intval($creatorid))    : null;
		if( $profileAccount==null) {
			$profileAccount               = Sirah_Fabric::getUser(23);
		}
		if( intval($demande->updateduserid) && $demande->web) {
			$contentData["creatorid"]     = $creatorid     = $demande->updateduserid;
			$profileAccount               = Sirah_Fabric::getUser($creatorid);
			$contentData["profile"]       = $profile       = $creatorProfile = $modelProfile->getRow($creatorid,true); 
			$signature_profileid          = $profileid     = ($profile)? $profile->profileid  : 9;
			$signature_author             = $contentData["signataire"] = ($profile)? sprintf("%s %s", $profile->firstname,$profile->lastname) : "";
            $dbAdapter->update( $prefixName."reservation_demandes", array("signature_profileid"=>$signature_profileid,"signature_author"=>$signature_author),array("demandeid=?"=>$demandeid));
			$contentData["addSignature"]  = $addSignature  =  1;
		}
		if(($profileAccount->isGuest() || !intval($profileAccount->userid)) && $demande->web) {
			$accounts                     = Sirah_User_Table::getUsers(array("localiteid"=>$localiteid,"roles"=>array("ADMINISTRATEUR_DU_FNRCCM","CONSEILLER_CEFORE","GREFFIER_AU_FNRCCM")));
			if(!count($accounts) || ($localiteid == 9)) {
				$accounts                 = Sirah_User_Table::getUsers(array("roles"=>array("ADMINISTRATEUR_DU_FNRCCM","GREFFIER_AU_FNRCCM")));
			}
			//var_dump($accounts); die();
			$contentData["creatorid"]     = $creatorid     = (isset($accounts[0]["userid"]))?$accounts[0]["userid"] : $me->userid;
		    $profileAccount               = (intval($creatorid))?Sirah_Fabric::getUser(intval($creatorid))  : $me;
		}
		if((empty($contentData["signataire"])) && intval($creatorid) && $demande->web) {
			$contentData["profile"]       = $profile       = $creatorProfile = $modelProfile->getRow($creatorid,true); 
			$signature_profileid          = $profileid     = ($profile)? $profile->profileid  : 9;
			$signature_author             = $contentData["signataire"] = ($profile)? sprintf("%s %s", $profile->firstname,$profile->lastname) : "";
            $dbAdapter->update( $prefixName."reservation_demandes", array("signature_profileid"=>$signature_profileid,"signature_author"=>$signature_author),array("demandeid=?"=>$demandeid));
		...
		if( $this->_request->isPost() ) { 
			...
			$demandeFilename           = $demandesPathRoot.DS. sprintf("%sNumero%s%s.pdf", $documentName , preg_replace("/[^A-Za-z0-9]/","_",$demandeNumero), time());
			if( file_exists($demandeFilename) ) {
				@unlink($demandeFilename);
			}
			//var_dump($profile->signature); die();
			$demandePDF                = $fichePDF = Sirah_Fabric::getPdf();
			$codeBarreParams           = $demandePDF->serializeTCPDFtagParameters(array($demandeNumero, 'QRCODE', '', '',14,14, '', 'N'));
			$contenu                   = preg_replace("/\[CODEBARRE\]/", "<tcpdf method=\"write2DBarcode\" params=\"".$codeBarreParams."\" />", $contenu);
			if( $addSignature && $profileid && $profile && $profileAccount) {			
				$signatureFilename     = (!empty($profile->signature))? basename($profile->signature)                     : "";
				$profileSignaturePath  = (!empty($signatureFilename ))? $profileAccount->getDatapath().$signatureFilename : "";
				if(!empty($signatureFilename) && file_exists($profileSignaturePath)) {
					$fileUsername      = Sirah_Filesystem::safeFilename(Sirah_Filesystem::cleanPath($profileAccount->username));
					$signatureImgSrc   = VIEW_BASE_URI."/myV1/documents/privatedata/sirahbf2546155aoo/users/{$fileUsername}/{$signatureFilename}";
				    $contenu           = preg_replace("/\[SIGNATURE\]/", "<img src=\"".$signatureImgSrc."\" />",$contenu);
				} else {
					$contenu           = preg_replace("/\[SIGNATURE\]/", " ",$contenu);
				}
			} else {
				$contenu               = preg_replace("/\[SIGNATURE\]/", " ",$contenu);
			}
			...
			if( empty( $errorMessages )) {
				if(!is_dir(  $demandesPathRoot ) ) {
			     ...
			     $demandePDF->SetAutoPageBreak(TRUE, 10);
			     $demandePDF->SetMargins(12,0,12);
				 $demandePDF->SetHeaderMargin(0);
				 ...
				 if(!file_exists( $demandeFilename )) {
					...
					$dbAdapter->delete(     $prefixName."system_users_documents", "filename LIKE \"%".$filename."%\"");
					if( $dbAdapter->insert( $prefixName."system_users_documents", $documentData)) {
						$documentid                = $dbAdapter->lastInsertId();
						$sync_demandeid            = $demandeid;
						$statutid                  = $demande->statutid;
						if( $demande->sync_demandeid ) {
							$sync_demandeid        = $demande->sync_demandeid;
						} else {
							$modelApiDemande       = new Model_Apidemande();
							if( $apiDemande = $modelApiDemande->findRow($demandeid,"sync_demandeid",null,false)){
								$sync_demandeid    = $apiDemande->demandeid;
							}
						}
						...
						$dbAdapter->delete( $prefixName."reservation_demandes_documents"    , array("demandeid=?" =>$demandeid  ));
					    ...
					    $dbAdapter->update( $prefixName."reservation_demandes_requests"     , array("notified"=>0,"processed"=>1), array("demandeid=?"=>$sync_demandeid));
					    $dbAdapter->update( $prefixName."api_reservation_demandes"          , array("statutid"=>$statutid,"synchronized"=>1,"disponible"=>$demande->disponible,"rejected"=>$demande->rejected), array("demandeid=?"=>$sync_demandeid));
						$dbAdapter->delete( $prefixName."reservation_demandes_notifications", array("demandeid=?"=>$sync_demandeid));
				 ...
				 if( empty( $errorMessages )) {
					 if( $documentOutput == "download" ) {
						 ...
						 $filePath   = "http://".APPLICATION_HOST. BASE_PATH. "myV1/documents/privatedata/sirahbf2546155aoo/demandes/". sprintf("%s_Numero%s.pdf", $documentName , preg_replace("/\s+/","_",$demande->numero));
        ...
        $this->view->contenu           = $contenu;
        $this->view->params            = $params;	
		$this->view->documentType      = $documentType;
	...
	public function rejectAction()
	{
		...
		if( $this->_request->isPost()) {
			...
			if( $statutId=="5" && empty($motifs)){
				$motifs[]                 = "DS, NC ou Sigle présente de fortes similitudes à une DS ou NC déjà immatriculé (e) au RCCM";
			}
			if( count( $motifs )) {
			...
			if( empty( $errorMessages )) {
				...
				if( $demande->save() ) {
					$dbAdapter->update( $prefixName."reservation_demandes_requests", array("processed"=>1,"validated"=>0,"notified"=>0,"updatedate"=>time(),"updateduserid"=>$me->userid), array("demandeid=?"=>$demandeid) );
					if( $statutId == 3 ) {
