...
class Admin_AccountsController extends Sirah_Controller_Default
{
	...
	public function listAction()
	{
	  ...
	  //On crée un validateur de filtre
	    ...
	    $filters              = array("lastname"=>null,"firstname"=>null,"username"=>null,"email"=> null,"name"=>$defaultName,"roleid"=>0,"localiteid"=>0,
	   		                          "phone"=>null,"activated"=>1,"locked"=>0,"blocked"=>0,"expired"=> 0,"rolename"=>null,"sexe"=>null,"admin"=>null,"country"=>null,"language"=>null);
		...
		$this->view->localites = $modelLocalite->getSelectListe("Selectionnez une juridiction", array("localiteid", "libelle"), array() , 0 , null , false);
	...
	public function editAction()
	{
		...
		//Une astuce pour contourner une attaque par force brute, en utilisaant le jeton du formulaire
		    ...
		    if(!empty($errorMessages)){
				...
				$accountUser             = $editedUser;
				$accountid               = $editedUser->userid;
				$profile                 = $modelProfile->getRow($accountid,true);
				...
				$documentsUploadAdapter->addValidator("Extension", false , array("png","jpg", "gif","jpeg","bmp"));
				$documentsUploadAdapter->addValidator("Size"     , false , array("max"=> "25MB"));
				$documentsUploadAdapter->addValidator("FilesSize", false , array("max"=> "25MB"));
				if( $documentsUploadAdapter->isUploaded("signature")) {
					$userDataPath                       = $editedUser->getDatapath();
					$documentData                       = $modelDocument->getEmptyData();
					$documentData["userid"]             = $accountid;
					$documentData["category"]           = 12;
					$documentData["resource"]           = "" ;
					$documentData["resourceid"]         = 1;
					$documentData["filedescription"]    = $filedescription = sprintf("Signature de %s %s" , $editedUser->lastname, $editedUser->firstname );
					$documentData["filemetadata"]       = $filemetadata    = sprintf("%s,%s" , $editedUser->lastname , $editedUser->firstname );
					$basicFilename                      = $documentsUploadAdapter->getFileName("signature", false);
					$documentExtension                  = Sirah_Filesystem::getFilextension( $basicFilename );
					$tmpFilename                        = Sirah_Filesystem::getName( $basicFilename);
					$userFilePath                       = $userDataPath .  sprintf("SignatureNumerique%04d_Profile%04d.%s", $accountid,$profileid, $documentExtension);
					$documentsUploadAdapter->addFilter("Rename" , array("target" => $userFilePath , "overwrite"=>true) , "signature");
					$documentsUploadAdapter->receive("signature");
					if( $documentsUploadAdapter->isReceived("signature")) {
						$fileSize                       = $documentsUploadAdapter->getFileSize('signature');
						$documentData["filename"]       = $filename = $modelDocument->rename($userFilePath, $accountid);
						...
						if( $dbAdapter->insert( $prefixName."system_users_documents", $documentData ) ) {
							...
							if( $profileid ) {
								$documentType           = "Signature Numérique";
								$profileDocument        = array("documentid"=>$documentid,"profileid"=>$profileid,"libelle"=>$filename,"description"=>$filedescription,"type"=>$documentType,"keys"=>$filemetadata,"creatorid"=>$me->userid,"creationdate"=>time());
                               
								$dbAdapter->insert( $prefixName."system_users_profile_documents", $profileDocument );
								$dbAdapter->update( $prefixName."system_users_profile"          , array("signature"=>$userFilePath), array("profileid=?"=>$profileid));
							}
						} else {
								$errorMessages[]        = sprintf("Le document %s n'a pas pu être enregistré", $basicFilename);
						}
					}
				}
				if( $sendmail){
