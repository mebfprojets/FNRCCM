...
$pdfTotextBinary = "E:\\webserver/www/erccm/binaries/poppler-25.07.0/Library/bin";
...
{	
	...
	{		
		...
		$annee                        = intval($this->_getParam("annee", 2022));
		...
		$selectEntreprises            = $dbAdapter->select()->from(array("E"=> $tablePrefix."rccm_registre"))
												  ->where("E.registreid NOT IN (SELECT RC.registreid FROM rccm_registre_dirigeants RC)")
												  ->order(array("E.annee ASC"));
		$missingRegistres             = $dbAdapter->fetchAll($selectEntreprises,array(),Zend_Db::FETCH_ASSOC);										  
		//var_dump($missingRegistres); die();
		$registreDefaultData          = $defaultData     = $modelRegistre->getEmptyData();
		$inserted                     = 0;
		if( count(   $missingRegistres) ) {
			foreach( $missingRegistres as $missingRegistre ) {
				     $registreid      = $missingRegistre["registreid"];
					 $entrepriseid    = $missingRegistre["entrepriseid"];
					 $numero          = (isset($missingRegistre["numero"]   ))? $missingRegistre["numero"] : "";
					 $telephone       = (isset($missingRegistre["telephone"]))? $missingRegistre["telephone"] : "";
					 if(!intval($registreid) || empty($telephone) || empty($numero)) {
						 continue;
					 }
					 $phones          = explode("/",$telephone);
					 if( count($phones) ) {
						 $registreDirigeants = $modelRegistre->dirigeantsByPhones($phones); 
						 if( count($registreDirigeants) ) {
							 foreach( $registreDirigeants as $registreDirigeant) {
								      ...
								      $dirigeantData   = array("registreid"=>$registreid,"representantid"=>$representantid,"fonction"=>"GERANT","qualiteCode"=>"","idFonction"=>72,"entrepriseid"=>0);
							          try {
										  if( $dbAdapter->insert( $tablePrefix."rccm_registre_dirigeants", $dirigeantData)) {
											  $successMessages[] = sprintf("Le dirigeant %s %s a été correctement rattaché à l'entreprise %s numéro RCCM %s", $registreDirigeant["lastname"], $registreDirigeant["firstname"], $missingRegistre["libelle"], $missingRegistre["numero"]);
										  }
									  } catch(Exception $e ) {
										  $errorMessages[] = sprintf("Erreur technique sur le dirigeant %s %s : %s",$registreDirigeant["lastname"], $registreDirigeant["firstname"],$e->getMessage());
									  }
							 }
						 }
					 }
	...
	public function importqualitesAction()
	{
		$this->view->title         = "Importer les qualités des dirigeants";
		$model                     = $modelQualite    = $this->getModel("representantqualite");
		$modelTable                = $model->getTable();
		...
		$errorMessages             = $successMessages = array();
		$jsonCsvRows               = array();
		$emptyQualiteData          = $model->getEmptyData();
		if( $this->_request->isPost() ) {
		    ...
		    $documentsUploadAdapter->addFilter("Rename", array("target" => $destinationName, "overwrite"=> true) , "qualites");
			if( $documentsUploadAdapter->isUploaded("qualites") ) {
				$documentsUploadAdapter->receive(   "qualites");
				if( $documentsUploadAdapter->isReceived("qualites") ) {
					...
					if( count(   $csvData )) {
						...
						foreach( $csvData as $csvRow ) {
								 ...
								 if( empty($Libelle) ) {
									 $errorMessages[]= sprintf("La désignation de la fonction est invalide à la ligne %d", $ligneid);
									 $ligneid++;
								     $i++;
									 continue;
								 }
								 $postData                        = array("code"=>$CodeFNRCCM,"qualiteCode"=>$CodeFNERE,"libelle"=>$Libelle,"description"=>$Libelle,"parentid"=>0,"profondeur"=>0,"special"=>0,"creationdate"=>time(),"creatorid"=>$me->userid,"updatedate"=>0,"updateduserid"=>0);
								 $insertData                      = array_merge($emptyQualiteData, array_intersect_key($postData,$emptyQualiteData));
								 $updateData                      = array();
								 $idFonction                      = 0;
								 if( $foundByFNRCCM       = $model->findRow($CodeFNRCCM,"code",null,false)) {
								     $updateData                  = array_merge($foundByFNRCCM->toArray(), $insertData);
									 $idFonction                  = $updateData["idFonction"] = $foundByFNRCCM->idFonction;
								 } elseif($foundByFNERE   = $model->findRow($CodeFNERE,"qualiteCode",null,false)) {
									 $updateData                  = array_merge($foundByFNERE->toArray(), $insertData);
									 $idFonction                  = $updateData["idFonction"] = $foundByFNERE->idFonction;
								 } elseif($foundByLibelle = $model->findRow($Libelle,"libelle",null,false)) {
									 $updateData                  = array_merge($foundByLibelle->toArray(), $insertData);
									 $idFonction                  = $updateData["idFonction"] = $foundByLibelle->idFonction;
								 }
								 if( count($updateData) && isset($updateData["idFonction"])) {
									 $idFonction                  = $updateData["idFonction"];
									 unset($updateData["idFonction"]);
									 $updateData["updatedate"]    = time();
									 $updateData["updateduserid"] = $me->userid;
									 try {
										 if( $dbAdapter->update( $prefixName."rccm_registre_dirigeants_fonctions", $updateData , array("idFonction=?"=>$idFonction))) {
										     $successMessages[]   = sprintf("La qualité %s a été mis à jour avec succès", $Libelle);
										 } else {
											 $errorMessages[]     = sprintf("La qualité %s n'a pas été mise à jour pour des raisons inconnues", $Libelle);
										 }
									 } catch(Exception $e) {
										 $errorMessages[]         = sprintf("La qualité %s n'a pas été mise à jour pour la raison : %s", $Libelle, $e->getMessage());
									 }
								 } else {
									 try {
										 if( $dbAdapter->insert( $prefixName."rccm_registre_dirigeants_fonctions", $insertData)) {
										     $successMessages[]   = sprintf("La qualité %s a été créée avec succès", $Libelle);
										 } else {
											 $errorMessages[]     = sprintf("La qualité %s n'a pas été créée pour des raisons inconnues", $Libelle);
										 }
									 } catch(Exception $e) {
										 $errorMessages[]         = sprintf("La qualité %s n'a pas été créée pour la raison : %s", $Libelle, $e->getMessage());
									 }
								 }
								 $ligneid++;
								 $i++;
		...
		$this->render("importqualites");	
	}

    public function importformsAction()
	{
		$this->view->title         = "Importer les formes juridiques";
		$model                     = $modelForme = $this->getModel("entrepriseforme");
		$modelTable                = $model->getTable();
		$prefixName                = $modelTable->info("namePrefix");
		$dbDestination             = $dbAdapter       = $modelTable->getAdapter();
		$me                        = Sirah_Fabric::getUser();
		...
		if( $this->_request->isPost() ) {
		    ...
		    $destinationName       = $me->getDatapath(). "csvFile.csv";						
		    $documentsUploadAdapter->addFilter("Rename", array("target" => $destinationName, "overwrite"=> true) , "formes");
			if( $documentsUploadAdapter->isUploaded("formes") ) {
				$documentsUploadAdapter->receive(   "formes");
				if( $documentsUploadAdapter->isReceived("formes") ) {
					$csvFile        = file($destinationName ,FILE_SKIP_EMPTY_LINES);
					$csvRows        = array_map("str_getcsv",$csvFile,array_fill(0, count($csvFile), ';'));
					$csvKeys        = array_shift($csvRows);
					$csvData        = array();
					...
					if( count(   $csvData )) {
						...
						foreach( $csvData as $csvRow ) {
								 ...
								 if( empty($Libelle) ) {
									 $errorMessages[]= sprintf("La désignation de la forme juridique est invalide à la ligne %d", $ligneid);
									 $ligneid++;
								     $i++;
									 continue;
								 }
								 ...
								 if( $foundByFNRCCM       = $model->findRow($CodeFNRCCM,"code",null,false)) {
								     $updateData                  = array_merge($foundByFNRCCM->toArray(), $insertData);
									 $formid                      = $updateData["formid"] = $foundByFNRCCM->formid;
								 } elseif($foundByFNERE   = $model->findRow($CodeFNERE,"code_fnere",null,false)) {
									 $updateData                  = array_merge($foundByFNERE->toArray(), $insertData);
									 $formid                      = $updateData["formid"] = $foundByFNERE->formid;
								 } elseif($foundByLibelle = $model->findRow($Libelle,"libelle",null,false)) {
									 $updateData                  = array_merge($foundByLibelle->toArray(), $insertData);
									 $formid                      = $updateData["formid"] = $foundByLibelle->formid;
								 }
								 if( count($updateData) && isset($updateData["formid"])) {
									 $formid                      = $updateData["formid"];
									 unset($updateData["formid"]);
									 $updateData["updatedate"]    = time();
									 $updateData["updateduserid"] = $me->userid;
									 try {
										 if( $dbAdapter->update( $prefixName."rccm_registre_entreprises_forme_juridique", $updateData , array("formid=?"=>$formid))) {
										     $successMessages[]   = sprintf("La forme %s a été mis à jour avec succès", $Libelle);
										 } else {
											 $errorMessages[]     = sprintf("La forme %s n'a pas été mise à jour pour des raisons inconnues", $Libelle);
										 }
									 } catch(Exception $e) {
										 $errorMessages[]         = sprintf("La forme %s n'a pas été mise à jour pour la raison : %s", $Libelle, $e->getMessage());
									 }
								 } else {
									 try {
										 if( $dbAdapter->insert( $prefixName."rccm_registre_entreprises_forme_juridique", $insertData)) {
										     $successMessages[]   = sprintf("La forme %s a été créée avec succès", $Libelle);
										 } else {
											 $errorMessages[]     = sprintf("La forme %s n'a pas été créée pour des raisons inconnues", $Libelle);
										 }
									 } catch(Exception $e) {
										 $errorMessages[]         = sprintf("La forme %s n'a pas été créée pour la raison : %s", $Libelle, $e->getMessage());
									 }
								 }
								 $ligneid++;
								 $i++;
    ...
    public function regexAction()
	{
		...
		$this->_helper->viewRenderer->setNoRender(true);
		$text           = <<<EOF
		  BURKINA FASO COUR D'APPEL DE FADA N'GOURMA ************** La Patrie ou la Mort, nous Vaincrons TRIBUNAL DE GRANDE INSTANCE m DE FADA N'GOURMA ********* BULLETIN N°3 EXTRAIT DU CASIER JUDICIAIRE CASIER JUDICIAIRE N°02134 /2025 au repertoire RELEVE des condamnations à des peines privatives de iiberté concernant Le/La nommé(e) OUOBA Lamoussa Fils/Fille de OUOBADiagnoigo Et de , J HARO Bandiba^ Le 31/12/1997 - Né(e) A ' Bassiéri Domicile' Fada N'Gourma Etat civil de famille Célibataire Profession CuLtivateyr Nationalité - .Burkinabé o in Nature des Dates des Nature et crimes et crimes et durée des délits délits peines Dates des Cours où Tribunaux condamnations Observations flSiîîUB & E.A.NN°251du 11/09/2023 de Gayéri O O -H. y !> I Timbre du tribunal Visa Fada N'Gourma,Le 20/05/2025 iV Le Chef de greffe Parquet ureurduFaso % r r* CD ^y' -Q reui Le foc^ fasc du V J '-iA h Eslfê Expire le: 20/08/2025 ^Ou âiJHKlNA rAsu Ait iNAllUNALb U IDENTITE BURKINABE 08030301000011838 Nom; OUOBA Prànoms: LAMOUSSA Hùtnu; Dépa'tf.'r.er» komanL/JCAR' OAiTHi Né(e)te: 31/12/1997 A BASSIERI P«fc' [} ... illettré Pour photocopie certifiée conforme à roriginal qui nous a élé présenté Madjoari le / L Officier El 7m URKiNA / qImunepe * V/?. EtatCW'd AT-CI Qt/lA^Û ^ajzL / IIBiCnONS REGIONALES B.P 12 B.P 103 B.P 47 OUAGADOUGOU 01 BP. 215 BOBO-OIOULASSO OUAHIGOUYA FADA N'GOURMA DEDOUGOU Tél 25.30.60.78-61 Tél 20.97.11.62/63 Tél 40.55.02.16 Tél 40.77.01.13 Tél 20.52.00.31 40.55.04.91 40.77.00.66 20.52.01.85 01 B.P 562 IIIEenONDI lECeunEMENT ETDUCONTENTIEIl Tel. : 50.31,20.48/50/54 HOTIFICBTIOH PBOMOTCDB Le Directeur Général de la Caisse Nationale de Sécurité Sociale accuse réception de votre demande d'Immatriculation ainsi référencée : Nom ou Raison Sociale : ouoba lamoussa Dénomination : TIENUFU TIPO Sigle : Secteur d'activités : commerce Adresse Postale : GAYERI TEL:77464206 Forme juridique : ENTREPRISE INDIVIDUELLE Date d'effet : En attente Et vous informe de votre immatriculation en qualité de promoteur sous le numéro : 14799430. N.B. Toute correspondance avec la CNSS devra mentionner le numéro:14799430^ qui ne confère nullement la qualité d'employeur tant qu'ilsera en attente. Fait à Fada N'Gourma Le 08/08/2025 P/Le Directeur Général P.D. sy le ;o\el Conuii'^icuy . CARTE PROFESSIONNELLE DE COMMERÇANT Natur* d* l'activfté :COMMERCE Nom : OUOBA N* RCCM : BF FDG 01 2025 A10 00183 Prénoms : LAMOU5SA Du : 04A)8/202S ADRESSE Date de naissance : 31/12/1997 Lieu de naissance : BASSIERl Previnc* KOMONOJARI :GAYÉRI Tél : 77 46 4206/6341 54 86 B.P; SISNATURE OUTnUUVftt SIGWnjRE DE L-AUTOMT6 COMPfelWie LeSP^UCt Pays d'ongme Burkina faso BURKINA FASO REGION DE L'EST PROVINCE DE LA KOMONDJARI Patrie ou la Morts Nous vaincrons rvi COMMUNE DE GAYERI MAIRIE CABINET N°2025-013/REST/PKMD/CGYR/M/CAB 1» Je soussigné, Monsieur LANKOANDE Emmanuel, premier Vice-président de la Délégation Spéciale de la Commune de Gayéri, Province de la Komondjari, certifie que Monsieur OUOBA Lamoussa, né le 31/12/1997 à Bassiéri, Province de la Komondjari,commune de Gayéri, Fils de:OUOBA Diaggnoigou Et de: HARO Bandiba Réside à Gayéri depuis sa naissance En foi de quoi, le présent certificat est établi pour servir et valoir ce que de droit. de mai 2025 ^0 y. so DB ■30 é o -e Président s 4^ IVIL rCoS LANKOANDE Emmanuel DECLARATION SUR L'HONNEUR Pour la déclaration d'activités d'un entreprenant ou l'immatriculation d'un commerçant au RCCM EN APPLICATION DES ARTICLES 45,47 al 4 et 63 al 3 DE L'ACTE UNIFORME SUR LE DROIT COMMERCIAL GENERAL(AUDCG) Je soussigné. Nom: Prénom(s): .Wl.O.wS..i. Né(e) le : ?\\slW(ê de : 0LIO.6.1^ ^ Et de : fs.O ^ Domicile : Promoteur/gérant de ; /X.tAf.Z,/...F'.L/..r..TX..f'Q. En création, ayant déposé une o Demande d'Immatrlçylation o Déclaration d'activité Au Registre de Commerce et du Crédit Mobilier du TribufiaPde Gommer-ce/Tribunal de Grande instance de f.AÙA DECLARE SUR L'HONNEUR Que je ne suis frappé d'aucune des interdictions et/ou condamnations visées par l'article 10 de TAUDCG. Par ailleurs je suis Informé des poursuites pénales en cas de fausse déclaration (article 281 du code pénal), et de l'annulation de mon inscription en cas de non production de mon bulletin de casier judiciaire dans un délai de 75 jours. F,i,à If&â. nÉs-U-oiS Signature N" du dossier (À compléter par le numéro du RCCM) W.L iiT . « • FORMULAIRE DE DEMANDE DE LA CARTE PROFESSIONNELLE DE COMMERÇANT Je soussigné... O.U.C.3./^ Lttm.o..u.SrS!:cL.... Commerçant domicilié au secteur de Référence du document d'identité : . Sollicite l'établissement d'une Carte Professionnelle de Commençant(CPC). Mise en demeure Signature obligatoire du Cachet et visa de l'autorité ■-* ' demandeur -www.cci, Iwww.me.bf — LJMttii i^fBFOOAAA Prix imprimé : Cinq cent (500) francs CFA ■( k bsL / ENR/Accueil/4 Rev2 FS 567012 Fiche de renseignements à remplir obligatoirement par le promoteur avant le dépôt de V son dossier Nom commercial de l'entreprise Zz Siqle (facultatif): ^ Activités menées : « _/ *- \y4V\.|>A.....dJt- / Principale ; ^ Secondaire (facultatif) ; Ç^ j ^ Nouveau : F fîl^.'ltv%^4^-rr0\) , 4i Adresse : Secteur • Ancien : Boite postale Téléphone.. TTkT Ut^?^ ' U-i. Sk Sc, ^ Chiffre d'affaires annuel prévisionnel : 6: Nombre d'employés prévus 7- Renseignement sur la possession d'une entreprise existante : V Avez-vous déjà une entreprise avec les pièces suivantes: NON NON NON NON CPC CNSS IFU RCCM OUI OUI OUI OUI Si oui. sous quelle forme ést-^fé constituée ? Personne moral Personne physique Cochez la branche d'activités de l'entreprise déjà existante Transport CZ] ^FP [ | Commerce Autres à préciser [ *SRCM «gnature *SRCM : services rendus aux collectivités et aux ménages % BURKINA FASO CAiité - Progrès - Ajstice MINISTERE DE L'ECONOMIE ET DES FINANCES SECRETARIAT GENERAL DIRECTION GENERALE DES IMPOTS DIRECTION REGIONALE DES IMPOTS DE L'EST DIRECTION PROVINCIALE DES IMPOTS DU KOMONDJARI s CERTIFICAT D'IMMATRICULATION IDENTIFIANT FINANCIER UNIQUE PHOTO (IFU) L'Usager: LAMOUSSA QUOBA Enseigne Corn.: TIENUFU TIPO Forme Juridique : PERSONNE PlJ^SIQUE Régime Fiscal: CONTRIBUTION DES MlCÎ^-ENTREPRISES RCCM : BF-FDG-01-2025-A10-00183 CNSS : Activité Principale : COMMERCE DE.DÉTAIL Ef^i MAGASIN SPÉCIALISÉ DE PRODUITS ALIMENTAIRES, BOISSONS ET TABACS MANUFACTURÉS -V> ■> > Adresse : Courriel: Secteur: ■ *:yx- \ Tél.: 77-46-42-06 Fax: Parcelle : CD Lot: Section : SECTEUR 4 a Rue : Immatri.: 07/08/2025 A été immatriculé sous le numéro Identifiant Financier Unique (IFU) 00279658E ^B: L'arrêté N" 2005-766/MFB/SG/DGI du 15 Décembre 2005 rend obligatoire la mention du numéro IFU sur toui ilocument produit dans le cadre de l'exercice de la profession sous peine des sanctions prévues par les textes er /igueur. Les modifications ultérieures devront être portées à la connaissance du Responsable de la Direction des mpôts compétent. Fait à GAYERI, le 08/08/2025 LE DIRECTEUR PROVINCIAL DES IMPOTS lil m AIME NAMA MINISTERE DE L'ADMINISTRATION TERRITORIALE BURKINA FASO DE DECENTRATION ET DE LA SECURITE Unité-Progrès-Justice REGION DE L'EST PROVINCE DE LA KOMONDJARI Gayéri, le 20 Mai 2025 COMMUNE DE GAYERI BUREAU DOMANIAL N°2025-0i3_/MATDS/REST/PKMD/C-GYR PERMIS D'OCCUPATION PROVISOIRE En application de l'article 177 de la loi n°34-2012/AN du 22 juillet 2012, portant réorganisation agraire et foncière au Burkina Faso ; le Président de la Délégation Spéciale de la commune de Gayéri permet à Monsieur OUGBA Lamoussa, N°CNIB:19435210 du 10/01/2024, d occuper à des fins lucratives le domaine public communal d'une superficie approximative de 04 m== au marché central de Gayéri, pour l'implantation d'une boutique. Le présent permis n'autorise que la réalisation des ouvrages en matériaux non définitifs. Il ne peut être cédé à un tiers sans l'autorisation écrite de l'autorité communale. Toute contestation qui pourrait survenir avec des voisins par la suite de l'existence de ces ouvrages, de même que tous les dommages que ces ouvrages pourraient causer sont aux risques et périls du détenteur de ce permis. Le Présent permis n équivaut pas à un bail ou à une vente et ne confère aucun droit de propriété sur le terrain sous-jacent faisant partie du domaine de la commune de Gayéri. II pourra être révoqué si le terrain est requis à des fins d'utile public ou municipal. Amnliation .• c,® de ^0. Le Président rfi - Perception •r. Cr -Intéressé lO d: -Chrono Emm K 2010-AE FORMULAIRE D'ACCUSE D'ENREGISTREMENT 1. Je soussigné Maître Adama TRAORE, Greffier en Chef, dudit Tribunal de Grande Instance de Fada N'Gourma, atteste avoir reçu la demande de : M Immatriculation lUDéclaration d'activités OModification CDissolution CRadiation DAutres 2. Concernant (dénomination) ; OUOBA Lamoussa (TIENUFU TIPO) Tel:77 46 42 06 / 63 41 54 86. 3. Numéro d'immatriculation : BF-FDG-01-2025-A10-00183 du 06/08/2025 Numéro de la formalité au registre d'arrivée lJÏÂj2025 Fait à Fada N'Gourma le 06 août 2025 (^tande r t.e Chef de Greffe 6® •Q h- aît^ Ad a TRAORE ^::jG^ffieir en Chef □MO iv^lier de l'Ordre du Mérite
EOF;
		$regex          = "#(?<=Mlle)(.*)(?=Prénoms)(.*)#";//Nom et prénom
	    $regex2         = '/(?<=LIEU de NAISSANCE:)(.*)(?: A)(.*)(?:NATIONALITE:)(.*)/';
		$regex1         = "/Enseigne Co(?:.{1,3})\.:\s*(.{1,250})\s*Forme Juridique :/";//Nom commercial
	    $regex3         = "#Dénomination(?:.*):(.*)(?=Sigle)#";//Nom commercial
	    $regex4         = "/(?<= postale|postal|postat|poslal\):)(.*)(?=Tel|Tél|Té)/";//Adresse
	    $regex5         = "#EXERCEE\s*\(S\))(?:.*):*(.*)#";//Activité Principale 1
		...
		$regex10        = '/(?<=NAISSANCE:)(.*)(?:à)(.*)(?:NATIONALITE.*\s*n*\s*:)(.*)/';
	    $regex11        = "#Tel(?:.*):(.*)(?=B.P)#";
		$regex12        = "#Tel(?:.*):(.*)(?=\.\.\.)#";
		$regex13        = "#DATE DE DEBUT(?:.*):(.*)\s*(?=N)#";
		$regex14        = "#Activité(?:.*):(.*)\s*(?=Ad)#";
	...
	public function backuprcAction()
	{		
	    @ini_set('memory_limit', '-1');
		$this->_helper->layout->disableLayout(true);
		$this->_helper->viewRenderer->setNoRender(true);
		...
		$selectEntreprises            = $dbAdapter->select()->from(array("E"=> $tablePrefix."rccm_registre_entreprises"))
												  ->where("E.registreid NOT IN (SELECT RC.registreid FROM rccm_registre RC)");
		$missingRegistres             = $dbAdapter->fetchAll($selectEntreprises,array(),Zend_Db::FETCH_ASSOC);										  
		//var_dump($missingRegistres); die();
		$registreDefaultData          = $defaultData     = $modelRegistre->getEmptyData();
		$inserted                     = 0;
		if( count(   $missingRegistres) ) {
			foreach( $missingRegistres as $missingRegistre ) {
				     $registreid      = $missingRegistre["registreid"];
					 $entrepriseid    = $missingRegistre["entrepriseid"];
					 $numero          = (isset($missingRegistre["num_rc"]))? $missingRegistre["num_rc"] : "";
					 if(!intval($registreid) || !intval($entrepriseid) || empty($numero)) {
						 continue;
					 }
					 if( $foundRCCM   = $modelRegistre->findRow($numero,"numero",null,false)) {
						 continue;
					 }
					 if(!$foundRCCM   = $modelRegistre->findRow($registreid,"registreid",null,false)) {
						 $insertRCCM                 = array_merge($registreDefaultData, array_intersect_key($missingRegistre,$registreDefaultData));
 
			             $insertRCCM["statut"]       = 1;
						 $insertRCCM["checked"]      = 1;
						 $insertRCCM["numero"]       = $numero = (isset($missingRegistre["num_rc"]   ))? $missingRegistre["num_rc"]              : "";
						 $insertRCCM["numifu"]       = (isset($missingRegistre["num_ifu"]            ))? $missingRegistre["num_ifu"]             : "";
						 $insertRCCM["numcnss"]      = (isset($missingRegistre["num_securite_social"]))? $missingRegistre["num_securite_social"] : "";
						 $insertRCCM["libelle"]      = (isset($missingRegistre["libelle"]            ))? $missingRegistre["libelle"]             : "";
						 $insertRCCM["denomination"] = $insertRCCM["libelle"];
						 $insertRCCM["sigle"]        = "";
						 $insertRCCM["description"]  = (isset($missingRegistre["presentation"]       ))? $missingRegistre["presentation"]        : "";
						 $insertRCCM["date"]         = (isset($missingRegistre["datecreation"]       ))? $missingRegistre["datecreation"]        : "";
					     $insertRCCM["capital_numeraire"] = $insertRCCM["capital_nature"] = $insertRCCM["capital"] = (isset($missingRegistre["capital"]))? $missingRegistre["capital"] : 0;
						 $insertRCCM["nbactions"]    = $insertRCCM["parentid"] = $insertRCCM["ifuid"] = $insertRCCM["cpcid"] = $insertRCCM["cnssid"] = $insertRCCM["addressid"] = $insertRCCM["communeid"] = $insertRCCM["statusid"] = 0;
						 $numeroParts                = $modelRegistre->getNumParts($numero);
						 $numTypeRegistre            = (isset($numeroParts["typeRCCM"]))? $numeroParts["typeRCCM"] : strtoupper(trim(substr($NumeroRCCM, 9, 1)));
			             $insertRCCM["annee"]        = $registreYear = (isset($numeroParts["annee"]))?intval($numeroParts["annee"]) : substr( $numero, 5, 4);
			             $insertRCCM["localiteid"]   = (isset($numeroParts["localite"]) && isset($localitesCodes[$numeroParts["localite"]]))? $localitesCodes[$numeroParts["localite"]] : 9;
					     $rccmTypeId                 = 2;
						 if( false!==stripos($numTypeRegistre,"A")) {
							 $rccmTypeId             = 1;
						 } elseif(false!==stripos($numTypeRegistre,"B")) {
							 $rccmTypeId             = 2;
						 }
						 $insertRCCM["category"]     = ($rccmTypeId==1)?"P0" : "M0";
						 $insertRCCM["type"]         = $rccmTypeId;
						 $insertRCCM["adresse"]      = (isset($missingRegistre["address"]))? $missingRegistre["address"]  : "";
						 $insertRCCM["telephone"]    = (isset($missingRegistre["phone1"] ))? $missingRegistre["phone1"]   : "";
						 $insertRCCM["params"]       = $insertRCCM["sigle"] = "";
						 try {
							if( $dbAdapter->insert( $prefixName."rccm_registre", $insertRCCM)) {
								$inserted++;
							} else {
								$errorMessages[]     = sprintf("Le RCCM %s n'a pas pû être créé", $numero);
							}
						 } catch(Exception $e) {
							$errorMessages[]         = sprintf("Une erreur technique a été détectée dans le RCCM %s : %s", $numero, $e->getMessage());
						 }
		...
		if( count($errorMessages) ) {
			...
			$successMessage  = sprintf("%d de RCCM ont pu être enregistrés avec succès.",$inserted);
		    if( $this->_request->isXmlHttpRequest()) {
				$this->_helper->viewRenderer->setNoRender(true);
				$jsonResponse                 = array();
	...
	{		
		...
		$dbSourceParams               = array("host"=>"localhost","username"=>"useradmin","password"=>"mebfSir@h1217","dbname"=>"erccmdb_online","isDefaultAdapter"=>0);
		...
		if( empty($errorMessages) && $dbSourceAdapter) {
			$filters                  = array("periode_start_day"=>1,"periode_start_month"=>01,"periode_start_year"=>2000,"periode_end_day"=>5,"periode_end_month"=>12,"periode_end_year"=>2025);
			...
			$selectRegistres          = $dbSourceAdapter->select()->from(    array("R" => $tablePrefix."rccm_registre" ))
		                                                          ->joinLeft(    array("RP"=> $tablePrefix."rccm_registre_dirigeants")   ,"RP.registreid=R.registreid"         , array("RP.representantid"))
																  ->joinLeft(    array("RE"=> $tablePrefix."rccm_registre_representants"),"RE.representantid=RP.representantid", array("RE.nom","RE.prenom","RE.adresse","RE.lieunaissance","RE.datenaissance","RE.cnib","RE.passport","RE.sexe","RE.telephone","RE.profession"))											  
            ...
            //$selectRegistres->limitPage(1,20000);
			...
			if( count(   $sourceRegistres) ) {
				foreach( $sourceRegistres as $sourceRegistre) {
						 ...
						 $numero          =  $sourceRegistre["numero"];
						 $entrepriseid    =  (isset($sourceRegistre["entrepriseid"]))? $sourceRegistre["entrepriseid"] : 0;
						 if(!$registreRow =  $modelRegistre->findRow($numero,"numero",null,false)) {
							 ...
							 try {
								...
								if( $foundRegistre = $modelRegistre->findRow($registreid,"registreid",null,false)) {
									$insertData["registreid"] = $registreid = 0;
								}
								if( $dbAdapter->insert( $prefixName."rccm_registre", $insertData)) {
									if(!intval($insertData["registreid"])) {
										$insertData["registreid"] = $registreid = $sourceRegistre["registreid"] = $dbAdapter->lastInsertId();
									}
									...
									if( isset($postData["estate"]) || isset($postData["titre"])) {
										...
										$surete_data["updateduserid"]           = (isset($postData["updateduserid"]      ))? intval($postData["updateduserid"]) : 0;
										$surete_data["updatedate"]              = (isset($postData["updatedate"]         ))? intval($postData["updatedate"])    : 0;	
										$surete_data["creatorid"]               = $insertData["creatorid"];
										$dbAdapter->delete(     $prefixName."rccm_registre_suretes", array("registreid=?"=>$registreid));
										if(!$dbAdapter->insert( $prefixName."rccm_registre_suretes", $surete_data)) {
											$errorMessages[$registreid]              = sprintf("La sûrété du RCCM N° %s ID %d n'a pas pu être enregistrée", $insertData["numero"],$registreid);
										    continue;
										}
									}
	...
	public function apiodscsvAction()
	{
		@ini_set('memory_limit', '-1');
		$this->_helper->viewRenderer->setNoRender(true);
		$this->_helper->layout->disableLayout(true);
		...
		$selectRegistre           = $dbAdapter->select()->from(array("R" => $tablePrefix."rccm_registre"), array("R.registreid","R.localiteid" ,"R.domaineid","R.numero","R.numifu","R.numcnss","R.libelle","R.description","R.date","R.category","R.type","date_registre"=>"FROM_UNIXTIME(R.date,'%d/%m/%Y')","annee"=>"FROM_UNIXTIME(R.date,'%Y')"))
		                                                ->where("R.numifu IS NULL OR R.numifu=''")
														->where("R.type=1 OR R.type=2")
														->order(array("R.annee DESC","R.date DESC"));
		
		$registres                = $dbAdapter->fetchAll($selectRegistre, array() , Zend_Db::FETCH_ASSOC);
		$totalRegistres           = count($registres);
		$totalFound               = 0;
		$foundRegistreNbre        = 0;
		$foundRegistresList       = $parameterData = $apiodsRows = $apiResponse = $csvRows = array();
		
		//print_r($totalRegistres); die();
		
		if( count(   $registres ) && empty($errorMessages)) {
			foreach( $registres as $registre ) {
				     $registreid     = $registre["registreid"];
				     $NumeroRCCM     = preg_replace("/[^a-z0-9]/i","",$registre["numero"] );
					 $NumeroIFU      = preg_replace("/[^a-z0-9]/i","",$registre["numifu"] );
					 $NumeroCNSS     = preg_replace("/[^a-z0-9]/i","",$registre["numcnss"] );
					 $NOM_COMMERCIAL = preg_replace("/[^a-z0-9\s,_\-]/i","",$registre["libelle"]);
					 $numCountry     = "BF";
					 $numLocalite    = strtoupper(trim(substr($NumeroRCCM, 2, 3)));
					 $numYear        = strtoupper(trim(substr($NumeroRCCM, 5, 4)));
					 $numType        = "A";
					 $registreTypeLibelle = "Personne Physique";
					 if( $registre["type"]==1) {
						 $numType    = "A";
						 $registreTypeLibelle = "Personne Physique"; 
					 } elseif( $registre["type"]== 2 ) {
						 $numType    = "B";
						 $registreTypeLibelle = "Personne Morale";
					 } elseif( $registre["type"]== 3) {
						 $numType    = "S";
					 } else {
						 continue;
					 }
                     $NumeroRCCMKey           = sprintf("%s%s%d%s",$numCountry, $numLocalite, $numYear, $numType);
					 $NumeroRCCMId            = str_ireplace($NumeroRCCMKey,"", $NumeroRCCM);
					 $NumeroRCCMAPI           = (intval($NumeroRCCMId)>=1000)?sprintf("%s%04d", $NumeroRCCMKey, intval($NumeroRCCMId)) : sprintf("%s%03d", $NumeroRCCMKey, intval($NumeroRCCMId));
					 
					 $parameterData["rccm"]   = $NumeroRCCMAPI;
					 try {
						 ...
						 if( isset( $apiResponseData["etat"]["code"]) && isset( $apiResponseData["reponse"]) && $apiResponseData["etat"]["code"]== "200") {
							 $apiodsRows      = $apiResponseData["reponse"];
						 }  
						 if( empty($apiodsRows) || !count($apiodsRows) ) {
							 $requestClient   = new  Zend_Http_Client( $API_URI, array('keepalive' => true));
							 $requestClient->setMethod($API_SEND_METHOD);
							 $requestClient->setParameterPost(array("num_ifu"=>$NumeroIFU));
							 $requestClient->setHeaders($headers);							
							 $apiResponse     = $requestClient->request();
							 $apiResponseData = ( $apiResponse )? json_decode($apiResponse->getBody(), true) : array();
							 if( isset( $apiResponseData["etat"]["code"]) && isset( $apiResponseData["reponse"]) && $apiResponseData["etat"]["code"]== "200") {
					 ...
					 if( isset( $apiodsRows[0]["RCCM"]) && !empty( $apiodsRows[0]["RCCM"]) && $apiodsRows[0]["RCCM"]!==null && $apiodsRows[0]["RCCM"]!=="null") {
 
						 $api_NumeroRCCM     = $apiodsRows[0]["RCCM"] ;
						 $api_NumeroIFU      = (isset($apiodsRows[0]["NUM_IFU"] ))?$apiodsRows[0]["NUM_IFU"]   : "";
						 $api_NumeroCNSS     = (isset($apiodsRows[0]["CNSS_DGI"]))?$apiodsRows[0]["CNSS_DGI"]  : "";
						 $api_Capital        = (isset($apiodsRows[0]["CAPITAL"] ))?$apiodsRows[0]["CAPITAL"]  : "";
						 $api_NumeroRCCMCode = str_ireplace($NumeroRCCMKey,"",$NumeroRCCM);  
                         $api_NumeroRCCMId   = intval(preg_replace("/[^0-9]/i","", substr($api_NumeroRCCM,-1,4)));				 													 
						 $api_CleanNumeroRCCM= $model->normalizeNum($api_NumeroRCCM,$numYear,$numLocalite);
						 
						 if( $api_NumeroRCCM==$NumeroRCCMAPI || $api_CleanNumeroRCCM==$NumeroRCCMAPI || $api_NumeroRCCMId==$NumeroRCCMId || $NumeroIFU==$api_NumeroIFU ) {
							 $totalFound = $totalFound + 1;
							 $updatedRCCMData= array("numifu"=>$api_NumeroIFU);
							 if(!empty($api_NumeroCNSS) && $api_NumeroCNSS!=="null" && $api_NumeroCNSS!==NULL ) {
								 $updatedRCCMData["numcnss"]          = $api_NumeroCNSS;
							 }
							 $updatedRCCMData["updateduserid"]        = $me->userid;
							 $updatedRCCMData["updatedate"]           = time();
							 if( $dbAdapter->update($prefixName."rccm_registre", $updatedRCCMData, array("registreid=?"=>$registreid))) {
								 //print_r($NumeroRCCM); die();
								 $entrepriseUpdatedData               = array("num_securite_social"=>$api_NumeroCNSS,"num_ifu"=>$api_NumeroIFU,"num_rc"=>$NumeroRCCM);
								 if( $api_Capital!==null && $api_Capital!=="null" && $api_Capital!==NULL) {
									 $entrepriseUpdatedData["capital"]= $api_Capital;
								 }	
                                 if( isset($apiodsRows[0]["CA_PREVISIONNEL"]) && $apiodsRows[0]["CA_PREVISIONNEL"]!==null && $apiodsRows[0]["CA_PREVISIONNEL"]!=="null" && $apiodsRows[0]["CA_PREVISIONNEL"]!==NULL) {
									 $entrepriseUpdatedData["chiffre_affaire"] = $apiodsRows[0]["CA_PREVISIONNEL"];
								 }	
                                 $dbAdapter->update( $prefixName."rccm_registre_entreprises",$entrepriseUpdatedData, array("registreid=?"=>$registreid));								 
								 $csvRowData                          = array();
								 $csvRowData["TYPE_ENTREPRISE"]       = $registreTypeLibelle;
								 $csvRowData["LOCALITE"]              = $registre["localite"];
								 $csvRowData["FNRCCM_NUMRCCM"]        = $NumeroRCCM;
								 $csvRowData["FNRCCM_NUMIFU"]         = $registre["numifu"];
								 $csvRowData["FNRCCM_NUMCNSS"]        = $registre["numcnss"];
								 $csvRowData["FNRCCM_NOMCOMMERCIAL"]  = $registre["libelle"];
								 $csvRowData["DGI_NUMRCCM"]           = $api_NumeroRCCM;
								 $csvRowData["DGI_NUMIFU"]            = $api_NumeroIFU;
								 $csvRowData["DGI_NUMCNSS"]           = $api_NumeroCNSS;
								 $csvRowData["DGI_NOMCOMMERCIAL"]     = (isset($apiodsRows[0]["ENSEIGNE_COMMERCIALE"]) && !empty($apiodsRows[0]["ENSEIGNE_COMMERCIALE"]))?$apiodsRows[0]["ENSEIGNE_COMMERCIALE"] : $apiodsRows[0]["NOM_RAISON_SOCIALE"];
								 $csvRowData["DGI_DATEIFU"]           = (isset($apiodsRows[0]["DATE_IFU"]))?$apiodsRows[0]["DATE_IFU"] : "";
								 $csvRowData["DGI_CAPITAL"]           = $api_Capital;
								 $csvRowData["DGI_ACTIVITE"]          = (isset($apiodsRows[0]["ACTIVITE"]))?$apiodsRows[0]["ACTIVITE"] : "";
								 $csvRows[$NumeroRCCM]                = $csvRowData;
							 }
						 }	 else {
							 //print_r(sprintf("%s : %s <br/>\n", $NumeroRCCMAPI, $api_NumeroRCCM));
						 }					 
					 } else {
						 //print_r(sprintf("%s : NON TROUVEE <br/>\n", $NumeroRCCMAPI ));
					 }
			}
		}
		//print_r($csvRows); die(); 
		if( count( $csvRows ) ) {
			...
			$rccmDocumentDest         = $myStoreDataPath . DS . "ARCHIVES" .DS.  "ERCCM_APIODS_CROISEMENT.csv" ;
			if( file_exists( $rccmDocumentDest )) {
				@unlink($rccmDocumentDest);
			}
			$csvHeader   = array("LOCALITE","TYPE_ENTREPRISE","FNRCCM_NUMRCCM","FNRCCM_NUMIFU","FNRCCM_NUMCNSS","FNRCCM_NOMCOMMERCIAL","DGI_NUMRCCM","DGI_NUMIFU","DGI_NOMCOMMERCIAL","CAPITAL","DGI_DATEIFU","DGI_CAPITAL","DGI_ACTIVITE");
			...
			if( $csvAdapter->save( $csvRows ) ) {
				$this->_helper->Message->addMessage( sprintf("Votre opération de création du fichier CSV s'est produite avec succès : %d/%d RCCMS ont une correspondance dans le FNRCCM VS APIODS", $totalFound, $totalRegistres), "success");
				$this->_helper->viewRenderer->setNoRender(true);
				$this->_helper->layout->disableLayout(true);
	...
	public function importifuAction()
	{
		@ini_set('memory_limit', '512M');		
		$this->view->title          = "Importer des données des impôts";
		$model                      = $this->getModel("registre");
		$modelFinance               = $this->getModel("registrefinance");
		$modelCaisse                = $this->getModel("registrecaisse");
		$modelLocalite              = $this->getModel("localite");
		...
		if( $this->_request->isPost() ) {
			...
			if( $documentsUploadAdapter->isUploaded("registres") && empty($errorMessages )) {
				$documentsUploadAdapter->receive(   "registres");
				if( $documentsUploadAdapter->isReceived("registres") ) {
					$csvHeader      = array("NumeroRCCM","NumeroIFU","NumeroCNSS","TELEPHONE","CAPITAL");
					$csvAdapter     = Sirah_Filesystem_File::fabric("Csv",array("filename"=>$csvStoreFilePath,"has_header"=> true,"header"=>$csvHeader), "rb");
					$csvLines       = $csvAdapter->getLines();
					$csvItems       = 1;
					if(isset($csvLines[0])) {
					...
					if( $totalRows ) {
						foreach( $csvRows as $csvRow ) {
							     $NumeroRCCM        = preg_replace("/[^a-z0-9]/i", "", $csvRow["NumeroRCCM"]);
								 $NumCNSS           = preg_replace("/[^a-z0-9]/i", "", $csvRow["NumeroCNSS"]);
								 $NumeroIFU         = preg_replace("/[^a-z0-9]/i", "", $csvRow["NumeroIFU"]);
								 $Telephone         = $csvRow["TELEPHONE"];
								 $Capital           = $csvRow["CAPITAL"];
								 if( empty($NumeroIFU) ) {
									 continue;
								 }									 
							     if( substr($NumeroRCCM,0, 2) != "BF" || stripos($NumeroRCCM,"BF")=== false ) {
									 $NumeroRCCM    = sprintf("BF%s", $NumeroRCCM);
								 }
								 $localiteCode      =  trim(substr($NumeroRCCM, 2, 3));
								 if(!isset($localiteIDS[$localiteCode]) ) {
									 continue;
								 }
								 if( strlen($NumeroRCCM )>=16 ) {
									 $codePays      = "BF";
									 $codeLocalite  = trim(substr($NumeroRCCM, 2, 3));
									 $codeType      = trim(substr($NumeroRCCM, 5, 2));
									 $codeAnnee     = trim(substr($NumeroRCCM, 7, 4));
									 $codeActivite  = trim(substr($NumeroRCCM,11, 3));
									 $codeIdEnt     = trim(substr($NumeroRCCM,14, 5));
									 $NumeroRCCM    = sprintf("%s-%s-%02d-%04d-%s-%05d", $codePays, $codeLocalite, $codeType, $codeAnnee, $codeActivite, $codeIdEnt);
								     /*echo $NumeroRCCM;
									 exit;*/
								 }
							     $CleanNumeroRCCM   = $model->normalizeNum($NumeroRCCM);
								 //$CleanNumeroRCCM   = $NumeroRCCM;
								 $registre          = $model->findRow($CleanNumeroRCCM,"numero",null,false);
								 if( isset($registre->registreid)  && !empty($CleanNumeroRCCM) && !empty($NumeroIFU) ) {									 
									 $registreData  = $registre->toArray();
                                     $registreid    = $registre->registreid;
                                     $updatedData   = array(); 
                                     if( empty($registre->numifu) || $registre->ifuid==0) {
										 $updatedData["numifu"]         = $NumeroIFU;
									 }
									 if( empty($registre->telephone) && !empty($Telephone)) {
										 $updatedData["telephone"]      = $Telephone;
									 }
									 if((empty($registre->capital) || $registre->capital==0) && !empty($Capital)) {
										 $updatedData["capital"]        = $Capital;
									 }
									 if( count($updatedData) ) {
										 $registreUpdateData            = array_intersect_key($updatedData,$registreData);
										 if( $dbAdapter->update( $prefixName."rccm_registre",$registreUpdateData, array("registreid=?"=>$registreid))) {
											 $importedRows[$registreid] = array_merge($registreData,$updatedData);
											 $imported++;
										 }
									 }
								 }		
	...
	public function apiodsearchAction()
	{
		...
		$this->view->domaines         = $modelDomaine->getSelectListe( "Secteur d'article" , array("domaineid" , "libelle") , array() , null , null , false );
    ...
    public function cleandbAction()
	{
		...
		$modelTable           = $model->getTable();
        $dbAdapter            = $modelTable->getAdapter();
		$periodStart          = mktime(0,0, 0,4,5,2022);
		$searchAnnee          = intval($this->_getParam("annee", $this->_getParam("year", 0)));
		$registres            = $model->getList(array("annee"=>$searchAnnee,"creatorid"=>26,"periodstart"=>$periodStart),0,0,array("R.creationdate"));
		$deletedRepresentants = $updatedRegistres = array();
		...
		if( count(   $registres) ) {
			foreach( $registres as $registre ) {
					 ...
					 $AnneeRCCM               = $registre["annee"];
					 $NomCommercial           = $registre["libelle"];
					 
					 if( $AnneeRCCM < 2022 && strlen($NumeroRCCM)<=15 ) {
						 $localite            = trim(  substr($NumeroRCCM,2,3));
			             $annee               = intval(substr($NumeroRCCM,5,4));
				         $typeRCCM            = trim(  substr($NumeroRCCM,9,1));
				         $numCode             = intval(trim(substr($NumeroRCCM,10)));
						 $searchNumeroRCCM    = sprintf("BF%s%d%s%04d",$localite, $annee,$typeRCCM,$numCode);
						 if( $foundRegistre   = $modelRegistre->findRow($searchNumeroRCCM,"numero",null,false) && ($typeRCCM=="A" || $typeRCCM=="B") && ($searchNumeroRCCM!=$NumeroRCCM)) {
						     if( $dbAdapter->delete($tablePrefix."rccm_registre"              , array("registreid=?"=>$registreid))) {
								 $dbAdapter->delete($tablePrefix."rccm_registre_entreprises"  , array("registreid=?"=>$registreid));
								 $dbAdapter->delete($tablePrefix."rccm_registre_representants", array("representantid=?"=>$cleanRepresentantId));
								 $dbAdapter->delete($tablePrefix."rccm_registre_dirigeants"   , array("representantid=?"=>$cleanRepresentantId));
							 }
							 continue;
						 }
					 }
					 ...
					 $foundDoublonRowIFU      = $modelRegistre->getList(array("numifu"=>$numeroIFU,"creatorid"=>26,"types"=>array(1,2)));
					 ...
					 if( count($foundDoublonRowIFU) >= 2 ) {
						 
						 //On vérifie pour voir si le numéro IFU n'existe pas 2X
						 $periodstart = mktime(0,0, 0,5,5,2022);
						 $result   = $dbAdapter->delete($tablePrefix."rccm_registre","creatorid='26' AND creationdate>='".$periodstart."' AND numifu=\"".$numeroIFU."\"");
                         //echo $result; var_dump($foundDoublonRowIFU); die();
					 }
					 ...
					 if( strlen($NumeroRCCM) <= 16) {
			             ...
			             $annee    = intval(substr($NumeroRCCM,5,4));
				         $typeRCCM = trim(  substr($NumeroRCCM,9,1));
				         $numCode  = trim(  substr($NumeroRCCM,10 ));
						
						 if( $localiteRow = $modelLocalite->findRow($localite,"code",null,false)) {
							 if( $annee  != $AnneeRCCM ) {
								 $localiteid         = $localiteRow->localiteid;
								 $registreUpdateData = array("annee"=>$annee,"localiteid"=>$localiteid,"libelle"=>sprintf("%s (A_VERIFIER)", $NomCommercial));
							     if( $dbAdapter->update($tablePrefix."rccm_registre",$registreUpdateData,array("registreid=?"=>$registreid))) {
								     $updatedRegistres[$registreid] = $registre;
								 }
							 }
							 if(($typeRCCM=="M" || $typeRCCM=="S") && ($registre["parentid"]==0)) {
								 if( $dbAdapter->delete($tablePrefix."rccm_registre",array("registreid=?"=>$registreid))) {
									 $dbAdapter->delete($tablePrefix."rccm_registre_representants", array("representantid=?"=>$cleanRepresentantId));
									 $dbAdapter->delete($tablePrefix."rccm_registre_dirigeants"   , array("representantid=?"=>$cleanRepresentantId));
									 $updatedRegistres[$registreid] = $registre;
								 }						 
							 } elseif(($typeRCCM=="M" || $typeRCCM=="S") && ($registre["parentid"]>0)) {
								 $dbAdapter->update($tablePrefix."rccm_registre",array("type"=>4),array("registreid=?"=>$registreid));
							 }
						 }
					 } elseif(false!==$modelModification->checkNum($NumeroRCCM) || ($registre["parentid"]>0)) {
						 if( $dbAdapter->update($tablePrefix."rccm_registre",array("type"=>4),array("registreid=?"=>$registreid))) {
							 $updatedRegistres[$registreid] = $registre;
						 }
					 }
					 $CleanNomCommercial = str_replace($NumeroRCCM,"",$NomCommercial);
					 if(!$modelRegistre->findRow($CleanNomCommercial,"libelle",null,false)) {
						 $registreUpdateData = array( "libelle"=>$CleanNomCommercial);
						 if( $dbAdapter->update($tablePrefix."rccm_registre",$registreUpdateData,array("registreid=?"=>$registreid))) {
							 $updatedRegistres[$registreid] = $registre;
						 }
					 }
		...
		$successMessage = sprintf("Le processus a réussi à supprimer %d doublons des promoteurs et mis à jour %d RCCM", count($deletedRepresentants), count($updatedRegistres));
		
		$this->setRedirect($successMessage, "success");
		$this->redirect("admin/registres/list");
	}
	
	 public function removefromyearAction()
	{
		...
		$currentDate          = date("Y");
		$deletedRepresentants = $updatedRegistres = array();
		if( intval($annee)  && (intval($annee)>$currentDate || intval($annee)<2000) ) {
			$registres        = $model->getList(array("annee"=>intval($annee)));
			if( count(   $registres) ) {
				foreach( $registres as $registre ) {
					     $registreid     = $registre["registreid"];
						 $representantid = $registre["representantid"];
					     if( $dbAdapter->delete($tablePrefix."rccm_registre"              , array("registreid=?"=>$registreid))) {
							 $dbAdapter->delete($tablePrefix."rccm_registre_representants", array("representantid=?"=>$representantid));
							 $dbAdapter->delete($tablePrefix."rccm_registre_dirigeants"   , array("representantid=?"=>$representantid));
							 $updatedRegistres[$registreid] = $registre;
						 }
				}
	...
	public function updateAction()
	{
		...
		$modelEntreprise      = $this->getModel("entreprise");
		$modelRepresentant    = $this->getModel("representant");
		$modelNere            = $modelFNere = new Model_Nerentreprise();
		...
		$fnereEntreprises     = $modelNere->getList(array(),1,10000);
		if( count(   $fnereEntreprises) ) {
			foreach( $fnereEntreprises as $fnereEntreprise ) {
				     $registreid          = (isset($fnereEntreprise["registreid"] ))? intval($fnereEntreprise["registreid"])                 : 0;
				     $NumeroRCCM          = (isset($fnereEntreprise["num_rccm"]   ))? $stringFilter->filter($fnereEntreprise["num_rccm"])    : "";
				     $NumeroIFU           = (isset($fnereEntreprise["num_ifu"]    ))? $stringFilter->filter($fnereEntreprise["num_ifu"])     : "";
				     $NumeroCNSS          = (isset($fnereEntreprise["num_cnss"]   ))? $stringFilter->filter($fnereEntreprise["num_cnss"])    : "";
				     $Description         = (isset($fnereEntreprise["description"]))? $stringFilter->filter($fnereEntreprise["description"]) : "";		
				     $Capital             = (isset($fnereEntreprise["capital"]    ))? $stringFilter->filter($fnereEntreprise["capital"])     : "";	
			         $updateRegistreData  = array();
					 if(!intval($registreid) || empty($NumeroRCCM)) {
						 $errorMessages[] = sprintf("Numéro RCMM %s invalide", $NumeroRCCM);
						 continue;
					 }
					 if( $registreRow     = $modelRegistre->findRow(intval($registreid),"registreid",null,false)) {
						 if( empty($registreRow->numifu) ) {
							 try {
								 $updateRegistreData["numifu"]        = $NumeroIFU;
								 $updateRegistreData["updatedate"]    = time();
							     $updateRegistreData["updateduserid"] = 1;
								 $dbAdapter->update( $tablePrefix."rccm_registre", $updateRegistreData, array("registreid=?"=>$registreid));
							 } catch(Exception $e ) {
								 $errorMessages[]                     = sprintf("La MAJ du numéro RCCM %s a echoué pour les raisons suivantes : %s ", $NumeroRCCM, $e->getMessage());
								 continue;
							 }
						 }
						 if( empty($registreRow->numcnss) ) {
							 try {
								 $updateRegistreData["numcnss"]       = $NumeroCNSS;
								 $updateRegistreData["updatedate"]    = time();
							     $updateRegistreData["updateduserid"] = 1;
								 $dbAdapter->update( $tablePrefix."rccm_registre", $updateRegistreData, array("registreid=?"=>$registreid));
							 } catch(Exception $e ) {
    ...
    public function viewpdfAction()
	{
      ...
      $pdfToText = XPDF\PdfToText::create(array("pdftotext.binaries" =>"E:\webserver\www\binaries\Xpdf\pdftotext.exe","pdftotext.timeout"=> 30,), $logger);
	...
	{		
		...
		$filters              = array("libelle"=>null,"numero"=>null,"numcnss"=>null,"numifu"=>null,"domaineids"=>null,"domaineid"=>0,"creatorid"=>0,"localiteids"=>null,"localiteid"=>0,"annee"=>0,"nom"=>null,"prenom"=>null,"searchQ"=>null,"date_year"=>null,"date_month"=>null,"date_day"=>null,"passport"=>null,"telephone"=>null,"country"=>0,"name"=>null,"type"=>0,"keywords"=>null,
		                              "searchfrom"=>"erccm","periode_start_day"=>1,"periode_start_month"=>1,"periode_start_year"=>2000,"periode_end_day" =>31,"periode_end_month"=>12,"periode_end_year"=>date("Y"));		
		$findSimilar          = (isset($params["findsimilar"])) ? intval($params["findsimilar"]) : 0;
		$similarSearchQ       = (isset($params["searchq"]    )) ? urldecode($params["searchq"]) : "";
		...
		$this->view->localites        = $modelLocalite->getSelectListe("Localité"         , array("localiteid", "libelle") , array() , null , null , false );
		$this->view->users            = array(0 => "Selectionnez le créateur")+$modelLocalite->getUsersList();
	...
	public function exportcsvAction()
	{
		...
		$pageNum              = (isset($params["page"]))    ? intval($params["page"])     : 1;
		$pageSize             = (isset($params["maxitems"]))? intval($params["maxitems"]) : 0;	
		
		$filters              = array("libelle"=>null,"numero"=>null,"domaineid"=>0,"creatorid"=>0,"localiteid"=>0,"annee"=>0,"nom"=>null,"prenom"=>null,"searchQ"=>null,
		                              "date_year"=> null,"date_month"=> null,"date_day"=>null,"passport"=>null,"telephone"=>null,"country"=>0,"name"=>null,"keywords"=>null,
				                      "periode_start_day"=>DEFAULT_START_DAY,"periode_start_month"=> DEFAULT_START_MONTH,"periode_start_year"=>DEFAULT_START_YEAR,
				                      "periode_end_day"  =>DEFAULT_END_DAY,"periode_end_month"=>DEFAULT_END_MONTH,"periode_end_year"=>DEFAULT_END_YEAR,"SANS_IFU"=>1);		
		$findSimilar          = (isset($params["findsimilar"]))? intval($params["findsimilar"]) : 0;
		$similarSearchQ       = (isset($params["searchq"]    ))? urldecode($params["searchq"])  : "";
		if(!empty(   $params )) {
			foreach( $params as $filterKey => $filterValue){
        ...
        $filters["SANS_IFU"]          = 1;
		$registres                    = $model->getList( $filters,1,100000);
		...
		if( count(   $registres ) ) {			
			...
			foreach( $registres as $registre ) {
					 ...
					 $capital                             = 0;
                     $registreType                        = (isset($registre["type"])           && intval($registre["type"]          ))?intval($registre["type"])   : 1;
					 $formeJuridique                      = "Entreprise Individuelle";
					 $registreTypeLibelle                 = "Personne Physique";
					 $entrepriseRow                       = $modelEntreprise->getList(array("registreid"=>$registreid));
					 if( isset($entrepriseRow[0]["capital"]) ) {
						 $capital                         = (floatval($entrepriseRow[0]["capital"]    ))? number_format($entrepriseRow[0]["capital"],0, " ", " ") : "";
						 $formeJuridique                  = (isset($entrepriseRow[0]["formeJuridique"]))? $entrepriseRow[0]["formeJuridique"] : "";
					 }
					 ...
					 switch( $registreType ) {
						...
						case 2:
							...
							break;
						case 3:
						    $registreTypeLibelle          = "Surété";
							break;
						case 4:
						    $registreTypeLibelle          = "Modification RCCM";
							break;
					 }					 
				     $csvRowData                          = array();
					 $csvRowData["TYPE_ENTREPRISE"]       = $registreTypeLibelle;
					 $csvRowData["LOCALITE"]              = strtoupper($registre["localite"]);
					 $csvRowData["NUMERO_RCCM"]           = strtoupper(Sirah_Functions_String::cleanUtf8($NumeroRCCM));
					 $csvRowData["NUMERO_IFU"]            = strtoupper(Sirah_Functions_String::cleanUtf8($registre["numifu"] ));
					 $csvRowData["NUMERO_CNSS"]           = strtoupper(Sirah_Functions_String::cleanUtf8($registre["numcnss"]));
					 $csvRowData["NOM"]                   = "\xEF\xBB\xBF".$registre["nom"];
					 $csvRowData["PRENOM"]                = "\xEF\xBB\xBF".$registre["prenom"];
					 $csvRowData["ADRESSE"]               = "\xEF\xBB\xBF".$registre["adresse"];
					 $csvRowData["EMAIL"]                 = $registre["email"];
					 $csvRowData["TELEPHONE"]             = preg_replace("/[^0-9\+\-\_\s]/i","",str_ireplace("\\"," ",$registre["telephone"]));
					 $csvRowData["DATE_NAISSANCE"]        = $registre["date_naissance"];
					 $csvRowData["LIEU_NAISSANCE"]        = "\xEF\xBB\xBF".$registre["lieunaissance"];
					 $csvRowData["NOM_COMMERCIAL"]        = $registre["libelle"];
					 $csvRowData["SECTEUR"]               = "\xEF\xBB\xBF".$registre["domaine"];
					 $csvRowData["ACTIVITE_DECLAREE"]     = "\xEF\xBB\xBF".$registre["description"];
					 $csvRowData["FORME_JURIDIQUE"]       = "\xEF\xBB\xBF".$formeJuridique;
					 $csvRowData["CAPITAL"]               = $capital;
					 $csvRowData["SEXE"]                  = ((stripos("M", $registre["sexe"])!==false) || (stripos("H", $registre["sexe"])!==false))? "Homme" : "Femme";
					 $csvRowData["PASSPORT"]              = strtoupper(Sirah_Functions_String::cleanUtf8($registre["passport"]));
					 $csvRowData["NATIONALITE"]           = (isset($nationalites[$registreCountry]))?Sirah_Functions_String::cleanUtf8($nationalites[$registreCountry]) : Sirah_Functions_String::cleanUtf8($registre["country"]);
					 $csvRowData["DATE_ENREGISTREMENT"]   = (isset($registre["date"]))?date("d/m/Y", $registre["date"]): $registre["date_registre"];
					 $csvRowData["SITUATION_MATRIMONIAL"] = strtoupper(Sirah_Functions_String::cleanUtf8($registre["marital_status"]));						
			         $csvRows[$NumeroRCCM]                = $csvRowData;	     
			}
			if( count( $csvRows )) {
				$csvHeader   = array("LOCALITE","TYPE_ENTREPRISE","NUMERO_RCCM","NUMERO_IFU","NUMERO_CNSS","DATE_ENREGISTREMENT","NOM_COMMERCIAL","SECTEUR","ACTIVITE_DECLAREE","FORME_JURIDIQUE","CAPITAL","NOM","PRENOM","LIEU_NAISSANCE","DATE_NAISSANCE","SEXE","ADRESSE","TELEPHONE","EMAIL","PASSPORT","NATIONALITE","SITUATION_MATRIMONIAL");
				$csvAdapter  = Sirah_Filesystem_File::fabric("Csv", array("filename"=>$rccmDocumentDest,"has_header" => true, "header" => $csvHeader ) , "wb+" );
				if( $csvAdapter->save( $csvRows ) ) {
				...
				if( count(   $userSimpleRegistres ) ) {
					foreach( $userSimpleRegistres as $simpleRegistre ) {
							 ...
							 if(!intval($simpleRegistre["domaineid"]) && empty($simpleRegistre["description"]) ) {
								 $isValid          = false;
								 $thisSolution     = $errorSolution.". Vous sélectionnez un domaine d'article, puis vous saisissez dans le champ `Décrire l'article`";
								 $opsDataErrors[$registreid]["errors"][] = array("title"=>"Domaine d'article non renseigné","description"=>"Le domaine d'article de ce RCCM est vide. Veuillez indiquer au moins un secteur d'article ","solution"=>$thisSolution);
							 }
	...
	protected function __getRccmDocument($numero,$registreid=0,$type="FORMULAIRE",$model=null)
	{
		if( empty($type) ) {
			$type          = "FORMULAIRE";
		}
		...
		if( null == $model) {
			$modelDocument = $model = $this->getModel("document");
		}
		$filename          = sprintf("%sFORMULAIRE", $numero);
		if( $type== "PERSONNEL") {
			$filename      = sprintf("%sPERSONNEL", $numero);
		} elseif($type=="STATUT") {
			$filename      = sprintf("%sSTATUT", $numero);
		}
		$modelTable        = $model->getTable();
		$prefixName        = $modelTable->info("namePrefix");
		$dbDestination     = $dbAdapter = $modelTable->getAdapter();
		$selectDocument    = $dbAdapter->select()->from(     array("D" => $tablePrefix."system_users_documents"))
											     ->joinLeft( array("RD"=> $tablePrefix."rccm_registre_documents"),"RD.documentid=D.documentid",array("RD.registreid"))
                                   			     ->where("D.filename LIKE ?",$filename);
		if( intval($registreid) ) {
			$selectDocument->orWhere("RD.registreid=?", intval($registreid));
		}			
        return 	$dbAdapter->fetchRow($selectDocument, array() , Zend_Db::FETCH_OBJ);
	}
	
	
	protected function __getDirigeantByPhone($telephone,$model=null)
	{
		if( empty($telephone) ) {
			return false;
		}
		if( null == $model) {
			$modelRepresentant      = $model = $this->getModel("representant");
		}
		$modelTable                 = $model->getTable();
		$prefixName                 = $modelTable->info("namePrefix");
		$dbDestination              = $dbAdapter = $modelTable->getAdapter();
		$phoneParts                 = preg_split("/[\-;,\\/]/", $telephone);
		 
		$querySelect                = "SELECT R.*,D.registreid FROM rccm_registre_representants R
											INNER JOIN rccm_registre_dirigeants D ON D.representantid=R.representantid ";
		if( count($phoneParts) && isset($phoneParts[0]) && !empty($phoneParts[0])) {
			if( isset($phoneParts[0]) && !empty($phoneParts[0])) {
				$phone1             = preg_replace('/[^0-9.]/', '',$phoneParts[0]);
				$querySelect       .= "  WHERE (REGEXP_REPLACE(R.telephone,'[^0-9]','') LIKE \"%".$phone1."%\")" ;
			}
			if( isset($phoneParts[1]) && !empty($phoneParts[1])) {
				$phone2             = preg_replace('/[^0-9.]/', '',$phoneParts[1]);
				$querySelect       .= " OR (REGEXP_REPLACE(R.telephone,'[^0-9]','') LIKE \"%".$phone2."%\")";
			}
			if( isset($phoneParts[2]) && !empty($phoneParts[2])) {
				$phone3             = preg_replace('/[^0-9.]/', '',$phoneParts[2]);
				$querySelect       .= " OR (REGEXP_REPLACE(R.telephone,'[^0-9]', '') LIKE \"%".$phone3."%\")";
			}
			 
			$statement              = $dbAdapter->query($querySelect);
	...
	public function importcsvAction()
	{
		...
		$errorMessages              = array();
		$jsonCsvRows                = array();
		$defaultData                = array("localiteid"=>1,"domaineid"=>79,"annee"=>2025);
		...
		$localitesCodes             = $modelLocalite->getSelectListe("Selectionnez une localité"         , array("code"      ,"localiteid"), array() , 0 , null , false);
		$annees                     = array("2000"=>"2000","2001"=>"2001","2002"=>"2002","2003"=>"2003","2004"=>"2004","2005"=>"2005","2006"=>"2006","2007"=>"2007","2008"=>"2008","2009"=>"2009","2010"=>"2010","2011"=>"2011",
		...
		if( $this->_request->isPost() ) {
			...
			$localiteid             = (isset($postData["localiteid"]) && isset($localites[$postData["localiteid"]]))? $postData["localiteid"] : 1;
			$domaineid              = (isset($postData["domaineid"])  && isset($domaines[$postData["domaineid"]]  ))? $postData["domaineid"]  : 79;	
			$annee                  = (isset($postData["annee"])      && isset($annees[$postData["annee"]]        ))? $postData["annee"]      : 2025;	
			$strNotEmptyValidator   = new Zend_Validate_NotEmpty(array("integer","zero","string","float","null"));
			//On crée les filtres qui seront utilisés sur les données du formulaire
			$stringFilter           = new Zend_Filter();
			$stringFilter->addFilter( new Zend_Filter_StringTrim());
			...
			if( $documentsUploadAdapter->isUploaded("registres") ) {
				...
				if( $documentsUploadAdapter->isReceived("registres") ) {
					...
					if( count(   $csvData )) {
						$jsonCsvRows["rows"]         = array();	
						foreach( $csvData as $csvRow ) {
							     $NumeroRCCM         = $numero = (isset($csvRow["NUMERO_RCCM"] ))? trim($csvRow["NUMERO_RCCM"])    : "";
								 $NumeroIFU          = (isset($csvRow["NUMERO_IFU"]    ))? trim($csvRow["NUMERO_IFU"])     : "";
								 $NumeroCNSS         = (isset($csvRow["NUMERO_CNSS"]   ))? trim($csvRow["NUMERO_CNSS"])    : "";
								 $NomCommercial      = (isset($csvRow["NOM_COMMERCIAL"]))? trim($csvRow["NOM_COMMERCIAL"]) : "";
								 $Activite           = (isset($csvRow["ACTIVITE"]      ))? trim($csvRow["ACTIVITE"])       : "";
								 $Telephone          = (isset($csvRow["TELEPHONE"]     ))? trim($csvRow["TELEPHONE"])      : "";
								 $DateEnregistrement = $dateRegistre = (isset($csvRow["DATE_ENREGISTREMENT"]))?$csvRow["DATE_ENREGISTREMENT"] : "";
								  
								 if( empty($NumeroRCCM) || empty($NomCommercial) || empty($Telephone) || empty($DateEnregistrement)) {
									 continue;
								 }
								 ...
								 $numTypeRegistre    = (isset($numeroParts["typeRCCM"]))? $numeroParts["typeRCCM"] : strtoupper(trim(substr($NumeroRCCM, 9, 1)));
								 $annee              = (isset($numeroParts["annee"]))?intval($numeroParts["annee"]) : substr($NumeroRCCM, 5, 4);
								 $localiteid         = (isset($numeroParts["localite"]) && isset($localitesCodes[$numeroParts["localite"]]))? $localitesCodes[$numeroParts["localite"]] : 0;
								 if(!intval($localiteid)) {
									 $localiteCode   = substr($NumeroRCCM,3,3);
									 var_dump($localiteCode); die();
								 }
								 if(!intval($localiteid) || !intval($annee)) {
									 $errorMessages[]= sprintf("Localité Invalide : %d / %d", $localiteid, $annee);
									 continue;
								 }
								 if(!empty($NomCommercial) && !empty($Telephone) && (!$foundRegistre=$modelRegistre->findRow($NumeroRCCM,"numero",null,false))) {
								     $foundDirigeant                      = $this->__getDirigeantByPhone($Telephone,$modelRepresentant);
									 if( false!= $foundDirigeant) {
										 $representantid                  = $dirigeantid         = (isset($foundDirigeant["representantid"]))? $foundDirigeant["representantid"] : 0;
										 $registreid                      = $dirigeantRegistreId = (isset($foundDirigeant["registreid"]    ))? $foundDirigeant["registreid"]     : 0;
										 $foundRegistreByDirigeant        = ($registreid)?$modelRegistre->findRow($registreid,"registreid",null,false) : null;
										 if( $foundRegistreByDirigeant ) {
											 $registreid                  = 0;
										 }
										 $registre_data                   = array();
										 $registre_data["registreid"]     = $registreid;
										 $registre_data["numero"]         = $NumeroRCCM;
										 $registre_data["libelle"]        = $NomCommercial;
										 $registre_data["localiteid"]     = $localiteid;
										 $registre_data["date"]           = $inscriptionDate;
										 $registre_data["annee"]          = $annee;
										 $registre_data["type"]           = 1;
										 $registre_data["statut"]         = 1;
										 $registre_data["category"]       = "P0";
										 $registre_data["checked"]        = 1;
										 $registre_data["description"]    = $Activite;
										 $registre_data["adresse"]        = (isset($foundDirigeant["adresse"]))?$foundDirigeant["adresse"] : "";
										 $registre_data["telephone"]      = $Telephone;
										 $registre_data["creatorid"]      = (isset($foundDirigeant["creatorid"]    ))? $foundDirigeant["creatorid"]     : $me->userid;
										 $registre_data["creationdate"]   = (isset($foundDirigeant["creationdate"] ))? $foundDirigeant["creationdate"]  : (time()+100);	
										 $registre_data["updateduserid"]  = (isset($foundDirigeant["updateduserid"]))? $foundDirigeant["updateduserid"] : 0;
										 $registre_data["updatedate"]     = (isset($foundDirigeant["updatedate"]   ))? $foundDirigeant["updatedate"]    : 0;
										 $registre_data["domaineid"]      = $domaineid;
										 $registre_data["parentid"]       = 0;
										 try {
											 if( $dbAdapter->insert( $prefixName."rccm_registre", $registre_data)) {
												 $registreid              = (intval($dirigeantRegistreId))?$dirigeantRegistreId : $dbAdapter->lastInsertId();
												 if(!intval($dirigeantRegistreId)) {
													 $dbAdapter->insert( $prefixName ."rccm_registre_dirigeants", array("registreid"=> $registreid,"representantid"=>$representantid,"fonction"=> "GERANT"));
												 }
												 $successMessages[]       = sprintf("Le RCCM numéro %s a été importé avec succès.", $NumeroRCCM);
											 } else {
												 $errorMessages[]         = sprintf("Le RCCM numéro %s n'a pas été importé.", $NumeroRCCM);
											 }
										 } catch(Exception $e) {
											 $errorMessages[]             = sprintf("Le RCCM numéro %s n'a pas été importé : %s", $NumeroRCCM, $e->getMessage());
										 }
									 } else {
										     $errorMessages[]             = sprintf("Le RCCM numéro %s avec le numéro téléphone %s n'a pas de dirigeants dans la base.", $NumeroRCCM,$Telephone);
									 }
								 }
						}
					}								
	...
	public function lostrccmAction()
	{
		Imagick::setResourceLimit(Imagick::RESOURCETYPE_MEMORY,33554432); 
		Imagick::setResourceLimit(Imagick::RESOURCETYPE_MAP, 63554432);
		$this->_helper->viewRenderer->setNoRender(true);
		$this->_helper->layout->disableLayout(true);
		$pdfTotextBinary    = "E:\\webserver/www/erccm/binaries/poppler-25.07.0/Library/bin/";
		...
		$defaultData        = array("annee"=>2025,"type"=>"PHYSIQUES","localites"=>array("ORD","DDG","DRI","TNK","GAO","FDG","LEO","BFR","ORD","ZNR","MNG","GAO","KYA","OHG","KDG"),"rootpath"=>APPLICATION_INDEXATION_STOCKAGE_FOLDER,"category" =>"A","findInRoot" => false, "from" => 1, "to" => 200);
		//$defaultData        = array("annee"=>2025,"type"=>"PHYSIQUES","localites"=>array("FDG"),"rootpath"=>APPLICATION_INDEXATION_STOCKAGE_FOLDER,"category" =>"A","findInRoot" => false, "from" => 1, "to" => 200);
		$errorMessages      = array();
	    ...
	    $rccmPath           = APPLICATION_INDEXATION_STOCKAGE_FOLDER;
		$localites          = $modelLocalite->getSelectListe(null, array("code", "libelle") , array() , null , null , false );
		$annees             = array("2000"=>"2000","2001"=>"2001","2002"=>"2002","2003"=>"2003","2004"=>"2004","2005"=>"2005","2006"=>"2006","2007"=>"2007","2008"=>"2008","2009"=>"2009","2010"=>"2010","2011"=>"2011",
		                            "2012"=>"2012","2013"=>"2013","2014"=>"2014","2015"=>"2015","2016"=>"2016","2017"=>"2017","2018"=>"2018","2019"=>"2019","2020"=>"2020","2021"=>"2021","2022"=>"2022");
		$selectedType       = (isset($defaultData["type"]     ))? $defaultData["type"]          : "PHYSIQUES";
		$selectedAnnee      = (isset($defaultData["annee"]    ))? intval($defaultData["annee"]) : 2025;
		$selectedLocalites  = (isset($defaultData["localites"]))? $defaultData["localites"]     : array("ORD","FDG","LEO","BFR");
	    if(!is_dir($TMPDir.DS.$selectedType)) {
			@chmod($TMPDir,0777);
			@mkdir($TMPDir.DS.$selectedType);
			@chmod($TMPDir.DS.$selectedType,0777);
		}
        $foundRCCMText      = array();
	    if( count(   $selectedLocalites) ) {
			foreach( $selectedLocalites as $selectedLocalite) {
				     $srcLocalitePath               = $rccmPath.DS.$selectedType.DS.$selectedLocalite.DS.$selectedAnnee;
					 $destLocalitePath              = $TMPDir  .DS.$selectedType.DS.$selectedLocalite.DS.$selectedAnnee;
					 $checkedFilesPS                = glob($srcLocalitePath."/*/*-PS.pdf");
					 if(!is_dir($TMPDir.DS.$selectedType.DS.$selectedLocalite)) {
						  @chmod($TMPDir.DS.$selectedType,0777);
						  @mkdir($TMPDir.DS.$selectedType.DS.$selectedLocalite);
						  @chmod($TMPDir.DS.$selectedType.DS.$selectedLocalite,0777);
					 }
					 if(!is_dir( $TMPDir.DS.$selectedType.DS.$selectedLocalite.DS.$selectedAnnee)) {
						  @chmod($TMPDir.DS.$selectedType.DS.$selectedLocalite,0777);
						  @mkdir($TMPDir.DS.$selectedType.DS.$selectedLocalite.DS.$selectedAnnee);
						  @chmod($TMPDir.DS.$selectedType.DS.$selectedLocalite.DS.$selectedAnnee,0777);
					 }
					 /*echo $srcLocalitePath;
					 var_dump($checkedFilesPS); die();*/
					 if( count(   $checkedFilesPS) ) {
						 foreach( $checkedFilesPS as $checkedFilePS ) {
							      $NumeroRCCM       = str_ireplace(array("-PS.pdf",".pdf"),"", basename($checkedFilePS));
								  $rccmText         = "";
								  $imageFiles       = array();
								  if( empty($NumeroRCCM)) {
									  continue;
								  }
								  if(!$foundRCCM    = $model->findRow($NumeroRCCM,"numero",null,false)) {
									  $foundItems[] = $NumeroRCCM;
								  }
								  if( file_exists($checkedFilePS) && !empty($NumeroRCCM)) {
									  $pdfToText    = new Pdf($pdfTotextBinary."pdftotext.exe");
									  $pdfToText->setPdf($checkedFilePS);
									  $pdfToText->setOptions(['layout', 'r 96']);
									  $foundRCCMText[$NumeroRCCM] = $rccmText = $pdfToText->text();
								  }
								  if( file_exists($checkedFilePS) && !empty($NumeroRCCM)) {
									  try {
										  $pdfImagePath     = $TMPDir.DS.$selectedType.DS.$selectedLocalite.DS.$selectedAnnee.DS.$NumeroRCCM.DS.sprintf("%s.png",$NumeroRCCM);
										  $outputPathPrefix = $TMPDir.DS.$selectedType.DS.$selectedLocalite.DS.$selectedAnnee.DS.$NumeroRCCM.DS.sprintf("%s_page_",$NumeroRCCM);
										  if(!is_dir($TMPDir.DS.$selectedType.DS.$selectedLocalite.DS.$selectedAnnee.DS.$NumeroRCCM)) {
										      @chmod($TMPDir.DS.$selectedType.DS.$selectedLocalite.DS.$selectedAnnee,0777);
											  @mkdir($TMPDir.DS.$selectedType.DS.$selectedLocalite.DS.$selectedAnnee.DS.$NumeroRCCM);
											  @chmod($TMPDir.DS.$selectedType.DS.$selectedLocalite.DS.$selectedAnnee.DS.$NumeroRCCM,0777);
										  }	
                                          if(!file_exists($outputPathPrefix."0.png")) {
											  $imagick      = new Imagick();
											  $imagick->setResolution(300, 300);
											  $imagick->readImage($checkedFilePS);
											  $imagick->setImageFormat('png');
											  $imagick->writeImages($outputPathPrefix . '%d.png', false);
											  $imagick->destroy();
											  
										  }	
                                          if( file_exists($outputPathPrefix."0.png")) {
											  $imageFiles = glob($outputPathPrefix.'*.png');
										  }											  
										  if( count(   $imageFiles) ) {
											  foreach( $imageFiles as $imageFile) {
													   $pdfToText  = new TesseractOCR($imageFile);
													   $pdfToText->lang('eng', 'fra', 'spa');
													   $text       = $pdfToText->run();
													   $rccmText  .= $text;
											  }
                                              $foundRCCMText[$NumeroRCCM] = $rccmText; 											  
										  }
									  } catch(Exception $e) {
										  $errorMessages[] = sprintf("Erreur de conversion en texte : %s", $e->getMessage());
									  }
								  }
						 }
					 }
		...
		$regex1         = "/Enseigne C(?:.{1,2})\.:\s*(.*)\s*Forme/";//Nom commercial
	    $regex2         = '/Nom commercial\s*:\s*(.{1,300})\s*\n*/';
		$regex3         = "#Dénomination\s*:\s*(.*)\s*\n*Sigle#";//Nom commercial
	    $regex4         = "/(?<= postale|postal|postat|poslal\):)(.*)(?=Tel|Tél|Té)/";//Adresse
	    $regex5         = "#EXERCEE\s*\(S\)\s*:\s*(.*)#";//Activité Principale 1
	    $regex6         = "#le\s*(.*)\s*2025#";//Date d'inscription 1
	    $regex7         = "#de début(?:.*):(.+)(?=N\"RCCM:)#";//Date d'inscription 2
	    $regex8         = "/Activité Principale\s*:\s*(.*)\s*\n*Adresse/";//Activité Principale 2
	    $regex9         = "/(?<=MATRIMONIALE:)(.*)/";//Situation matrimoniale
		$regex10        = '/\.Tel\s*:(.{1,50})/';
	    $regex11        = "#Tél\s*:\s*(.*)\s*\n*B\.?P#";
		$regex12        = "#Tél|Tel\.*\s*:\s*(.{1,50})\s*\n*:#";
		$regex13        = "#DATE DE DEBUT(?:.*):(.*)\s*\n*(?=N)#";
		$regex14        = "#Activité\s*Principale\s*:\s*(.*)\s*\n*Adresse\s:#";
		$regex15        = "#Téléphone\s*:\s*(.*)\s*BP#";
		$regex16        = "#Secteur d\'activités\s*:\s*(.*)\s*\n*Adresse\s:#";
		$regex17        = "#AUTRES PRECISIONS*:\s*(.*)\s*\n*COORDONNEES#";
		$regex18        = "#CNIB N°\s*:\s*(.*)\s*DU#";
		$missingOutput  = "
							<table width=\"100%\" class=\"table table-striped\">
							    <thead>
									<tr>
										<th width=\"15%\">Numéros RCCM </th>
										<th width=\"20%\">Noms commerciaux</th>
										<th width=\"25%\">Activités</th>
										<th width=\"13%\">Téléphones</th>
										<th width=\"12%\">CNIB</th>
										<th width=\"15%\">Dates d'inscriptions</th>
									</tr>
                                </thead>								
								<tbody>";
		if( count(   $foundItems)) {
			foreach( $foundItems as $index  => $NumeroRCCM ) {
				     $csvRowData            = array();
				     $rccmText              = (isset($foundRCCMText[$NumeroRCCM]))?$foundRCCMText[$NumeroRCCM] : "";
					 $foundRegexTelephones  = $foundRegexTelephone   = $foundRegexPhone= $foundRegexNom=$foundRegexNoms=$foundRegexNC=$foundRegexActivities=$foundRegexActivite=array();
					 $NomCommercial         = $Activite = $Telephone = $CNIB = $Date   = "";
					 if(!empty($rccmText)) {
						 preg_match($regex1,$rccmText,$foundRegexNom);
						 $NomCommercial     = (isset($foundRegexNom[1]))?$foundRegexNom[1] : "";
						 if( empty($NomCommercial) ) {
							 preg_match($regex2,$rccmText,$foundRegexNoms);
							 $NomCommercial = (isset($foundRegexNoms[1]))?$foundRegexNoms[1] : "";
						 }
						 if( empty($NomCommercial) ) {
							 preg_match($regex3,$rccmText,$foundRegexNC);
							 $NomCommercial = (isset($foundRegexNC[1]))?$foundRegexNC[1] : "";
						 }
						 preg_match($regex8,$rccmText,$foundRegexActivities);
						 $Activite          = (isset($foundRegexActivities[1]))?$foundRegexActivities[1] : "";
						 if( empty($Activite)) {
							 preg_match($regex5,$rccmText,$foundRegexActivite);
						     $Activite      = (isset($foundRegexActivite[1]))?$foundRegexActivite[1] : "";
						 }
						 if( empty($Activite) ) {
							 preg_match($regex16,$rccmText,$foundRegexActivity);
							 $Activite      = (isset($foundRegexActivity[1]))?$foundRegexActivity[1] : "";
						 }
						 if( empty($Activite) ) {
							 preg_match($regex14,$rccmText,$foundRegexActivites);
							 $Activite      = (isset($foundRegexActivites[1]))?$foundRegexActivites[1] : "";
						 }
						 preg_match($regex10,$rccmText,$foundRegexTelephone);
						 $Telephone         = (isset($foundRegexTelephone[1]))?$foundRegexTelephone[1] : "";
						 if( empty($Telephone) ) {
							 preg_match($regex11,$rccmText,$foundRegexPhone);
							 $Telephone     = (isset($foundRegexPhone[1]))?$foundRegexPhone[1] : "";
						 }
						 if( empty($Telephone) ) {
							 preg_match($regex12,$rccmText,$foundRegexTelephones);
							 $Telephone     = (isset($foundRegexTelephones[1]))?$foundRegexTelephones[1] : "";
						 }
						 if( empty($Telephone) ) {
							 preg_match($regex15,$rccmText,$foundRegexPhones);
							 $Telephone     = (isset($foundRegexPhones[1]))?$foundRegexPhones[1] : "";
						 }
						if( empty($Telephone) ) {
							 preg_match($regex17,$rccmText,$foundRegexNumero);
							 $Telephone     = (isset($foundRegexNumero[1]))?$foundRegexNumero[1] : "";
						 }
						 preg_match($regex6,$rccmText,$foundRegexDate);
						 $Date              = (isset($foundRegexDate[1]))?$foundRegexDate[1] : "";
						 if( empty($Date) ) {
							 preg_match($regex13,$rccmText,$foundRegexDates);
							 $Date          = (isset($foundRegexDates[1]))?$foundRegexDates[1] : "";
						 }
						 preg_match($regex18,$rccmText,$foundCNIB);
						 $CNIB              = (isset($foundCNIB[1]))?$foundCNIB[1] : "";
						 if( empty($Activite) ) {
							 //$Activite  = $rccmText;
						 }							 
					 }
					 $csvRowData["NUMERO_RCCM"]           = $NumeroRCCM;
					 $csvRowData["NUMERO_IFU"]            = "";
					 $csvRowData["NUMERO_CNSS"]           = "";
					 $csvRowData["NOM"]                   = "";
					 $csvRowData["PRENOM"]                = "";
					 $csvRowData["ADRESSE"]               = "";
					 ...
					 $csvRowData["NOM_COMMERCIAL"]        = $NomCommercial;
					 $csvRowData["ACTIVITE"]              = $Activite;
					 $csvRowData["SEXE"]                  = "";
					 $csvRowData["PASSPORT"]              = $CNIB;
					 $csvRowData["NATIONALITE"]           = "";
					 $csvRowData["DATE_ENREGISTREMENT"]   = $Date;
					 $csvRowData["SITUATION_MATRIMONIAL"] = "Celibataire";						
			         $csvRows[$index]                     = $csvRowData;
					 
					 $missingOutput.= "<tr>";
					 $missingOutput.=        "<td>".$NumeroRCCM    ."</td>";
					 $missingOutput.=        "<td>".$NomCommercial ."</td>";
					 $missingOutput.=        "<td>".$Activite      ."</td>";
					 $missingOutput.=        "<td>".$Telephone     ."</td>";
					 $missingOutput.=        "<td>".$CNIB          ."</td>";
					 $missingOutput.=        "<td>".$Date          ."</td>";
					 $missingOutput.= "</tr>";
				     
			}
		} else {
			         $missingOutput.= "<tr><td align=\"center\" COLSPAN=\"6\"> AUCUN RCCM NON INDEXE N'A ETE TROUVE </td></tr>";
		}
		$missingOutput  .= "     </tbody>
		                      </table>";
		$rccmDocumentDest         = $TMPDir. DS . "ARCHIVES" .DS.  "ListRCCM.csv" ;
		if(!is_dir( $TMPDir. DS . "ARCHIVES")) {
			@mkdir( $TMPDir. DS . "ARCHIVES");
			@chmod();
		}
		if( file_exists( $rccmDocumentDest )) {
			@unlink($rccmDocumentDest);
		}					  
		if( count( $csvRows ) && empty($errorMessages)) {
	...
	public function findmissingAction()
	{
		...
		$defaultData        = array("start_annee"=>2000,"end_annee"=>2021,"localites"=>array("OUA","BBD","BFR","ORD","ZNR","MNG","GAO","KYA","OHG","KDG"),"rootpath"=>APPLICATION_INDEXATION_STOCKAGE_FOLDER,"category" =>"A","findInRoot" => false, "from" => 1, "to" => 200);
		$errorMessages      = array();
		$successMessages    = array();
		$notSavedItems      = array();
		...
		if( $this->_request->isPost( )) {						
			...
			if(!empty($categoryLibelle)) {
				$srcPath         = $srcPath.DS.$categoryLibelle;
			}
			if(!is_dir($srcPath )) {
			...
			if( empty( $errorMessages)) {
				foreach($checkedLocalites as $checkedLocaliteCode ) {
						...
						foreach( $years as $annee ) {
								 ...
								 echo $checkedFilePath."/".$numRccm;
								 var_dump(is_dir($checkedFilePath)); die();
								 ...
								 if( count($totalFiles)) {
									 ...
									 ECHO $checkedFilePath."/".$numRccm."*.pdf"; die();
								 } 								 
						}						  
				}			
			}  
	...
	public function localitestatsAction()
	{
		...
		$localites             = $localitesCodes = $modelLocalite->getSelectListe("Selectionnez une localité", array("code","libelle"), array() , 0 , null , false);
		$localiteIDS           = $modelLocalite->getSelectListe("Selectionnez une localité", array("code","localiteid"), array() , 0 , null , false);
		$annees                = array("2000"=>"2000","2001"=>"2001","2002"=>"2002","2003"=>"2003","2004"=>"2004","2005"=>"2005","2006"=>"2006","2007"=>"2007","2008"=>"2008","2009"=>"2009","2010"=>"2010",
		                               "2011"=> "2011","2012"=>"2012","2013"=> "2013", "2014"=> "2014","2015"=> "2015", "2016"=> "2016", "2017" => "2017","2018"=>"2018","2019"=>"2019","2020"=>"2020","2021"=>"2021","2022"=>2022);
		$defaultMaxNumByYears  = array("2000"=>"526","2001"=>"755","2002"=>"926","2003"=>"792","2004"=>"930","2005"=>"868","2006"=>"737","2007"=>"648","2008"=>"569","2009"=>"508","2010"=>"594","2011"=> "668","2012"=>"878","2013"=> "939","2014"=>"1401","2015"=>"1339","2016"=>"2147","2017"=>"2112","2018"=>"5000",
		                               "2019"=>"5000","2020"=>"10000","2021"=>"10000","2022"=>"10000");       
		...
		if( $this->_request->isPost() ) {
			...
			if( count(   $checkedAnnees ) && empty($errorMessages) ) {
				foreach( $checkedAnnees as $checkedAnneeKey => $checkedAnneeVal ) {
						 ...
						 $checkedDirs    = glob($srcPath."/*/".$localiteCode."/".$checkedAnneeVal."/*", GLOB_ONLYDIR);
						 
						 $checkedDirs    = array_filter($checkedDirs, function($v) {return false === strpos(basename($v), 'SURETE');});
						 $totalRegistres = count($checkedDirs);
						 if( count($checkedDirs) ) {
							 asort($checkedDirs);
						 }
						 $lastDir        = end($checkedDirs);
						 $Dir_NumeroRCCM = ($totalRegistres)?basename($lastDir) : "";  
						 if(!empty($Dir_NumeroRCCM) ) {
							 $lastNumRCCM= $model->normalizeNum($Dir_NumeroRCCM);
							 $lastNum    = intval(trim(substr($lastNumRCCM,10)));
                         } else {
						 ...
						 if( $totalRegistres ) {
							 for( $i =1; $i<= $lastNum; $i++ ) {
						          ...
						          if( file_exists( $srcPath."/PHYSIQUES/".$localiteCode."/".$checkedAnneeVal."/".$numRccmPhysique."/".$numRccmPhysique."-FR.pdf" ) ||
									  ...
									  $checkedDocuments["numero"]     = $numRccm = sprintf("BF%s%dA|B|M%04d", $localiteCode, $checkedAnneeVal, $i);
                                  ...
                                  /*if( $minNbPages && isset($checkedDocuments["personnel"]) && file_exists($checkedDocuments["personnel"]))	 {
									  try{
									       $logger          = new Logger('MyLogger');
                                           $pdfInfo         = XPDF\PdfInfo::create(array('pdfinfo.binaries'=> 'E:\webserver\www\binaries\Xpdf\pdfinfo.exe','pdfinfo.timeout' => 30,), $logger);
								  ...
								  if( empty( $invalidMsg ) ) {
									  ...
									  if( is_dir( $srcPath."/".$localiteCode."/".$checkedAnneeVal."/".$numRccmPhysique) ||
										  ...
										  //$resteAcollecter[$checkedAnneeVal]["total"] = $resteAcollecter[$checkedAnneeVal]["total"]+1;								  
										  $output .=" <tr><td>".$j."</td> <td> ".$checkedDocuments["numero"]."</td><td> ".$invalidMsg." </td> </tr> ";
									  }										  									  
								  }
							 }
							  ...
							  if( $totalACollecter > 0) {
								  ...
								  $output      .= "<tr class=\"alert alert-success\"><td align=\"center\" colspan=\"4\" style=\"text-align:center;\" class=\"alert alert-success siccess\"><strong>".sprintf('AUCUN DOCUMENT MANQUANT POUR L\'ANNEE %d : TOUT CE QUI A ETE COLLECTE, RETRAITE A ETE AUSSI INDEXE. ',$checkedAnneeVal)."</strong></td> </tr>";
							  }								  
						 } else {
							  $totalACollecter  = (isset($resteAcollecter[$checkedAnneeVal]["total"]))?$resteAcollecter[$checkedAnneeVal]["total"] : 0;
							  $output .= "<tr><td align=\"center\" colspan=\"4\" style=\"text-align:center;\"><strong>".sprintf('Année %d', $checkedAnneeVal )."</strong></td> </tr>";
			                  $output .= "<tr><td align=\"center\" colspan=\"4\" style=\"text-align:center;color:white;\"> AUCUN DOCUMENT INDEXE N'A ETE TROUVE POUR L'ANNEE ".$checkedAnneeVal." </td> </tr>";
						 }
						 //$totalReste   = $j;
				}
	...
	public function importsiguedataAction()
	{
		...
		if( $this->_request->isPost() ) {
			...
			if( $documentsUploadAdapter->isUploaded("registres") && empty($errorMessages )) {
				$documentsUploadAdapter->receive(   "registres");
				if( $documentsUploadAdapter->isReceived("registres") ) {
					$csvHeader      = array("NumeroRCCM","DateRCCM","AncienRCCM","NomCommercial","SecteurArticle","Nom","Prenom","DateNaissance","LieuNaissance","Sexe","Telephone","Adresse","NumeroPiece","LibelleTypePiece");
					$csvAdapter     = Sirah_Filesystem_File::fabric("Csv",array("filename"=>$csvStoreFilePath,"has_header"=> true,"header"=>$csvHeader), "rb");
					$csvLines       = $csvAdapter->getLines();
					$csvItems       = 1;
					if(isset($csvLines[0])) {
					...
					//print_r($csvRows);die();
					 if( count(  $csvRows ) ) {
						 foreach($csvRows as $csvKey => $csvRow ) {
								 ...
								 $Description        = (isset($csvRow["SecteurArticle"] ))?$stringFilter->filter($csvRow["SecteurArticle"] )     : "";
		...
		if( $this->_request->isPost() )  {
            ...
            if( count( $errorMessages ) ) {
			   ...
			   foreach( $errorMessages as $message ) {
				...
				$output        .= "<table border='1' width='100%' cellspacing='2' cellpadding='2'> 
										<thead><tr><th width='5%'> N° d'ordre </th><th width='10%'> Localités </th><th width='5%'> Années </th><th width='10%'> Numéros RCCM </th><th width='5%'> N° IFU </th><th width='5%'> N° CNSS </th><th width='25%'> Promoteurs</th> <th width='40%'> Secteurs d'articles </th></tr> </thead>
										<tbody>";
				$i              = 1;						
				if( count($missings) ) {
	...
	public function historyAction()
	{
		...
		if( $this->_request->isPost() ) {
			...
			if( empty($errorMessages) ) {
				...
				$output        .= "<table border='1' width='100%' cellspacing='2' cellpadding='2'> 
										<thead><tr><th width='5%'> N° d'ordre </th><th width='20%'> Localités </th><th width='15%'> Années </th><th width='15%'> Numéros </th><th width='30%'> Promoteurs</th> <th width='30%'> Secteurs d'articles </th></tr> </thead>
										<tbody>";
				$i              = 1;						
				if( count($registres) ) {
					foreach( $registres as $registre ) {
