...
class Admin_RegistresuretesController extends Sirah_Controller_Default
{
	...
	{		
		...
		if( empty($errorMessages) && $dbSourceAdapter) {
			...
			$selectRegistres          = $dbSourceAdapter->select()->from( array("R" => $tablePrefix."rccm_registre" ))
																  ...
																  ->join( array("S" => $tablePrefix."rccm_registre_suretes")      ,"S.registreid=R.registreid"          , array("S.titre","S.nom_constituant","S.numrccm_constituant","S.estate","S.valeur","typeSurete"=>"S.type","S.periodstart","S.periodend"))
	...
	public function createAction()
	{
		...
		if( $this->_request->isPost() ) {
            ...
            if(!$modelSurete->checkNum($registre_data["numero"])) {
				$errorMessages[]                = sprintf("La sûrété numéro %s que vous avez indiqué ne semble pas correct. Il doit comporter 14 ou 18 caractères", $registre_data["numero"] );
			}
			...
			$registre_data["description"]       = (isset($postData["description"]      ))? $stringFilter->filter($postData["description"]) : (($parent)?$parent->description : "");
			...
			$registre_data["capital_numeraire"] = (isset($postData["capital_numeraire"]))? floatval($postData["capital_numeraire"])        : (($parent)?$parent->capital_numeraire : 0);
				...
				if( $dbAdapter->insert( $prefixName ."rccm_registre", $registre_data) ) {
					...
					if( $dbAdapter->insert( $prefixName . "rccm_registre_suretes", $surete_data )) {
						if( !$representantid ) {
					        ...
					        $dateNaissanceDay                     = (isset($postData["date_naissance_day"]) && ($postData["date_naissance_day"] != "00" ))? $stringFilter->filter($postData["date_naissance_day"]) : "00";
					  
					        if(!intval($dateNaissanceYear) || !intval($dateNaissanceMonth) || !intval($dateNaissanceDay)) {
								$representant_data["datenaissance"]   = "0000-00-00 00:00:00";
							} else {
							...
							$representant_data["cnib"]            = $stringFilter->filter( $representant_data["passport"] );
	...
	public function editAction()
	{
		...
		if( $this->_request->isPost()) {
			...
			$update_surete_data["nom_constituant"]     = (isset( $postData["nom_constituant"]    )) ? $stringFilter->filter( $postData["nom_constituant"] ) : "";
			$update_surete_data["numrccm_constituant"] = (isset( $postData["numrccm_constituant"])) ? $stringFilter->filter( $postData["numrccm_constituant"]) : "";
