...
{	
	...
	public function localitestatsAction()
	{
		...
		/*$defaultMaxNumByYears  = array("2000"=>"2031","2001"=>"2714","2002"=>"3144","2003"=>"3423","2004"=>"3984","2005"=>"4049","2006"=>"3591","2007"=>"4005","2008"=>"3702",
		                               "2009"=>"4900","2010"=>"4394","2011"=> "4794","2012"=>"5776","2013"=> "6480","2014"=> "7115","2015"=>"6613","2016"=> "9230","2017"=>"10000");*/
        
		$defaultMaxNumByYears  = array("2000"=>"1500","2001"=>"1500","2002"=>"1500","2003"=>"1500","2004"=>"1500","2005"=>"1500","2006"=>"1500","2007"=>"1500","2008"=>"1500",
		                               "2009"=>"1500","2010"=>"1500","2011"=> "1500","2012"=>"1500","2013"=>"1500","2014"=> "1500","2015"=>"1500","2016"=> "1500");									   
		...
		if( $this->_request->isPost() ) {
			...
			if( empty( $errorMessages )) {
				...
				$client->setHeaders(array( "Host"              => "10.1.25.13",
										   ...
										   "Cookie"            => "amfid=1884581252; JSESSIONID=012617A2BB9B335EBA0246AA688EC521; docubase_password_cookie=11:hamed.banaoBHME@31",
							 ...
							 if( count($missingRegistres)) {
								 ...
								 if(!$dbAdapter->update( $tablePrefix. "rheaweb_registres_missings", $missingData, array("registreid=".$missingregistreid))) {
								    $errorMessages[]             = sprintf("Les informations du registre numéro %s n'ont pas pu être mises à jour", $cleanNumRccm);
                                     ...
                                     $dbAdapter->delete( $tablePrefix. "rheaweb_registres_missings",array("numkey='".$rheawebNumKey."'"));								 
														  ...
														  if(   $browseJsonData["result"]["foundDoc"] == true ) {
																...
																if( !empty($viewReturnString) ) {
																	  ...
																	  if( isset($viewReturnJsonData["uniqueIdentifier"]) && isset($viewReturnJsonData["filename"])) {
																		  ...
																		  if( false !==stripos($rcFileTitle, "PIECES JOINTES") ) {
																			  $documentName  = $numRccm."-PS.pdf";
																			  $documentType  = "PiecesJointes";
																		  }
																		  if( false !== stripos($rcFileTitle, "ACTES CONSTITUTIFS")) {
																			  $documentName  = $numRccm."-ST.pdf";
																			  $documentType  = "ActesConstitutifs";
																		  }
																		  ...
																		  if( file_exists($newRccmFilename) && (($documentType=="Formulaire") || ($documentType=="ActesConstitutifs") )) {
																			  @unlink($newRccmFilename);
																			  if( true==copy($browseResponse->getStreamName(), $newRccmFilename)) {
																			      $copied[]       = $newRccmFilename;
																		      }
																		  } else {
																			  $newRccmTMPFilename = $newRccmFilename = $destPath.DS.$localiteCode.DS. $annee.DS.$numRccm.DS.$numRccm."TMP-PS.pdf";														  
																		      if(true==copy($browseResponse->getStreamName(), $newRccmTMPFilename)) {
																				  $copied[]       = $newRccmTMPFilename;
																			  }
																		  }
																		  $newRccmPSFilename      = $destPath.DS.$localiteCode.DS. $annee.DS.$numRccm.DS.$numRccm."-PS.pdf";	
																		  if( file_exists( $newRccmPSFilename )) {
																			  try {
																				 $combinedFiles   = array($newRccmPSFilename, $newRccmFilename );
					...
					if( isset($csvRows[0]) ) {
					 ...
					 if( count(  $csvRows ) ) {
						 foreach($csvRows as $csvKey => $csvRow ) {
								 ...
								 $sigueData                              = array("numero"=>$cleanNumRccm,"nom_commercial"=>$NomCommercial,"nom"=>$Nom,"prenom"=>$Prenom,"date_enregistrement"=>$DateDemande,"telephone"=>$Telephone,
	...
	public function importsiguefilesAction()
	{
		...
		$annees                     = array("2000"=>"2000","2001"=>"2001","2002"=>"2002","2003"=>"2003","2004"=>"2004","2005"=>"2005","2006"=>"2006","2007"=>"2007","2008"=>"2008",
		                                    "2009"=>"2009","2010"=>"2010","2011"=>"2011","2012"=>"2012","2013"=>"2013","2014"=>"2014","2015"=>"2015","2016"=>"2016","2017"=>"2017");
		$months                     = array(1=>"Janvier",2=>"Fevrier",3=>"Mars",4=>"Avril",5=>"Mai",6=>"Juin",7=>"Juillet",8=>"Aout",9=>"Septembre",10=>"Octobre",11=>"Novembre",12=>"Decembre");
		$defaultInitData            = array("srcpath"  =>"F:\\FNRCCM2017-2018\\SIGUE\\ORIGINAL","destpath"=>"F:\\FNRCCM2017-2018\\SIGUE\\A_RETRAITER","annee"=>2016,
		                                    "checkpath"=>"G:\\ERCCM2","incoherencespath"=>"G:\\ERCCM_INCOHERENCES","overwrite"=>"COMBINE","sigueuser"=>"compte.OPS",
		...
		if( $this->_request->isPost() ) {
			...
			$incoherencesPath       = (isset($postData["incoherencespath"]))? $postData["incoherencespath"]      : "C:\\FNRCCM2017-2018";
			...
			if( $documentsUploadAdapter->isUploaded("registres") && empty( $errorMessages )) {
				...
				if( $documentsUploadAdapter->isReceived("registres") ) {
                    ...
                    //print_r($csvLines); die();					
					@uasort(     $csvLines,function($a, $b){if($a['DateDemande']==$b['DateDemande']){return 0;} return ($a['DateDemande'] < $b['DateDemande']) ? -1 : 1; });
				    if( count(   $csvLines ) ) {
						foreach( $csvLines as $csvKey => $csvRow ) {
                                 ...
                                 if( empty( $idTypePIECE ) ) {
									 $idTypePIECE      = $numeroPIECE;
								 }									 
								 if(strlen($numeroRCCM) < 10 ) {
									$errorMessages[]   = sprintf("Le numéro RCCM %s de SIGUE est invalide", $numeroRCCM );
									continue;
								 ...
								 $rccmDayValue         =          ($zendDate) ? $zendDate->toString("dd")               : "";
                                 $documentName         = $cleanNumRccm."-FR.pdf";
                                 $rccmTmpDocument      = $rccmDocumentPathRoot     . DS . $numYear     . DS . $idPIECE.".pdf";
                                 $psDocumentFilename   = $destPath . DS . $numYear . DS . $cleanNumRccm. DS . $cleanNumRccm ."-PS.pdf";								 
								 
								 if(!intval($rccmYear) || !$zendDate || !intval($rccmMonthValue) || !intval($rccmDayValue)) {
									 $errorMessages[]  = sprintf("La date du numéro RCCM %s est invalide : %s", $numeroRCCM, $dateDemande );
									 continue;
								 }
								 if(strlen($cleanNumRccm) !=14 ) {
									 $errorMessages[]  = sprintf("Le numéro RCCM %s est invalide.", $cleanNumRccm );
									 continue;
								 }
                                 /*if( is_dir( $checkPath )) {
									 $checkFilePS      = $checkPath . DS . $rccmYear . DS . $cleanNumRccm . DS . $cleanNumRccm."-FR.pdf";
									 $checkedFilePs    = "F:\\ERCCM/OUA/". $rccmYear . DS . $cleanNumRccm . DS . $cleanNumRccm."-FR.pdf";
									 if( file_exists( $checkFilePS ) || file_exists($checkedFilePs)) {
										 continue;
									 }
								 }*/									 
                                 $documentsPath        = $destPath . DS . DS . $numYear . DS . $cleanNumRccm;
								 $documentFilename     = $documentsPath . DS . $documentName;
								 ...
								 if( (false!== stripos($formulaireName,"P2")) || (false!== stripos($formulaireName,"RCCM")) || (false!== stripos($formulaireName, "P1")) || (false!== stripos($formulaireName,"formulaire-rccm")) || (false!==stripos($formulaireName, "M1")) || (false!== stripos($formulaireName, "M0"))){
									 $documentName     = $cleanNumRccm."-FR.pdf";
									 $documentFilename = $documentsPath . DS . $documentName;
									 $doctype          = "Formulaire";
								 ...
								 $localDataFolder1     = $checkPath           . $numYear . DS . $cleanNumRccm;
								 $localDataFolder2     = "C:\\ERCCM/DATA/OUA/". $numYear . DS . $cleanNumRccm;
								 $localDataFolder3     = "F:\\ERCCM/OUA/"     . $numYear . DS . $cleanNumRccm;
								 $localDataFolder4     = "F:\\ERCCM2/OUA/"    . $numYear . DS . $cleanNumRccm;
                                 if( file_exists( $localDataFolder1.DS.$documentName) || file_exists( $localDataFolder2.DS.$documentName) || file_exists( $localDataFolder3.DS.$documentName) || file_exists( $localDataFolder4.DS.$documentName)) {
								    continue;
								 }									 
                                 $rccmDocumentPath     = $rccmDocumentPathRoot. DS . $numYear;								 
								 $rccmDocument         = $rccmDocumentPath    . DS . $cleanNumRccm.".pdf";
								 
								 if(!is_dir( $rccmDocumentPath) ) {
									 if(!is_dir( $rccmDocumentPathRoot . DS . $numYear)) {
										 @chmod( $rccmDocumentPathRoot , 0777);
										 @mkdir( $rccmDocumentPathRoot . DS . $numYear);
									 }									 
								 }
								 $sigueUri             = preg_replace ("/^ /", "", $sigueUri);
								 $sigueUri             = preg_replace ("/ $/", "", $sigueUri);
								...
								//Set the post parameters
								 ...
								 if( file_exists($rccmTmpDocument ) && filesize($rccmTmpDocument)) {
									 if(!is_dir( $destPath . DS . $numYear ) ) {
										 @chmod( $destPath , 0777);
										 @mkdir( $destPath . DS . $numYear);
									 }
									 if(!is_dir( $destPath . DS . $numYear. DS . $cleanNumRccm ) ) {
										 @mkdir( $destPath . DS . $numYear. DS . $cleanNumRccm);
									 }									 
	...
	{		
		...
		$annees                   = array("2000"=>"2000","2001"=>"2001","2002"=>"2002","2003"=>"2003","2004"=>"2004","2005"=>"2005","2006"=>"2006","2007"=>"2007","2008"=>"2008",
		                                  "2009"=>"2009","2010"=>"2010","2011"=>"2011","2012"=>"2012","2013"=>"2013","2014"=>"2014","2015"=>"2015","2016"=>"2016","2017"=>"2017");
		$defaultInitData          = array("srcpath"=>"C:\\ERCCM\\DATA","destpath"=>"C:\\Users\User\\Desktop\\FNRCCM-2017-2018\\INDEXATIONS","localite"=>"OUA","annee"=>2000,"nbre_documents"=>1000,"sigar_dbhost" => "");
			...
			if( empty(   $errorMessages ) ) {				
				foreach( $dataItems     as $rccmDirectory ) {
						 ...
						 if(!empty( $newDirRccmNum ) ) {
							 ...
							 $directoryFiles = glob( $rccmDirectory."/*");
							 if( count(   $directoryFiles ) ) {
								 foreach( $directoryFiles as $directoryFile ) {
									      if( is_dir( $directoryFile ) ) {
											  @rmdir( $directoryFile );
										  } elseif( is_file( $directoryFile ) ) {
	...
	public function saveusersdataAction()
	{
		...
		$defaultData        = array("annee"=>2016,"localite"=>"OUA","doublonspath"=>"F:\\ERCCM_DOUBLONS","srcpath"=>"F:\\FNRCCM2017-2018\\OPS","finalpath"=>"F:\\ERCCM", "destpath"=>"F:\\ERCCM2","ops_username"=>"");
		$errorMessages      = array();
		$successMessages    = array();
		$notSavedItems      = array();
		$opsName            = "";
	
		$savedItems         = $invalidItems = $errorItems = $dataItems = array();
	
	    $opsUsernames       = array(0=> "Sélectionnez un OPS","ouedraogo_alida"=>"OUEDRAOGO ALIDA","sogbo_aurele"=>"SOGBO AURELE","traore_korotimi"=>"TRAORE KOROTIMI","dayamba_raissa"=>"DAYAMBA RAISSA","sangare_alimata"=>"SANGARE ALIMATA","traore_sandrine"=>"TRAORE SANDRINE");
		$localites          = $modelLocalite->getSelectListe(null, array("code", "libelle") , array() , null , null , false );
		$annees             = array("2000"=>"2000","2001"=>"2001","2002"=>"2002","2003"=>"2003","2004"=>"2004","2005"=>"2005","2006"=>"2006","2007"=>"2007","2008"=>"2008",
		                            "2009"=>"2009","2010"=>"2010","2011"=>"2011","2012"=>"2012","2013"=>"2013","2014"=>"2014","2015"=>"2015","2016"=>"2016","2017"=>"2017");
		if( $this->_request->isPost( )) {
			...
			$srcPath              = ( isset( $postData["srcpath"]        )) ? $postData["srcpath"]               : $defaultData["srcpath"];
			$destPath             = ( isset( $postData["destpath"]       )) ? $postData["destpath"]              : $defaultData["destpath"];
			$finalPath            = ( isset( $postData["finalpath"]      )) ? $postData["finalpath"]             : $defaultData["finalpath"];
			$doublonsPath         = ( isset( $postData["doublonspath"]   )) ? $postData["doublonspath"]          : $defaultData["doublonspath"];
			$localite             = ( isset( $postData["localite"]       )) ? $postData["localite"]              : $defaultData["localite"];
			$annee                = ( isset( $postData["annee"]          )) ? intval($postData["annee"])         : $defaultData["annee"];
			$opsUsername          = ( isset( $postData["ops_username"]   )) ? $postData["ops_username"]          : $defaultData["ops_username"];
			$opsName              = ( isset( $opsUsernames[$opsUsername] )) ? $opsUsernames[$opsUsername]        : "";
			...
			if(!is_dir($finalPath ) ) {
				$errorMessages[]  = sprintf("Le dossier final %s n'a pas été trouvé", $finalPath);
			}
			if(!count( $dataItems ) ) {
				$errorMessages[]  = sprintf( "Le dossier de l'utilisateur %s  pour l'année %d ne contient aucun document. Veuillez vérifier.",  $opsName, $annee );
			}
			if( empty(   $errorMessages ) ) {				
				foreach( $dataItems    as $rccmDirectory ) {
					     $dirRccmNum    = Sirah_Filesystem::mb_basename($rccmDirectory);
						 $rccmLocalite  = trim(substr( $dirRccmNum, 2, 3));
						 $rccmAnnee     = trim(substr( $dirRccmNum, 5, 4));
						 $rccmTypeCode  = trim(substr( $dirRccmNum, 9, 1));
						 $rccmId        = trim(substr( $dirRccmNum, 10));
						 
						 if(!isset( $localites[$rccmLocalite] ) ) {
							 $errorMessages[]  = $message = sprintf("La localité code numéro %s semble invalide dans le dossier %s", $rccmLocalite, $dirRccmNum);
							 $errorItems[$dirRccmNum]     = $message;
							 continue;
						 }
						 if(!isset( $annees[$rccmAnnee] ) ) {
							 $errorMessages[]  = $message = sprintf("L'année %s semble invalide dans le dossier %s",$rccmAnnee, $dirRccmNum);
							 $errorItems[$dirRccmNum]     = $message;
							 continue;
						 }
						 ...
						 $statuteFile                     = $rccmDirectory . DS . $dirRccmNum."-ST.pdf";
						 $completeFile                    = $rccmDirectory . DS . $dirRccmNum."-PS.pdf";
						 $finalRccmPath                   = $finalPath     . DS . $rccmLocalite . DS . $rccmAnnee . DS . $dirRccmNum;
						 $doublonsRccmPath                = $doublonsPath  . DS . $rccmLocalite . DS . $rccmAnnee . DS . $dirRccmNum;
						 if( is_dir( $finalRccmPath) && !is_dir( $doublonsRccmPath ) ) {
							 if( is_dir(     $doublonsPath ) ) {
								 if(!is_dir( $doublonsPath . DS . $rccmLocalite) ) {
									 @chmod( $doublonsPath, 0777);
									 @mkdir( $doublonsPath . DS . $rccmLocalite );
									 @chmod( $doublonsPath . DS . $rccmLocalite, 0777);
								 }
								 if(!is_dir( $doublonsPath . DS . $rccmLocalite . DS . $rccmAnnee) ) {									 
									 @mkdir( $doublonsPath . DS . $rccmLocalite . DS . $rccmAnnee );
									 @chmod( $doublonsPath . DS . $rccmLocalite . DS . $rccmAnnee, 0777);
								 }
								     @mkdir( $doublonsPath . DS . $rccmLocalite . DS . $rccmAnnee    . DS . $dirRccmNum);
								 if( file_exists( $formulaireFile) ) {
									 $doublonFormulaireFile   =   $doublonsPath . DS . $rccmLocalite . DS . $rccmAnnee . DS . $dirRccmNum . DS . $dirRccmNum."-FR.pdf";
									 copy( $formulaireFile, $doublonFormulaireFile);
								 }
                                 if( file_exists($statuteFile) ) {
									 $doublonStatuteFile      = $doublonsPath  . DS . $rccmLocalite . DS . $rccmAnnee . DS . $dirRccmNum . DS . $dirRccmNum."-ST.pdf";
									 copy($statuteFile, $doublonFormulaireFile);
								 }
                                 if( file_exists($completeFile) ) {
									 $doublonCompleteFile     = $doublonsPath  . DS . $rccmLocalite . DS . $rccmAnnee . DS . $dirRccmNum . DS . $dirRccmNum."-PS.pdf";
									 copy($completeFile, $doublonCompleteFile);
								 }								 
							 }
							 continue;
						 }
						 ...
						 if(is_dir(  $rccmSavingPath) ) {
							 ...
							 if( $overwriteOption=== "ERASE" ) {
								 if( file_exists( $savedFormulaireFile )) {
									 unlink(      $savedFormulaireFile );
								 }
								 if( file_exists( $savedStatuteFile ) ) {
									 unlink(      $savedStatuteFile );
								 }
								 if( file_exists( $savedCompleteFile ) ) {
									 unlink(      $savedCompleteFile );
								 }
							 }							 
						 ...
						 $savedItems[]  =  $dirRccmNum;
				}
			}			
			$validHtml           =" <table cellspacing=\"2\" cellpadding=\"2\" width=\"100%\" border=\"1\" align=\"center\" style=\"text-align:center;\" >";
			          $validHtml.="    <tr><td width=\"100%\" style=\"font-size:13pt;text-align:center;\"  align=\"center\"><b> ". sprintf("%d dossiers valide de l'utilisateur %s pour l'année %d ont été sauvegardés avec succès ", count($validItems) , $opsName, $annee) ." </b></td></tr>";
            $validHtml          .=" </table>";			
            if( count( $errorMessages )) {
				$errorHtml       =" <table cellspacing=\"2\" cellpadding=\"2\" width=\"100%\" border=\"1\" align=\"center\" style=\"text-align:center;\" >";
                      $errorHtml.="<tr><td width=\"100%\" style=\"font-size:13pt; text-align:center;background-color:#E5E5E5\" align=\"center\"><b> ".sprintf('HISTORIQUE DES ERREURS PRODUITES PAR %s : AU TOTAL %d ERREURS ', $opsName, count( $errorMessages ))." </b></td></tr>";
            ...
            $PDF                 = new Sirah_Pdf_Default("L", "mm", "A3", true , "UTF-8");
            $PDF->SetCreator(sprintf("%s", $opsUsername));
            $PDF->SetTitle(  sprintf("Sauvegarde des retraitements effectués par %s", $opsName));
			$PDF->SetPrintHeader(false);
		    $PDF->SetPrintFooter(false);
	...
	public  function searchsiguemissingsAction()
	{
		@ini_set('memory_limit', '512M');
		require_once("tcpdf/tcpdf.php");
		require_once("Fpdi/fpdi.php");
		...
		$annees                   = array("2000"=>"2000","2001"=>"2001","2002"=>"2002","2003"=>"2003","2004"=>"2004","2005"=>"2005","2006"=>"2006","2007"=>"2007","2008"=>"2008",
		                                  "2009"=>"2009","2010"=>"2010","2011"=> "2011","2012"=>"2012","2013"=> "2013", "2014"=> "2014","2015"=> "2015", "2016"=> "2016", "2017" => "2017");		 							   
		$defaultData              = array("annee" =>2016,"localite"=>"OUA","folderstocheck"=>"F:\\ERCCM");
		$foundRegistres           = $errorMessages = $dataFolders = array();
		...
		if ( $this->_request->isPost() ) {
			...
			$foldersToCheck       = (isset($postData["folderstocheck"]))? $postData["folderstocheck"] : "";
			$annee                = (isset($postData["annee"]         ))? $postData["annee"]          : 0;
			$localiteCode         = (isset($postData["localite"]      ))? $postData["localite"]       : $defaultData["localite"];
			$localiteid           = (isset($localiteIDS[$localiteCode]))?$localiteIDS[$localiteCode] : 0;
			//On crée les filtres qui seront utilisés sur les paramètres de recherche
			...
			if(empty($foldersToCheck)) {
				$errorMessages[]  = "Veuillez renseigner les dossiers à vérifier";
			} else {
				$foldersArray     = preg_split("/;|,/", $foldersToCheck );
				if( count(   $foldersArray ) ) {
					foreach( $foldersArray as $folderToCheck ) {
						     if( is_dir( $folderToCheck ) ) {
								 $folders[]   = $folderToCheck;
							 }
					}
				}
			}
			if(!count( $folders ) ) {
				$errorMessages[]    = "Les dossiers à vérifier ne sont pas valides";
			}
			$documentsUploadAdapter = new Zend_File_Transfer();
			$documentsUploadAdapter->addValidator('Count'    , false , 1);
			$documentsUploadAdapter->addValidator("Extension", false , array("csv", "xls", "xlxs"));
					...
					if( isset($csvLines[0]) ) {
					 ...
					 if( count(  $csvRows ) ) {
						foreach( $csvRows as $csvKey => $csvRow ) {
								 ...
								 $found                = false;
								 $sigueRccmData        = array("numero"  =>$numeroRCCM,"numkey"=>$numKey,"cleanum"=>$cleanNumRccm,"localiteid"=>$localiteid,"valid"=>1,"found"=>0,"date"=>$rccmDate,
                                                               "localite"=>$numLocalite,"nomcommercial"=>$nomCommercial,"annee"=>$numYear,"datedemande"=>$dateDemande);
								 $rccmFormulaire       = "F:\\ERCCM\OUA". DS . $numYear . DS . $cleanNumRccm. DS . $cleanNumRccm."-FR.pdf";
								 $rccmComplete         = "F:\\ERCCM\OUA". DS . $numYear . DS . $cleanNumRccm. DS . $cleanNumRccm."-PS.pdf";
								 //print_r($rccmFormulaire);print_r(file_exists( $rccmFormulaire ) );die();
								 if( file_exists( $rccmFormulaire ) && file_exists( $rccmComplete )) {
									  continue;
								 }
								 $sigueRccm            = $model->findsiguerc($numeroRCCM);
								 					 								 								 								 								 							 								 								 								 
								 if( $sigueRccm) {
									 //print_r($sigueRccm); die();
									 $registreid                     = $sigueRccm->registreid;
									 $sigueRccmData["updateduserid"] = $me->userid;
									 $sigueRccmData["updatedate"]    = time();
									 if(!$dbAdapter->update( $prefixName . "sigue_registre", $sigueRccmData, array("numero='".$numeroRCCM."'"))) {
                                 ...
                                 $sigueDocumentData                  = array("documentid"=>0,"registreid"=>$registreid,"numrccm"=>$cleanNumRccm,"documentkey"=>$idPIECE,"numdoctype"=>$numeroPIECE,"doctype"=>$doctype,"documentname"=>$documentName);								 
                                     $dbAdapter->delete( $prefixName . "sigue_registre_missings_documents", array("documentkey='".$idPIECE."'") );
								 if( $dbAdapter->insert( $prefixName . "sigue_registre_missings_documents", $sigueDocumentData ) ) {
									 $foundRegistres[$registreid]    = $sigueRccmData;
								 } else {
									 $errorMessages[]  = sprintf("Les informations du document manquant %s n'ont pas pu être enregistrées", $idPIECE);
								 }									 
					    }
					 }
				}
			...
			if( count( $errorMessages ) ) {
			   ...
			   foreach( $errorMessages as $message ) {
				...
				$successMessage     = sprintf("L'opération s'est effectuée avec succès : nous avons détecté %d registres de commerce manquants", count($foundRegistres));
				if( $this->_request->isXmlHttpRequest()) {
				    $this->_helper->viewRenderer->setNoRender(true);
				    echo ZendX_JQuery::encodeJson(array("success" => $successMessage ));
				    exit;
				...
				$this->setRedirect($successMessage, "success");
				$this->redirect("admin/registres/downloadsiguemissings/annee/".$annee."/localiteid/".$localiteid); 
			}
		}
		...
		$this->view->localites    =  $localites;
		$this->view->annees       =  $annees;
		$this->render("siguemissings");
	}
	
	public function downloadsiguemissingsAction()
	{
		@ini_set('memory_limit', '512M');
		require_once("tcpdf/tcpdf.php");
		require_once("Fpdi/fpdi.php");
		...
		$annees                   = array("2000"=>"2000","2001"=>"2001","2002"=>"2002","2003"=>"2003","2004"=>"2004","2005"=>"2005","2006"=>"2006","2007"=>"2007","2008"=>"2008",
		                                  "2009"=>"2009","2010"=>"2010","2011"=> "2011","2012"=>"2012","2013"=> "2013", "2014"=> "2014","2015"=> "2015", "2016"=> "2016", "2017" => "2017");		 							   
		$defaultData              = array("annee"=>2016,"localite"=>"OUA","destpath"=>"G:\\FNRCCM/SIGUE","sigueuser"=>"compte.OPS",
		                                  "sigueuri"=>"http://10.60.16.17:8014/Piece/ShowFile/","overwrite"=>"COMBINE","siguepwd"=>"P@ssw0rd","sigueuauth_type"=>CURLAUTH_NTLM);
		$totalDocuments           = 0;								  
		...
		if ($this->_request->isPost() ) {
			...
			$localiteCode         = (isset($postData["localite"]        ))? $postData["localite"]       : $defaultData["localite"];
			$localiteid           = (isset($localiteIDS[$localiteCode]  ))? $localiteIDS[$localiteCode] : 0;
			$destinationPath      = (isset($postData["destpath"]        ))? $postData["destpath"]       : "";
			$sigueUri             = (isset($postData["sigueuri"]        ))? $postData["sigueuri"]       : "http://10.60.16.17:8014/Piece/ShowFile/";
			$sigueAuthType        = (isset($postData["sigueuauth_type"] ))? $postData["sigueuauth_type"]: CURLAUTH_NTLM;
			$sigueUsername        = (isset($postData["sigueuser"]       ))? $postData["sigueuser"]      : "compte.OPS";
			$siguePassword        = (isset($postData["siguepwd"]        ))? $postData["siguepwd"]       : "P@ssw0rd";
			...
			if(!is_dir( $destinationPath ) ) {
				$errorMessages[]  = "Le dossier de destination n'est pas valide";
			}
			if( empty( $errorMessages) ) {
				$filters          = array("annee"=> $annee,"localiteid"=>$localiteid,"found"=>0);
				$sigueMissings    = $model->findsiguercmms($filters);
				if( count(   $sigueMissings ) ) {
					foreach( $sigueMissings   as $sigueMissing ) {
						     $registreid       = $sigueMissing["registreid"];
						     $numeroRCCM       = $sigueMissing["numero"];
							 $numYear          = $sigueMissing["annee"];
							 $numLocalite      = $sigueMissing["localite"];
							 $cleanNumeroRCCM  = $sigueMissing["cleanum"];
							 $sigueDocuments   = $model->findsiguemissingsdocs($registreid);
							 
							 if(!is_dir( $destinationPath . DS . $numLocalite ) ) {
								 @chmod( $destinationPath, 0777);
								 @mkdir( $destinationPath . DS . $numLocalite );
								 @chmod( $destinationPath . DS . $numLocalite, 0777);
							 }
                             if(!is_dir( $destinationPath . DS . $numLocalite . DS . $numYear) ) {
								 @mkdir( $destinationPath . DS . $numLocalite . DS . $numYear);
								 @chmod( $destinationPath . DS . $numLocalite . DS . $numYear, 0777);
							 }	
                             //print_r($sigueDocuments); die();							 
							 if( count(   $sigueDocuments  ) ) {
								 foreach( $sigueDocuments     as $sigueDocument ) {
									      $documentTransfered = false;
									      $idPIECE            =  $sigueDocument["documentkey"];
										  $doctype            =  $sigueDocument["doctype"];
										  $documentName       =  $sigueDocument["documentname"];
										  $rccmDocument       =  $destinationPath . DS .  "ORIGINAL/".$idPIECE.".pdf";
										  $documentFilename   =  $destinationPath . DS . $numLocalite . DS . $numYear . DS . $cleanNumeroRCCM . DS . $documentName;
                                          if(((false!==stripos($documentName, "FR" )) || (false!==stripos($documentName, "ST" ))) && file_exists($documentFilename) ) {
											  @unlink($documentFilename);
										  }	
                                          if(file_exists( $rccmDocument )) {
											  continue;
										  }											  
										  if(!is_dir( $destinationPath . DS . $numLocalite . DS . $numYear . DS . $cleanNumeroRCCM ) ) {
											  @mkdir( $destinationPath . DS . $numLocalite . DS . $numYear . DS . $cleanNumeroRCCM);
											  @chmod( $destinationPath . DS . $numLocalite . DS . $numYear . DS . $cleanNumeroRCCM, 0777);
										  }											  
										  $sigueUri           = preg_replace ("/^ /", "", $sigueUri);
										  $sigueUri           = preg_replace ("/ $/", "", $sigueUri);
										  $url                = trim($sigueUri,"/")."/".$idPIECE;									 
										  ...
										  $sigueDocumentFile = curl_exec($ch);
										  file_put_contents($rccmDocument, $sigueDocumentFile);
										  if( file_exists(  $rccmDocument ) && ($documentFileSize=filesize($rccmDocument) )) {
											  if((false!==stripos($documentName, "FR" )) || (false!==stripos($documentName, "ST" ))) {
												  $documentTransfered = copy( $rccmDocument, $documentFilename );
											  }
                                              $completeDocument       =  $destinationPath . DS . $numLocalite . DS . $numYear . DS . $cleanNumeroRCCM . DS . $cleanNumeroRCCM."-PS.pdf";
                                              if(!file_exists( $completeDocument ) ) {
												  $documentTransfered = copy( $rccmDocument, $completeDocument);
											  } else {
												  try{
													  $combinedFiles      = array($rccmDocument, $completeDocument);
													  $combinedFilePDF    = new Fpdi\Fpdi();
													  foreach($combinedFiles as $combinedFile ) {
															  if( file_exists($combinedFile)) {
																  $pageCount = $combinedFilePDF->setSourceFile($combinedFile);
													  ...
													  $combinedFilePDF->Output("F", $completeDocument);
													  $documentTransfered     = true;
												  } catch( Exception $e ) {
													  $errorMessages[]    = $e->getMessage();
													  $documentTransfered     = false;
                                              ...
                                              if( true === $documentTransfered ) {
											      $foundDocuments[$registreid][$idPIECE]  = $documentFilename;
												  $totalDocuments++;												   
											  }																						  
										  }
								 }
							 }
							 if( count($foundDocuments[$registreid]) ) {
								 $dbAdapter->update( $prefixName . "sigue_registre", array("found" =>1), array("registreid='".$registreid."'") );
							 }
					}
				}
			}
		...
		$this->view->localites    =  $localites;
		$this->view->annees       =  $annees;
		$this->render("downloadsiguemissings");
	}
