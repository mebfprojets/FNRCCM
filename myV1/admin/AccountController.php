...
class Admin_AccountController extends Sirah_Controller_Default
{
	...
	{		
		...
		$this->_helper->layout->setLayout("login")->setLayoutPath(APPLICATION_TEMPLATES .'/rccm');	
		...
		if( isset( $captchaSession->showcaptcha ) && ( true == $captchaSession->showcaptcha  )) {
			$imgDir               = realpath( DOCUMENTS_PATH  ."/../../myTpl/gestoptic/images/captchas/")	;
			$imgUrl               = ROOT_PATH ."/myTpl/gestoptic/images/captchas/";
			...
			if( $this->_helper->csrf->isValid() ) {				
				...
				if(count(    $errorMessages ) ) {									
					...
					if(!isset( $captchaSession->checksAuth    ) ) {
						...
						$captchaSession->showcaptcha= true;
						if( null==$captcha ) {
							$captcha = $this->_createCaptcha( 8, "trebucit.ttf", realpath(DOCUMENTS_PATH."/../../myTpl/gestoptic/images/captchas/"), (ROOT_PATH."/myTpl/gestoptic/images/captchas/"));
						}
					} 					
				}
				if( $authSuccess )  {
					$cacheManager   = Sirah_Fabric::getCachemanager();
					$successMsg     = sprintf("Bienvenue Mr/Mrs %s %s sur votre compte sur le logiciel SIRAAH.", $loggedInUser->lastname, $loggedInUser->firstname );
					...
					if ( Zend_Uri::check( $nextUri ) ) {
						...
						$defaultControllerName = "dashboard";
						$defaultActionName     = "list";
						if( $this->_request->isXmlHttpRequest() ) {
							$this->_helper->viewRenderer->setNoRender( true );
							$this->_helper->layout->disableLayout(     true );
							$this->getHelper("Message")->addMessage( $successMsg ,"success");
							echo ZendX_JQuery::encodeJson(array("reload"=> true, "newurl" => $this->view->url(array("controller" => $defaultControllerName,"action" => $defaultActionName,"module" => "admin"), "default")));
							exit;
						}
						$this->setRedirect( $successMsg , "success");
						$this->redirect(    "admin/".$defaultControllerName ."/". $defaultActionName );
					 }					
				   } 
				} else {
				   if(  $this->_request->isXmlHttpRequest( ) ) {
	...
	public function logoutAction()
	{
		...
		$loggedInUser->logout();
		$front                 = Zend_Controller_Front::getInstance();
		$defaultControllerName = $front->getDefaultControllerName();
		$defaultActionName     = $front->getDefaultAction();
		$this->redirect($defaultControllerName . "/" . $defaultActionName );	
	}
	...
	{		
		...
		if( $this->_request->isPost()){             			
			...
			if(!empty($errorMessages)){
				...
				$linkActivateWithToken= Sirah_Functions::url($this->view->url(array("controller" => "account", "module" => "admin", "action" => "activate")),"token=".$activateAccountToken."&uid=".$newuserid, 81 , APPLICATION_HOST);
				$linkSimpleActivate   = Sirah_Functions::url($this->view->url(array("controller" => "account", "module" => "admin", "action" => "activate")),"uid=".$newuserid, 81 , APPLICATION_HOST);
	...
	public function editAction()
	{
		...
		if( null== $doneUri ) {
			$doneUri = Sirah_Functions::url($this->view->url(array("controller" => "account", "module" => "admin", "action" => "settings")),"", 81 , APPLICATION_HOST);
		}		
		if( $this->_request->isPost() ) {			
			//Une astuce pour contourner une attaque par force brute, en utilisaant le jeton du formulaire
			if(!isset($defaultSession->token) || ($this->_getParam("t","") != $defaultSession->token) ){
				$defaultSession->token = Sirah_User_Helper::getToken(15).time();
				$urlDone          = Sirah_Functions::url($this->view->url(array("controller" => "account", "module" => "admin", "action" => "edit")),"t=".$defaultSession->token, 81 , APPLICATION_HOST);
			...
			if(!$strNotEmptyValidator->isValid($update_data["firstname"]) || ( !$nameValidator->isValid( $update["firstname"] ) ) ){
				$errorMessages[]         = " Votre prénom saisi est invalide ";
			} else {
				$update_data["firstname"]= $stringFilter->filter( $update_data["firstname"] );
			}			 
			if(!$strNotEmptyValidator->isValid($update_data["lastname"]) || ( !$nameValidator->isValid( $update["lastname"] ) ) ) {
	...
	public function reinitactivationAction()
	{
		...
		if( $this->_request->isPost() || $this->_hasParam("email")){			
			...
			if($row = $userTable->find(array("email" => $email))){				
				...
				$linkActivateWithToken   = Sirah_Functions::url($this->view->url(array("controller" => "account", "module" => "admin", "action"  => "activate")),"token=".$activateAccountToken."&uid=".$userid, 81 , APPLICATION_HOST);
				$linkSimpleActivate      = Sirah_Functions::url($this->view->url(array("controller" => "account", "module" => "admin", "action"  => "activate")),"uid=".$userid, 81 , APPLICATION_HOST);
			...
			if(($userActivationSession->code != $token)){
				...
				if($this->_request->isXmlHttpRequest()){
					echo ZendX_JQuery::encodeJson(array("error"     => "Impossible d'effectuer cette action, des paramètres supplementaires sont nécessaires",
							                            "reload"    => true,
							                            "reloadUrl" => $this->view->url(array("controller" => "account", "module" => "admin", "action" => "reinitactivation", "token" => $newToken ))));
					exit;
				}
