...
class Model_Registre extends Sirah_Model_Default
{
	...
	public function getList( $filters = array() , $pageNum = 0 , $pageSize = 0)
	{
		...
		$dbAdapter      = $table->getAdapter();
		$tablePrefix    = $table->info("namePrefix");
		$selectRegistre = $dbAdapter->select()->from(array("R"    => $tablePrefix ."rccm_registre" ), array("R.registreid", "R.numero", "R.libelle", "R.description","R.date","R.category", 
		                                                                                                    "date_registre" => "FROM_UNIXTIME(R.date,'%d/%m/%Y')","annee" => "FROM_UNIXTIME(R.date,'%Y')"))
