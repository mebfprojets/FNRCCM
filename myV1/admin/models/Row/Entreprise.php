...
class Model_Entreprise extends Sirah_Model_Default
{
    ...
    public function projects( $entrepriseid = 0, $filters = array() , $pageNum = 0 , $pageSize = 0)
	{
        ...
        $selectProject = $dbAdapter->select()->from(array("P" => $tablePrefix ."gestoptic_projet_application"))
	...
	public function documents( $entrepriseid = null )
	{
		...
		$selectDocument   = $dbAdapter->select()->from(array("D" => $tablePrefix."system_users_documents" ))
		                                        ->join(array("E" => $tablePrefix."gestoptic_clients_entreprises_documents"), "E.documentid = D.documentid", null )
    ...
    public function getList( $filters = array() , $pageNum = 0 , $pageSize = 0)
	{
		...
		$dbAdapter        = $table->getAdapter();
		$tablePrefix      = $table->info("namePrefix");
		$selectEntreprise = $dbAdapter->select()->from(array("E"     => $tablePrefix ."gestoptic_clients_entreprises" ))
		                                        ->joinLeft(array("D" => $tablePrefix ."system_domaines"), "D.domaineid = E.domaineid", array("domaine"=> "D.libelle"));
	...
	public function getListPaginator( $filters = array() )
	{
		...
		$dbAdapter     = $table->getAdapter();
		$tablePrefix   = $table->info("namePrefix");
		$selectEntreprise = $dbAdapter->select()->from(array("E"     => $tablePrefix ."gestoptic_clients_entreprises" ), array("E.domaineid"))
		                                        ->joinLeft(array("D" => $tablePrefix ."system_domaines"), "D.domaineid = E.domaineid", array("domaine"=> "D.libelle"));
