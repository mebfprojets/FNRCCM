...
class Model_Registremorale extends Sirah_Model_Default
{
	...
	public function getList( $filters = array() , $pageNum = 0 , $pageSize = 0)
	{
		...
		$selectRegistre = $dbAdapter->select()->from(array("R"    => $tablePrefix ."rccm_registre")            , array("R.registreid","R.numero","R.libelle","R.description","R.date","R.category","annee" => "FROM_UNIXTIME(R.date,'%Y')"))
		                                      ->join(array("RM"   => $tablePrefix ."rccm_registre_moral")      , "RM.registreid   = R.registreid",array("RM.entrepriseid"))
		                                      ->join(array("RE"   => $tablePrefix ."rccm_registre_entreprises"), "RE.entrepriseid = RM.entrepriseid", array("entreprise"=>"RE.libelle","RE.pagekey","RE.reference", "administrateur" => "RE.responsable"))
	   ...
	   if(isset($filters["searchQ"]) && !empty($filters["searchQ"]) && (null!==$filters["searchQ"])){
			...
			$selectRegistre->where("{$likeNumero} OR {$likeLibelle}");
		}
	...
	public function getListPaginator( $filters = array() )
	{
		...
		$selectRegistre = $dbAdapter->select()->from(array("R"    => $tablePrefix ."rccm_registre")            , array("R.registreid"))
		                                      ->join(array("RM"   => $tablePrefix ."rccm_registre_moral")      , "RM.registreid   = R.registreid", null )
		                                      ->join(array("RE"   => $tablePrefix ."rccm_registre_entreprises"), "RE.entrepriseid = RM.entrepriseid", null )
