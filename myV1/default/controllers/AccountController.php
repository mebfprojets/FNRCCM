				...
				if( $authSuccess )  {
					...
					if ( Zend_Uri::check( $nextUri ) ) {
						...
						if( $this->_request->isXmlHttpRequest() ) {
							...
							echo ZendX_JQuery::encodeJson(array("reload"=> true, "newurl" => $this->view->url(array("controller" => $defaultControllerName, "action" => $defaultActionName  ))));
							exit;
						}
		...
		if( $this->_request->isPost() || $this->_hasParam("email")){			
			...
			if($row = $userTable->find(array("email" => $email))){				
				...
				try{
					...
					$this->redirect("account/reinitactivation/token/".$token);
				}				
				$activateSession->setExpirationSeconds(86400);
	...
	public function activateAction()
	{
		...
		if( null!==$token  ) {
			...
			if(($userActivationSession->code != $token)){
				...
				$this->redirect("account/reinitactivation/token/".$newToken);
			}
