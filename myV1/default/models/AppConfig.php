	...
	public function preDispatch(Zend_Controller_Request_Abstract $request)
	{
		...
		if( isset($appConfigSession->project->projectid) && intval($appConfigSession->project->projectid)) {
			...
			if( $projectInstance ) {							
				...
				$projectData                                                   = array();
				if( count(   $projectValues ) ) {
				    foreach( $projectValues as $projectValueKey => $projectValue ) {
					         $projectData[$projectValueKey]                    = $projectValue ;
				    }
				    $_SESSION["AppConfig"]["project"]                          = $projectData;
				}
				if( count(   $entrepriseArray ) ) {
				    foreach( $entrepriseArray as $entrepriseArrayKey => $entrepriseArrayValue ) {
					         $entrepriseData[unserialize($entrepriseArrayKey)] = unserialize($entrepriseArrayValue);
				    }
				    $_SESSION["AppConfig"]["entreprise"]                       = (isset($entrepriseData['entrepriseid']) ) ? $entrepriseData : $entrepriseArray;
				}
				
				$_SESSION["AppConfig"]["params"]["apply_tva"]                  = (isset($paramsRow["apply_tva"]              ))? $paramsRow["apply_tva"]                : 1;
			    $_SESSION["AppConfig"]["params"]["val_tva"]                    = (isset($paramsRow["val_tva"]                ))? $paramsRow["val_tva"]                  : 18;
			    $_SESSION["AppConfig"]["params"]["apply_bic"]                  = (isset($paramsRow["apply_bic"]              ))? $paramsRow["apply_bic"]                : 0;
			    $_SESSION["AppConfig"]["params"]["val_bic"]                    = (isset($paramsRow["val_bic"]                ))? $paramsRow["val_bic"]                  : 0;
                $_SESSION["AppConfig"]["params"]["nb_elements_page"]           = (isset($paramsRow["nb_elements_page"]       ))? $paramsRow["nb_elements_page"]         : 25;
				$_SESSION["AppConfig"]["params"]["default_period_start"]       = (isset($paramsRow["default_period_start"]   ))? $paramsRow["default_period_start"]     : 0;
				$_SESSION["AppConfig"]["params"]["default_period_end"]         = (isset($paramsRow["default_period_end"]     ))? $paramsRow["default_period_end"]       : 0;
				$_SESSION["AppConfig"]["params"]["default_pdf_manager"]        = (isset($paramsRow["default_pdf_manager"]    ))? $paramsRow["default_pdf_manager"]      : "default";
				$_SESSION["AppConfig"]["params"]["default_pdf_header"]         = (isset($paramsRow["default_pdf_header"]     ))? $paramsRow["default_pdf_header"]       : "";
				$_SESSION["AppConfig"]["params"]["default_pdf_footer"]         = (isset($paramsRow["default_pdf_footer"]     ))? $paramsRow["default_pdf_footer"]       : "default";
				
				$_SESSION["AppConfig"]["params"]["default_pdf_margins"]        = (isset($paramsRow["default_pdf_margins"]    ))? $paramsRow["default_pdf_margins"]      : 0;
				$_SESSION["AppConfig"]["params"]["default_pdf_width"]          = (isset($paramsRow["default_pdf_width"]      ))? $paramsRow["default_pdf_width"]        : 0;
				$_SESSION["AppConfig"]["params"]["show_pdf_header"]            = (isset($paramsRow["show_pdf_header"]        ))? $paramsRow["show_pdf_header"]          : 1;
				$_SESSION["AppConfig"]["params"]["show_pdf_footer"]            = (isset($paramsRow["show_pdf_footer"]        ))? $paramsRow["show_pdf_footer"]          : 1;					    			
		...
		if( $moduleName == "public") {
			...
			if(!isset($application->initialized)) {
				$application->initialized    = 1;
				$application->project        = $appConfigSession->project;
