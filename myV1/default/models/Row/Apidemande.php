...
class Model_Apidemande extends Sirah_Model_Default
{
	...
	public function getStatYears()
	{
		...
		$selectStatistiques->group(array("FROM_UNIXTIME(R.date,'%Y')"))->order(array("FROM_UNIXTIME(R.date,'%Y') DESC"))->limitPage(1 , 22);
	...
	public function getList($filters = array() , $pageNum = 0 , $pageSize = 0, $orders=array("R.demandeid DESC","R.date DESC","D.lastname ASC","D.firstname ASC"))
	{
		$modelTable     = $this->getTable();
		$dbAdapter      = $modelTable->getAdapter();
		$tablePrefix    = $modelTable->info("namePrefix");		
		...
		$selectDemandes = $dbAdapter->select()->from(    array("R" => $tablePrefix."reservation_demandes"))
											  ...
											  ->join(    array("E" => $tablePrefix."reservation_demandes_entreprises"),"E.demandeid=R.demandeid",array("E.nomcommercial","E.sigle","E.denomination"))
