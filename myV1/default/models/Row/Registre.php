...
class Model_Registre extends Sirah_Model_Default
{
	...
	public function getListAfter($registreid,$periodStart=0,$limitPage=100,$annee=0,$orders=array("R.registreid DESC"))
	{
		...
		$selectRegistres= $dbAdapter->select()->from(    array("R" => $tablePrefix."rccm_registre" ))
											  ...
											  ->joinLeft(array("E" => $tablePrefix."rccm_registre_entreprises")  ,"E.registreid=R.registreid"          , array("E.responsable","E.responsableid","E.responsable_email","E.num_securite_social","E.num_ifu","E.num_rc","E.reference","E.chiffre_affaire","E.groupid","E.address","E.email","E.phone1","E.phone2","E.siteweb","E.country","E.city","E.zip","E.nbemployes_min","E.nbemployes_max","E.datecreation","E.presentation","E.region","E.formid"))
	...
	public function dirigeantsByPhones($phones) 
	{		
		if( is_string($phones)) {
		...
		$selectDirigeants  = $dbAdapter->select()->from(     array("RE"=>$tablePrefix."rccm_registre_representants"), array("RE.representantid","RE.nom","RE.prenom","RE.adresse","RE.lieunaissance","RE.datenaissance","RE.marital_status","RE.email","RE.telephone","RE.cnib","RE.passport","RE.representantid","RE.sexe","RE.country","RE.city","date_naissance_year"=>"YEAR(RE.datenaissance)","date_naissance_month"=>"MONTH(RE.datenaissance)","date_naissance_day"=>"DAYOFMONTH(RE.datenaissance)","RE.creationdate","RE.creatorid"))
		                                         ->joinLeft( array("D" =>$tablePrefix."rccm_registre_dirigeants"),"D.representantid=RE.representantid" ,array("D.fonction","profession"=>"D.fonction","D.registreid","D.idFonction"))
											     ->joinLeft( array("F" =>$tablePrefix."rccm_registre_dirigeants_fonctions"),"F.idFonction=D.idFonction",array("codeFonction"=>"F.code","code_qres"=>"F.qualiteCode","qualite"=>"F.libelle"))
        ...
        foreach( $phones as $phone ) {
			     $clauses[] = sprintf("(RE.telephone LIKE '%%".$phone."%%')");
		}	
		if( count($clauses) > 1 ) {
			$selectDirigeants->where(implode(" OR ", $clauses));
		} elseif(isset($clauses[0])){
			$selectDirigeants->where($clauses[0]);
		}
	...
	{		
		...
		$tablePrefix       = $table->info("namePrefix");		
		$selectDirigeants  = $dbAdapter->select()->from(     array("RE"=>$tablePrefix."rccm_registre_representants"), array("RE.representantid","RE.nom","RE.prenom","RE.adresse","RE.lieunaissance","RE.datenaissance","RE.marital_status","RE.email","RE.telephone","RE.cnib","RE.passport","RE.representantid","RE.sexe","RE.country","RE.city","date_naissance_year"=>"YEAR(RE.datenaissance)","date_naissance_month"=>"MONTH(RE.datenaissance)","date_naissance_day"=>"DAYOFMONTH(RE.datenaissance)","RE.creationdate","RE.creatorid"))
		                                         ->join(     array("D" =>$tablePrefix."rccm_registre_dirigeants"),"D.representantid=RE.representantid" ,array("D.fonction","profession"=>"D.fonction","D.registreid","D.idFonction"))
											     ->joinLeft( array("F" =>$tablePrefix."rccm_registre_dirigeants_fonctions"),"F.idFonction=D.idFonction",array("codeFonction"=>"F.code","code_qres"=>"F.qualiteCode","qualite"=>"F.libelle"))
	...
	public function getNbreLocaliteAnnee( $localiteid, $annee, $creatorid =0,$filters=array())
	{
		...
		if( isset($filters["periodend"]) && intval($filters["periodend"]) ){
			$selectStatistiques->where("R.creationdate <= ? " , intval($filters["periodend"]));
		}
		if( isset($filters["periodstart"]) && intval($filters["periodstart"]) ){
			$selectStatistiques->where("R.creationdate >= ? " , intval($filters["periodstart"]));
		}
	...
	public function getListMissings( $filters = array() , $pageNum = 0 , $pageSize = 0)
	{
		...
		$dbAdapter      = $table->getAdapter();
		$tablePrefix    = $table->info("namePrefix");
		$selectRegistres = $dbAdapter->select()->from(array("R"=> $tablePrefix ."rheaweb_registres_missings"), array("R.registreid","R.numero","R.rheanum","R.observations","R.numkey","R.annee","localiteCode"=>"R.localite","R.localiteid","R.found"))
	                                          ->join(array("L"=> $tablePrefix ."rccm_localites"), "L.localiteid= R.localiteid", array("localite"=>"L.libelle"));
		...
		if( isset($filters["searchQ"]) && !empty($filters["searchQ"])){
			...
			$likeNumRheaweb = new Zend_Db_Expr("R.rheanum LIKE \"%".strip_tags($filters["searchQ"])."%\"");
			$likeNumKey     = new Zend_Db_Expr("R.numkey  LIKE \"%".strip_tags($filters["searchQ"])."%\"");
			$selectRegistres->where("{$likeNumero} OR {$likeNumRheaweb} OR {$likeNumKey}");
		}
		if( isset($filters["annee"]) && intval($filters["annee"])   ){
			$selectRegistres->where("R.annee = ?", intval( $filters["annee"] ));
		}
		...
		$selectRegistres->order(array("R.annee ASC","R.numkey ASC","R.numero ASC","R.localiteid DESC"));
		if( intval($pageNum) && intval($pageSize)) {
			$selectRegistres->limitPage( $pageNum , $pageSize);
		}		
		$registres     = array();
		try {
			$registres = $dbAdapter->fetchAll( $selectRegistres, array() , Zend_Db::FETCH_ASSOC);
		} catch(Exception $e) {
			$this->setError($e->getMessage());
			$registres = array();
		}	
	...
	public function getListMissingsPaginator( $filters = array() , $pageNum = 0 , $pageSize = 0)
	{
		...
		$dbAdapter      = $table->getAdapter();
		$tablePrefix    = $table->info("namePrefix");
		$selectRegistres = $dbAdapter->select()->from(array("R"=> $tablePrefix ."rheaweb_registres_missings"), array("R.registreid","R.numero","R.rheanum","R.observations","R.numkey","R.annee","localiteCode"=>"R.localite","R.localiteid"))
	                                          ->join(array("L"=> $tablePrefix ."rccm_localites"), "L.localiteid= R.localiteid", array("localite"=>"L.libelle"));	
	    if( isset($filters["numero"]) && !empty($filters["numero"]) ){
			$selectRegistres->where("R.numero LIKE ?","%".strip_tags($filters["numero"])."%");
		}
		if(isset($filters["searchQ"]) && !empty($filters["searchQ"]) && (null!==$filters["searchQ"])){
			$likeNumero     = new Zend_Db_Expr("R.numero  LIKE \"%".strip_tags($filters["searchQ"])."%\"");
			$likeNumRheaweb = new Zend_Db_Expr("R.rheanum LIKE \"%".strip_tags($filters["searchQ"])."%\"");
	...
	public function getSimilarList($searchQuery, $split_length= 4, $pageNum = 0 , $pageSize = 0, $orders = array("R.numero ASC","L.libelle ASC","R.libelle ASC","R.date DESC"))
	{
		...
		$dbAdapter      = $table->getAdapter();
		$tablePrefix    = $table->info("namePrefix");
		$selectRegistres = $dbAdapter->select()->from(array("R" => $tablePrefix ."rccm_registre" ), array("R.registreid","R.numero","R.libelle","R.description","R.date","R.category","R.type","date_registre" => "FROM_UNIXTIME(R.date,'%d/%m/%Y')","annee" => "FROM_UNIXTIME(R.date,'%Y')"))
		                                      ->join(array("RP"=> $tablePrefix ."rccm_registre_dirigeants")   , "RP.registreid     = R.registreid",array("RP.representantid"))
		...
		$selectRegistres->where("{$likeLibelle}");
	...
	public function simpleList( $filters = array() , $pageNum = 0 , $pageSize = 0, $orders = array("R.numero ASC","R.libelle ASC","R.date DESC") )
	{
		...
		$dbAdapter      = $table->getAdapter();
		$tablePrefix    = $table->info("namePrefix");
		$selectRegistres = $dbAdapter->select()->from(array("R"=>$tablePrefix."rccm_registre" ), array("R.registreid","R.type","R.localiteid","R.domaineid","R.numero","R.libelle","R.description","R.date","date_registre"=>"FROM_UNIXTIME(R.date,'%d/%m/%Y')","annee"=>"R.annee"));
		...
		if( isset($filters["type"]) && intval($filters["type"])  ) {
			$selectRegistres->where("R.type=?", intval( $filters["type"] ) );
		}
		if( isset($filters["types"]) && count($filters["types"])  && is_array($filters["types"])) {
			$selectRegistres->where("R.type IN (?)", array_map("intval",$filters["types"]));
		}
		if( isset($filters["date"]) && !empty($filters["date"]) && (null !== $filters["date"] ) ) {
		...
		if( isset($filters["periode_end"]) && intval($filters["periode_end"]) ){
			$selectRegistres->where("R.creationdate<=?", intval($filters["periode_end"]));
		}
		if( isset($filters["periode_start"]) && intval($filters["periode_start"]) ){
			$selectRegistres->where("R.creationdate>=?", intval($filters["periode_start"]));
		}
	...
	public function basicList( $filters = array() , $pageNum = 0 , $pageSize = 0, $orders = array("R.numero ASC","L.libelle ASC","R.libelle ASC","R.date DESC") )
	{
		...
		$dbAdapter      = $table->getAdapter();
		$tablePrefix    = $table->info("namePrefix");
		$selectRegistres= $dbAdapter->select()->from(array("R" => $tablePrefix."rccm_registre" ), array("R.registreid","R.type","R.localiteid","R.domaineid","R.numero","R.libelle","R.description","R.date","date_registre"=>"R.date","annee"=>"R.annee"))
		                                      ->join(array("RP"=> $tablePrefix."rccm_registre_dirigeants")   ,"RP.registreid=R.registreid",array("RP.representantid"))
		...
		if( isset($filters["searchQ"]) && !empty($filters["searchQ"]) ){
			...
			if( $this->checkNum($filters["searchQ"])) {
				...
				$selectRegistres->where("{$likeRCName}");
			}
		...
		if( isset($filters["name"]) && !empty($filters["name"]) ){
			...
			if( empty( $nom ) ) {
				$selectRegistres->where(new Zend_Db_Expr("RE.nom LIKE \"%".strip_tags($nom)."%\""));
			}
			if( empty( $prenom ) ) {
				$selectRegistres->where(new Zend_Db_Expr("RE.prenom LIKE \"%".strip_tags($prenom)."%\""));
			}
		}
		if( isset($filters["nom"]) && !empty($filters["nom"]) ){
		...
		if( isset($filters["registreid"]) && intval($filters["registreid"])  ) {
			$selectRegistres->where("R.registreid = ?", intval( $filters["registreid"] ) );
		}
		if( isset( $filters["registreids"] ) && is_array( $filters["registreids"] )) {
			if( count( $filters["registreids"])) {
				$selectRegistres->where("R.registreid IN (?)", array_map("intval",$filters["registreids"]));
		...
		if( isset( $filters["domaineids"] ) && is_array( $filters["domaineids"] )) {
			if( count( $filters["domaineids"])) {
				$selectRegistres->where("R.domaineid IN (?)", array_map("intval",$filters["domaineids"]));
			}			
		}
		if( isset( $filters["localiteid"] ) && intval( $filters["localiteid"] ) ) {
			$selectRegistres->where("R.localiteid=?" , intval( $filters["localiteid"] ) );
		...
		if( isset( $filters["localiteids"] ) && is_array( $filters["localiteids"] )) {
			if( count( $filters["localiteids"])) {
				$selectRegistres->where("R.localiteid IN (?)", array_map("intval",$filters["localiteids"]));
			}			
		}
		if( is_array( $orders ) && !empty( $orders) ) {
			$selectRegistres->order( $orders );
		...
		if( intval($pageNum) && intval($pageSize)) {
			...
			$selectRegistres->limitPage(1,10);
		}	
        
		try {
			$selectRegistres->group(array("R.numero","R.registreid","RP.registreid")); 	
		    return $dbAdapter->fetchAll( $selectRegistres, array() , Zend_Db::FETCH_ASSOC);
		} catch(Exception $e) {
			$this->setError($e->getMessage());
			return false;
	...
	public function groupListByType( $filters = array() , $pageNum = 1 , $pageSize = 10, $orders = array("COUNT(R.registreid) DESC") )
	{
		...
		$dbAdapter      = $table->getAdapter();
		$tablePrefix    = $table->info("namePrefix");
		$selectRegistres= $dbAdapter->select()->from(array("R" => $tablePrefix."rccm_registre"),array("nombre"=>"COUNT(R.registreid)","R.localiteid","R.type"))
		                                      ->join(array("RP"=> $tablePrefix."rccm_registre_dirigeants")   ,"RP.registreid=R.registreid",null)
		                                      ->join(array("RE"=> $tablePrefix."rccm_registre_representants"),"RE.representantid=RP.representantid", null);
        ...
        $selectRegistres->group(array("R.type"));
        $registres     = array();		
		try {
		    $registres = $dbAdapter->fetchAll( $selectRegistres, array() , Zend_Db::FETCH_ASSOC);
		} catch(Exception $e) {
	...
	public function groupListByLocality( $filters = array() , $pageNum = 1 , $pageSize = 10, $orders = array("COUNT(R.registreid) DESC") )
	{
		...
		$dbAdapter      = $table->getAdapter();
		$tablePrefix    = $table->info("namePrefix");
		$selectRegistres= $dbAdapter->select()->from(array("R" => $tablePrefix."rccm_registre"),array("nombre"=>"COUNT(R.registreid)","R.localiteid"))
		                                      ->join(array("RP"=> $tablePrefix."rccm_registre_dirigeants")   ,"RP.registreid=R.registreid",null)
		                                      ->join(array("RE"=> $tablePrefix."rccm_registre_representants"),"RE.representantid=RP.representantid", null)
											  ->join(array("L" => $tablePrefix."rccm_localites"),"L.localiteid=R.localiteid", array("localite"=>"L.libelle"));
		...
		if( isset($filters["types"]) && count($filters["types"])) {
			$selectRegistres->where("R.type IN (?)", array_map("intval",$filters["types"]));
		}
		if( isset($filters["date"]) && !empty($filters["date"]) && (null !== $filters["date"] ) ) {
			if( Zend_Date::isDate($filters["date"], "YYYY-MM-dd")) {
        ...
        $selectRegistres->group(array("R.localiteid"));	
        $registres     = array();		
		try {
		    $registres = $dbAdapter->fetchAll( $selectRegistres, array() , Zend_Db::FETCH_ASSOC);
		} catch(Exception $e) {
	...
	{		
		...
		$dbAdapter      = $table->getAdapter();
		$tablePrefix    = $table->info("namePrefix");
		$selectRegistres= $dbAdapter->select()->from(array("R" => $tablePrefix."rccm_registre" )             , array("R.registreid","R.localiteid","R.domaineid","R.numero","R.numifu","R.numcnss","R.libelle","R.description","R.date","R.category","R.type","date_registre"=>"R.date","annee"=>"R.annee"))
		                                      ->join(array("RP"=> $tablePrefix."rccm_registre_dirigeants")   ,"RP.registreid=R.registreid"         , array("RP.representantid"))
		                                      ->join(array("RE"=> $tablePrefix."rccm_registre_representants"),"RE.representantid=RP.representantid", array("RE.nom","RE.prenom","RE.adresse","RE.lieunaissance","RE.datenaissance","RE.passport","RE.sexe","RE.telephone","RE.email"))											  
											  ->join(array("L" => $tablePrefix."rccm_localites")             ,"L.localiteid=R.localiteid"          , array("localite"=>"L.libelle"))
		                                      ->join(array("D" => $tablePrefix."rccm_domaines")              ,"D.domaineid=R.domaineid"            , array("domaine" =>"D.libelle"));
	    if( isset($filters["SANS_IFU"]) && ($filters["SANS_IFU"]==1) ){
			$selectRegistres->where("(R.numifu='') OR (R.numifu IS NULL) OR (R.numifu <> '')");
		}
		if( isset($filters["keywords"]) && !empty($filters["keywords"]) ){
			$emptyWords               = $this->emptywords();
			if( $this->checkNum($filters["keywords"])) {
		...
		if( isset($filters["periodend"])     && intval($filters["periodend"]) ){
			//$selectRegistres->where("R.creationdate<=?", intval($filters["periodend"]));
		}
		if( isset($filters["periodstart"])   && intval($filters["periodstart"]) ){
			//$selectRegistres->where("R.creationdate>=?", intval($filters["periodstart"]));
		}
		if( isset($filters["creatorid"])     && intval($filters["creatorid"])  ) {
			$selectRegistres->where("R.creatorid = ?", intval( $filters["creatorid"] ) );
		}
		...
		$selectRegistres->group(array("R.localiteid","R.domaineid","R.registreid"));
		if( intval($pageNum) && intval( $pageSize)) {
			$selectRegistres->limitPage($pageNum,$pageSize);
		}	
        //print_r($selectRegistres->__toString()); die();		
	...
	public function getListPaginator( $filters = array() )
	{
		...
		try {
			$selectRegistres->group(array("R.registreid","RP.registreid")); 
			$paginationAdapter = new Zend_Paginator_Adapter_DbSelect( $selectRegistres );
			$rowCount          = intval(count($dbAdapter->fetchAll(   $selectRegistres )));
			$paginationAdapter->setRowCount($rowCount);
			$paginator         = new Zend_Paginator($paginationAdapter);
