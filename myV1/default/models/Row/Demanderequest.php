...
class Model_Demanderequest extends Sirah_Model_Default
{
	...
	public function foundrequest($demandeid=0)
	{
		...
		return $dbAdapter->fetchRow($selectOperator,array(),Zend_Db::FETCH_ASSOC);									  
	}
	
	public function noRequested22($demandeid=0)
	{
		$modelTable     = $this->getTable();
		$dbAdapter      = $modelTable->getAdapter();
		$tablePrefix    = $modelTable->info("namePrefix");		
		$tableName      = $modelTable->info("name");
		$selectRequests = $dbAdapter->select()->from(array("R"=> $tablePrefix."reservation_demandes"))
											  ->where("(R.demandeid NOT IN (SELECT OP2.demandeid FROM ".$tablePrefix."reservation_demandes_requests OP2))
													OR (R.demandeid NOT IN (SELECT OP3.demandeid FROM ".$tablePrefix."reservation_demandes_requests_operators OP3))")
											  ->where("R.web=1")
											  ->where("R.statutid IN (1,7,8)")
											  ->order(array("R.creationdate DESC"));
	     
		if( intval($demandeid) ){
			$selectRequests->where("R.demandeid=?",intval($demandeid));
		}
		 
		return $dbAdapter->fetchAll($selectRequests,array(),Zend_Db::FETCH_ASSOC);
	}
	
	public function noRequested($demandeid=0)
	{
		$modelTable     = $this->getTable();
		$dbAdapter      = $modelTable->getAdapter();
		$tablePrefix    = $modelTable->info("namePrefix");		
		$tableName      = $modelTable->info("name");
		$selectRequests = $dbAdapter->select()->from(array("R" => $tablePrefix."reservation_demandes"))
		                                      ->join(array("AP"=> $tablePrefix."api_reservation_demandes"),"(AP.sync_demandeid=R.demandeid) OR (AP.numero=R.reference) OR (AP.denomination=R.denomination AND AP.objet=R.objet)",array("remote_demandeid"=>"AP.demandeid","local_demandeid"=>"AP.sync_demandeid"))
											  ->where("(AP.demandeid NOT IN (SELECT OP2.demandeid FROM ".$tablePrefix."reservation_demandes_requests OP2))
													OR (AP.demandeid NOT IN (SELECT OP3.demandeid FROM ".$tablePrefix."reservation_demandes_requests_operators OP3))")
	...
	public function hasSent($demandeid,$memberid=0,$email=null)
	{
		$table          = $this->getTable();
		$dbAdapter      = $table->getAdapter();
		$tablePrefix    = $table->info("namePrefix");
		$tableName      = $table->info("name");		
		$selectDemande  = $dbAdapter->select()->from(array("O"=> $tablePrefix."reservation_demandes_notifications"),array("O.demandeid"))											  
											  ->where("O.demandeid=?" ,intval($demandeid))
											  ->group(array("O.demandeid","O.memberid","O.demandeid"));
	    if( intval($memberid) ) {
			$selectDemande->where("O.memberid=?",intval($memberid));
		}
		if( null == $email) {
			$selectDemande->where("O.email=?", $email);
		}
		return $dbAdapter->fetchOne($selectDemande);
	}
	
	
	public function hasNotified($requestid,$operatorid,$demandeid=null)
	{
		$table          = $this->getTable();
		$dbAdapter      = $table->getAdapter();
		$tablePrefix    = $table->info("namePrefix");
		$tableName      = $table->info("name");		
		$selectRequest  = $dbAdapter->select()->from(array("O"=> $tablePrefix."reservation_demandes_requests_operators"),array("O.sent"))
		                                      ->join(array("R"=> $tableName),"R.requestid=O.requestid",null)
											  ->join(array("D"=> $tablePrefix."reservation_demandes"),"(D.sync_demandeid=R.demandeid) OR (D.libelle=R.objet)",null)
											  ->where("O.operatorid=?",intval($operatorid))
											  ->where("O.requestid=?" ,intval($requestid))
											  ->group(array("O.requestid","O.operatorid","O.demandeid"));
	    if( null!== $demandeid ) {
			$selectRequest->where("O.demandeid=?", intval($demandeid));
		}
		return $dbAdapter->fetchOne($selectRequest);
	}
	
	public function demandExist($operatorid,$demandeurid,$denomination)
	{
		$table          = $this->getTable();
		$dbAdapter      = $table->getAdapter();
		$tablePrefix    = $table->info("namePrefix");
		...
		$selectRequest  = $dbAdapter->select()->from(array("O"=> $tablePrefix."reservation_demandes_requests_operators"),array("O.sent"))
											  ...
											  ->join(array("D"=> $tablePrefix."reservation_demandes"),"D.sync_demandeid=R.demandeid OR D.libelle=R.objet",null)
											  ->where("O.operatorid=?",intval($operatorid))
											  ->group(array("O.operatorid","O.demandeid","O.requestid"));
	    if(!empty($denomination)) {
			$selectRequest->where("D.denomination LIKE ?",$denomination);
		}			
		if( intval($demandeurid) ) {
			$selectRequest->where("D.demandeurid=?",intval($demandeurid));
		}
		return $dbAdapter->fetchOne($selectRequest);
	}
	
	public function demanderequestHasNotified($demandeid,$statutid=null)
	{
		...
		$selectOperator = $dbAdapter->select()->from(array("R"=> $tableName))
											  ...
											  ->where("O.sent=1");
		if( null!= $statutid) {
			$selectOperator->join(array("D"=>$tablePrefix."reservation_demandes"),"D.demandeid=R.demandeid",array("D.statutid"))
			               ->where("D.statutid=?", intval($statutid));
	...
	public function nonProcessedRequests($periodStart=0,$creatorid=0,$notified=0,$limitSize=200,$order="DESC")
	{
		$modelTable     = $this->getTable();
		$dbAdapter      = $modelTable->getAdapter();
		$tablePrefix    = $modelTable->info("namePrefix");		
		$tableName      = $modelTable->info("name");
		$selectRequests = $dbAdapter->select()->from(    array("RQ"=> $tableName), array("remote_demandeid"=>"RQ.demandeid","RQ.requestid","RQ.processed" ,"RQ.validated","RQ.notified","RQ.operatorid","RQ.updatedate","RQ.creationdate"))
		                                      ->join(    array("R" => $tablePrefix."reservation_demandes")             ,"(R.sync_demandeid=RQ.demandeid) OR (R.libelle=RQ.objet)", "R.*")
											  ...
											  ->join(    array("E" => $tablePrefix."reservation_demandes_entreprises") ,"(E.demandeid=R.demandeid) OR (E.nomcommercial=R.denomination AND E.demandeurid=R.demandeurid AND E.promoteurid=R.promoteurid)", array("E.nomcommercial","E.sigle","E.denomination","E.formid","E.address","E.activite","E.telephone","E.domaineid"))
											  ...
											  ->where(new Zend_Db_Expr("R.statutid IN (1,7,8)"))
		...
		if( intval($creatorid) ){
			...
			$selectRequests->where("(RQ.operatorid='".$creatorid."')  OR (RQ.accountid='".$creatorid."') OR (R.creatorid='".$creatorid."') OR (RQ.requestid IN (SELECT OP.requestid FROM ".$tablePrefix."reservation_demandes_requests_operators OP WHERE OP.operatorid='".$creatorid."'))");
		}
		if(!empty($order) && ($order=="ASC")) {
			$selectRequests->order(array("R.date ASC" ,"R.demandeid ASC","RQ.creationdate ASC"));
		} else {
			$selectRequests->order(array("R.date DESC","R.demandeid DESC","RQ.creationdate DESC"));
		}
		if( intval($limitSize)) {
			$selectRequests->limitPage(1, $limitSize);
		}
		$selectRequests->group(array("R.localiteid","R.demandeurid","R.promoteurid","R.demandeid"));
		return $dbAdapter->fetchAll($selectRequests,array(),Zend_Db::FETCH_ASSOC);
	}
	
	public function processedRequests($periodStart=0,$creatorid=0,$notified=0,$limitSize=0,$order="DESC")
	{
		$modelTable     = $this->getTable();
		$dbAdapter      = $modelTable->getAdapter();
		$tablePrefix    = $modelTable->info("namePrefix");		
		$tableName      = $modelTable->info("name");
		$selectRequests = $dbAdapter->select()->from(    array("RQ"=> $tableName), array("request_numero"=>"RQ.numero" ,"request_demandeid"=>"RQ.demandeid","RQ.requestid","RQ.processed" ,"RQ.validated","RQ.notified","RQ.operatorid"))
		                                      ->join(    array("AP"=> $tablePrefix."api_reservation_demandes")         ,"AP.demandeid=RQ.demandeid"    , array("remote_demandeid"=>"AP.demandeid","remote_numero"=>"AP.numero","remote_reference"=>"AP.reference","local_demandeid"=>"AP.sync_demandeid","remote_demandeurid"=>"AP.demandeurid","remote_promoteurid"=>"AP.promoteurid"))
											  ->join(    array("R" => $tablePrefix."reservation_demandes")             ,"R.demandeid=AP.sync_demandeid", "R.*")
											  
											  ->join(    array("RD"=> $tablePrefix."reservation_demandeurs")           ,"RD.demandeurid=R.demandeurid" , array("RD.accountid","demandeurName"=>"RD.name","demandeurLastname"=>"RD.lastname","demandeurFirstname"=>"RD.firstname","demandeurIdentite"=>"RD.numidentite","demandeurPhone"=>"RD.telephone","demandeurEmail"=>"RD.email"))
											  ->join(    array("P" => $tablePrefix."reservation_promoteurs")           ,"P.promoteurid=R.promoteurid"  , array("promoteurName"=>"P.name","promoteurLastname"=>"P.lastname","promoteurFirstname"=>"P.firstname","promoteurIdentite"=>"P.numidentite","promoteurPhone"=>"P.telephone","promoteurEmail"=>"P.email"))
											  ->joinLeft(array("E" => $tablePrefix."reservation_demandes_entreprises") ,"(E.demandeid=R.demandeid) AND (E.nomcommercial=R.denomination OR E.entrepriseid=R.entrepriseid)", array("E.nomcommercial","E.sigle","E.formid","E.address","E.activite","E.telephone","E.domaineid"))
											  ->joinLeft(array("S" => $tablePrefix."reservation_demandes_statuts")     ,"S.statutid=R.statutid"        , array("statut"       =>"S.libelle"))
											  ->join(    array("DC"=> $tablePrefix."reservation_demandes_documents")   ,"DC.demandeid=R.demandeid"     , array("documentitle" =>"DC.libelle"))
											  ->join(    array("D" => $tablePrefix."system_users_documents")           ,"D.documentid=DC.documentid"   , array("D.documentid","D.userid","D.filename","D.filepath","D.resource","D.resourceid","D.category","D.filemetadata","D.filextension","D.filedescription","D.filesize","D.access"))
											  ->joinLeft(array("M" => $tablePrefix."rccm_members")                     ,"(M.accountid=RD.accountid) OR (M.email=RD.email)", array("M.memberid","memberName"=>"M.name","M.sexe","M.email","M.tel1"))
											  ->joinLeft(array("CT"=> $tablePrefix."system_users_documents_categories"),"CT.id=D.category"             , array("documentCategorie"=>"CT.libelle","CT.icon","documentCatId"=>"CT.id"))
											  ->where("R.statutid IN (2,3,4,5,6)");
	    if( intval($periodStart) ) {
		...
		if( $notified===0 || $notified===1 ){
			...
			if( $notified===0 ) {
				//$selectRequests->where("RQ.demandeid NOT IN (SELECT N.demandeid FROM reservation_demandes_notifications N)");
			}
		}
		if(!empty($order) && ($order=="ASC")) {
			$selectRequests->order(array("R.updatedate ASC","R.date ASC","R.demandeid ASC","R.creationdate ASC"));
		} else {
			$selectRequests->order(array("R.updatedate DESC","R.date DESC","R.demandeid DESC","R.creationdate DESC"));
		}
		if( intval($limitSize)) {
			$selectRequests->limitPage(1, $limitSize);
		}
