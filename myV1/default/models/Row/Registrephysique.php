...
class Model_Registrephysique extends Sirah_Model_Default
{
	...
	public function getNumParts($numRccm)
	{
		...
		if( strlen($numRccm)>18) {
			$pattern = "/BF\-(?<localite>[a-zA-Z])\-(<codeJuridiction>[0-2]{2})\-(?<annee>[0-9]{4})\-(?<typeRCCM>(?:A|E|T)[0-9]{1,2})\-?(?<numero>[0-9]{5})$/i";		    
		} else {
			$pattern = "/BF\-?(?<localite>[a-zA-Z]{2,3})\-?(?<annee>[0-9]{4})\-?(?<typeRCCM>(?:A|E|T)[0-9]{0,2})\-?(?<numero>[0-9]{4,5})$/i";
		}		
	...
	public function checkNum($numRccm)
	{
		...
		if( strlen($numRccm)>18) {
			$pattern = "/BF\-[a-zA-Z]{2,3}\-[0-2]{2}\-[0-9]{4}\-(?:A|E|T)[0-9]{2}\-[0-9]{5}$/i";
		} else {
			$pattern = "/BF\-?[a-zA-Z]{2,3}\-?[0-9]{0,2}\-?[0-9]{4}\-?(?:A|E|T)[0-9]{0,2}\-?[0-9]{4,5}$/i";
		}
	...
	public function normalizeNum($numRccm,$annee=null,$localiteCode=null,$countryCode="BF",$idLength=4)
	{
		...
		$rccmNormalTypes       = array("A","D","E","G","T");
		$numRccm               = preg_replace("/\s+/"," ",str_ireplace(array(" -"," - "),"-",trim($numRccm)));
	...
	public function getSimilarList($searchQuery, $split_length= 4, $pageNum = 0 , $pageSize = 0, $orders = array("R.numero ASC","L.libelle ASC","R.libelle ASC","R.date DESC"))
	{
		...
		$dbAdapter      = $table->getAdapter();
		$tablePrefix    = $table->info("namePrefix");
		$selectRegistres= $dbAdapter->select()->from(array("R" => $tablePrefix."rccm_registre" ), array("R.registreid", "R.numero", "R.libelle", "R.description", "R.date","R.category", "annee" => "FROM_UNIXTIME(R.date,'%Y')"))
	   ...
	   if( empty( $searchQuery )) {
		...
		$likeLibelle       = new Zend_Db_Expr("MATCH(R.numero,R.libelle) AGAINST (\"".$againstValue."\")");
	...
	public function getList( $filters = array() , $pageNum = 0 , $pageSize = 0, $orders = array("R.numero ASC","L.libelle ASC","R.libelle ASC","R.date DESC"))
	{
		...
		$selectRegistres= $dbAdapter->select()->from(array("R" => $tablePrefix."rccm_registre" ), array("R.registreid","R.numero","R.numifu","R.numcnss","R.libelle","R.description","R.date","R.category", "annee" => "FROM_UNIXTIME(R.date,'%Y')"))
		                                      ...
		                                      ->join(array("L" => $tablePrefix."rccm_localites"), "L.localiteid= R.localiteid", array("localite"=>"L.libelle"))
		                                      ->joinLeft(array("D"=> $tablePrefix ."rccm_domaines" ), "D.domaineid = R.domaineid ", array("domaine" =>"D.libelle"))->where("R.type=1");
